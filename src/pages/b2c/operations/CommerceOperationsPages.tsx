...
function ClaimPage({ kind, history = false }: { kind: ClaimKind; history?: boolean }) {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{['전체', ...config.stages].map((status) => <button type="button" key={status} className={`${shared.qfBtn} ${quick === status ? styles.quickActive : ''}`} onClick={() => setQuick(status)}><span className={shared.qfLabel}>{status}</span><span className={shared.qfCount}>{records.filter((item) => status === '전체' || item.stage === status).length}</span></button>)}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>통합 검색</option><option>처리번호</option><option>주문번호</option><option>회원</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder={`${config.label}번호 / 주문번호 / 회원 / 상품 / 사유`}/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={assignee} onChange={(event) => setAssignee(event.target.value)}><option value="">전체 담당자</option><option value="미배정">미배정</option><option value="배정">배정 완료</option></select><span>접수일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-20"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>필터 초기화</button></div></FilterBox></ControlArea>
