...
export function AdminHistoryPage() {
    ...
    () =>
      LOGIN_LOGS.filter((l) => {
        ...
        if (search) {
          ...
          if (
            !(
              l.adminId.toLowerCase().includes(k) ||
              adminName(l.adminId).toLowerCase().includes(k) ||
              l.ip.includes(k)
            )
          )
            return false;
    ...
    () =>
      ACTION_LOGS.filter((a) => {
        ...
        if (search) {
          ...
          if (
            !(
              a.adminId.toLowerCase().includes(k) ||
              adminName(a.adminId).toLowerCase().includes(k) ||
              a.targetId.toLowerCase().includes(k) ||
              a.ip.includes(k)
            )
          )
            return false;
  ...
  const loginRows: GridRow[] = filteredLogins.map((l) => {
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: `${l.adminId} · ${adminName(l.adminId)}`,
        color: "#3f3f46",
        size: "12px",
        weight: 500,
      },
      {
        kind: "badge",
        text: l.result,
        bg: l.result === "성공" ? "#ecfdf5" : "#fef2f2",
        fg: l.result === "성공" ? "#059669" : "#b91c1c",
      },
      {
        kind: "text",
        text: l.ip,
  ...
  const actionRows: GridRow[] = filteredActions.map((a) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: `${a.adminId} · ${adminName(a.adminId)}`,
        color: "#3f3f46",
        size: "12px",
        weight: 500,
      },
  ...
  const total =
    tab === "로그인 이력" ? filteredLogins.length : filteredActions.length;
  ...
  return (
    <div className={styles.page}>
      ...
      {loginDrawer && (
        <AdminHistoryDetailDrawer
          kind="login"
          item={loginDrawer}
          onClose={() => setLoginDrawer(null)}
        />
      )}
      {actionDrawer && (
        <AdminHistoryDetailDrawer
