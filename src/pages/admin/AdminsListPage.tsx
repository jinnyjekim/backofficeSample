...
const GRID_TEMPLATE =
  "72px minmax(90px,1fr) minmax(130px,1.2fr) minmax(120px,1.3fr) 66px 124px 86px";
...
type EditorState =
  | { mode: "create" }
  | { mode: "edit"; admin: AdminAccount }
  | null;
type ConfirmState = {
  kind: "resetPassword" | "deactivate";
  admin: AdminAccount;
} | null;
...
function history(
  ...
  return {
    ...
    history: [
      ...a.history,
      {
        id: `H-${a.id}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        at: `${TODAY} 15:00`,
        by: "admin001",
        action,
        detail,
...
export function AdminsListPage() {
  ...
  const issuesMap = useMemo(() => {
    ...
    admins.forEach((a) => {
      map[a.id] = computeIssues(a, admins);
    });
    return map;
  }, [admins]);
  ...
  const statusCounts = useMemo(() => {
    ...
    ADMIN_STATUSES.forEach((s) => {
      c[s] = admins.filter((a) => a.status === s).length;
    });
    return c;
  }, [admins]);
  ...
  const selected = drawerId
    ? (admins.find((a) => a.id === drawerId) ?? null)
    : null;
  ...
  function unlock(a: AdminAccount) {
    update(a.id, (x) =>
      history({ ...x, status: "정상", loginFailCount: 0 }, "로그인 잠금 해제"),
    );
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "resetPassword") {
      update(confirm.admin.id, (a) =>
        history(a, "비밀번호 재설정 안내 발송", confirm.admin.email),
      );
      toastBriefly(
        `${confirm.admin.email} 으로 비밀번호 재설정 안내를 발송했습니다.`,
      );
    } else if (confirm.kind === "deactivate") {
      update(confirm.admin.id, (a) =>
        history({ ...a, status: "비활성" }, "계정 비활성화"),
      );
  ...
  const rows: GridRow[] = filtered.map((a) => {
    ...
    const rolesLabel =
      a.roleIds.length > 1
        ? `${roleName(a.roleIds[0])} +${a.roleIds.length - 1}`
        : a.roleIds.length === 1
          ? roleName(a.roleIds[0])
          : "없음";
    const cells: Cell[] = [
      {
        kind: "text",
      ...
      {
        kind: "text",
        text: a.lastLoginAt ?? "이력 없음",
        color: a.lastLoginAt ? "#52525b" : "#a1a1aa",
        size: "11.5px",
        weight: 500,
        numeric: true,
      },
    ...
    <div className={styles.page} onClick={() => menuId && setMenuId(null)}>
        ...
        <div className={styles.headRow}>
          <div>
            ...
            <div className={styles.subtitle}>
              백오피스에 접근할 수 있는 관리자 계정을 조회하고 관리합니다.
            </div>
          </div>
          <button
            type="button"
      ...
      <div className={styles.gridWrap}>
        <DataGrid
          ...
          emptySubtext={
            admins.length === 0
              ? "백오피스에 접근할 관리자를 등록해 주세요."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={admins.length === 0 ? "＋ 관리자 등록" : "초기화"}
          emptyActionClick={
            admins.length === 0
      ...
      {selected && (
        <AdminDetailDrawer
          ...
          onResetPassword={() =>
            setConfirm({ kind: "resetPassword", admin: selected })
          }
          onUnlock={() => unlock(selected)}
          onToggleActive={() => toggleActive(selected)}
          onAddMemo={(text) =>
            update(selected.id, (a) => ({
              ...a,
              memos: [
                ...a.memos,
                {
        ...
        >
          <div className={styles.dialogBox}>
            <div className={styles.dialogTitle}>
              비밀번호 재설정 안내를 발송하시겠습니까?
            </div>
            <div className={styles.dialogBody}>
              {confirm.admin.email} 으로 비밀번호 재설정 안내가 발송됩니다.
            </div>
      ...
      {confirm?.kind === "deactivate" && (
        ...
        >
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogBody}>
              비활성화 후 해당 관리자는 백오피스에 로그인할 수 없습니다. 기존
              작업 이력은 유지됩니다.
            </div>
            <div className={styles.dialogActions}>
              <button
