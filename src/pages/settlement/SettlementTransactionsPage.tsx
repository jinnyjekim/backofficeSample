...
export function SettlementTransactionsPage() {
  ...
  const targetOptions = useMemo(
    () => ["전체", ...new Set(all.map((item) => item.target))],
    [all],
  );
  const statusOptions = useMemo(
    () => ["전체", ...new Set(all.map((item) => item.status))],
    [all],
  );
  const monthOptions = useMemo(
    () => ["전체", ...new Set(all.map((item) => item.date.slice(0, 2) + "월"))],
    [all],
  );
  const filtered = useMemo(
    () =>
      all.filter((t) => {
        ...
        if (month !== "전체" && `${t.date.slice(0, 2)}월` !== month)
          return false;
        return true;
      }),
  ...
  const rows: GridRow[] = filtered.map((t, i) => ({
    ...
    cells: [
      ...
      },
      {
        kind: "badge",
        text: t.type,
        bg: t.type === "환불" ? "#fef2f2" : "#f4f4f5",
        fg: t.type === "환불" ? "#b91c1c" : "#52525b",
      },
      {
        kind: "text",
        text: t.settlementId,
        ...
        <div className={styles.filterBox}>
          <div className={styles.filterRow1}>
            <input
              ...
              placeholder="거래번호, 정산번호 또는 정산대상으로 검색"
            />
