...
export function CancelApprovalPage() {
  ...
  const approvalItems = useMemo(() => items.filter((it) => it.stage === '취소 승인'), [items]);
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              history: [
                ...item.history,
                { when: '08.27 16:15', title: 'PG사 결제 취소 및 환불 완료 처리', by: 'admin01' },
              ],
            }
          : item
      )
    );
    showToast({ message: '환불이 정상 완료되었습니다. (취소 완료 메뉴로 이동)', type: 'success' });
    setSelectedId(null);
  };
  ...
  const rows: GridRow[] = filtered.map((item) => {
    return {
      ...
      cells: [
        ...
        { kind: 'text', text: item.paymentMethod, color: '#52525b', size: '12px' },
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>승인 완료된 취소 건의 PG 결제 취소 및 계좌 환불 대기 상태를 관리합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 환불 대기`}</span>
      ...
      <SplitPaneLayout
        ...
        <DetailDrawer
          ...
          stats={[
            { label: '환불 예정액', value: `${selected.cancelAmount.toLocaleString()}원` },
        ...
        >
          ...
          <div style={{ fontSize: '13px', lineHeight: 1.6, color: '#334155', background: '#f8fafc', padding: '12px 14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            [환불 실행] 버튼을 클릭하면 {selected.paymentMethod}을(를) 통해 자동 승인 취소 전문이 전송됩니다.
          </div>
        </DetailDrawer>
          )
        }
        emptyMessage={<>왼쪽 목록에서 건을 선택하면<br />취소 승인 및 환불 실행 상세가 여기에 표시됩니다.</>}
      />
    </div>
  );
}
