...
export function CancelHistoryPage() {
  ...
  const rows: GridRow[] = filtered.map((item) => {
    ...
    const latestDate = item.completedAt || item.rejectedAt || item.approvedAt || item.requestedAt;
    return {
      id: item.id,
      ...
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>주문 취소 신청 접수부터 승인, 반려 및 최종 환불 처리까지의 전체 감사 로그를 조회합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 취소 이력`}</span>
      ...
      {selected && (
        <DetailDrawer
          ...
          stats={[
            ...
            { label: '환불 금액', value: `${selected.cancelAmount.toLocaleString()}원` },
          ]}
          fields={[
