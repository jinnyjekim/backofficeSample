...
const GRID_TEMPLATE =
  "140px 140px 90px 90px 110px minmax(200px, 1fr) 90px 120px";
...
export function CancelRequestsPage() {
  ...
  const requestItems = useMemo(
    () => items.filter((it) => it.stage === "취소 요청"),
    [items],
  );
  ...
  const handleApprove = (id: string) => {
    ...
    showToast({
      message: "취소 요청이 승인되었습니다. (취소 승인 메뉴로 이동)",
      type: "success",
    });
    setSelectedId(null);
  };
  ...
  const handleReject = (id: string) => {
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              rejectReason:
                "이미 출고 작업이 진행되어 취소가 불가합니다. 상품 수령 후 반품으로 신청해 주세요.",
              assignee: "admin01",
              history: [
                ...item.history,
                {
                  when: "08.27 16:10",
                  title: "출고 진행으로 인한 취소 반려",
                  by: "admin01",
                },
              ],
            }
    ...
    showToast({
      message: "취소 요청이 반려되었습니다. (취소 반려 메뉴로 이동)",
      type: "info",
    });
    setSelectedId(null);
  };
  ...
  const rows: GridRow[] = filtered.map((item) => {
    const rm = CANCEL_REASON_META[item.reasonCategory] ?? {
      bg: "#f4f4f5",
      fg: "#52525b",
    };
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: item.cancelType,
          bg: item.cancelType === "부분 취소" ? "#eff6ff" : "#f4f4f5",
          fg: item.cancelType === "부분 취소" ? "#1d4ed8" : "#3f3f46",
        },
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              고객이 결제 후 신청한 주문 취소 건을 검토하고 승인하거나 반려를
              처리합니다.
            </div>
          </div>
        </div>
          ...
          <div className={styles.filterRow1}>
            ...
            <div className={styles.quickFilters}>
              {QUICK_FILTERS.map((k) => {
                ...
                const count = requestItems.filter(
                  (item) => k === "전체" || item.reasonCategory === k,
                ).length;
                return (
                  <CommonButton
                    key={k}
      ...
      {selected && (
        <DetailDrawer
          ...
          fields={[
            ...
            {
              label: "쿠폰 차감",
              value: selected.deductCoupon
                ? `${selected.deductCoupon.toLocaleString()}원 차감 환불`
                : "없음",
            },
          ]}
        >
