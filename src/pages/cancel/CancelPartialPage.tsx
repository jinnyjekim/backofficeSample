...
const GRID_TEMPLATE =
  "140px 140px 90px minmax(200px, 1fr) 100px 90px 90px 100px";
...
export function CancelPartialPage() {
  ...
  const partialItems = useMemo(
    () => items.filter((it) => it.cancelType === "부분 취소"),
    [items],
  );
  ...
  const rows: GridRow[] = filtered.map((item) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "text",
          text: `${item.totalOrderAmount.toLocaleString()}원`,
          align: "right",
          size: "12px",
          numeric: true,
        },
        {
          kind: "text",
          text: item.deductCoupon
            ? `-${item.deductCoupon.toLocaleString()}원`
            : "0원",
          align: "right",
          size: "12px",
          color: "#b45309",
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              주문 내 일부 품목 취소 건의 할인 쿠폰 및 적립금 재계산 환불 내역을
              조회합니다.
            </div>
          </div>
        </div>
          ...
          stats={[
            ...
            },
            {
              ...
              value: selected.deductCoupon
                ? `${selected.deductCoupon.toLocaleString()}원`
                : "0원",
            },
          ]}
          fields={[
