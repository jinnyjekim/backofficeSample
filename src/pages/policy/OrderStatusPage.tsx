...
type ConfirmState =
  | { kind: "deactivate"; item: OrderStatusEntry }
  | { kind: "delete"; item: OrderStatusEntry }
  | null;
...
function natureLabel(s: OrderStatusEntry): {
  text: string;
  bg: string;
  fg: string;
} {
...
export function OrderStatusPage() {
  ...
  const warnings = useMemo(
    () => computeValidationWarnings(statuses, transitions),
  ...
  const save = (item: OrderStatusEntry) => {
    if (isNew) {
      const saved = history(
        { ...item, order: statuses.length + 1, history: [] },
        "상태 생성",
      );
      ...
      const saved =
        previous && previous.name !== item.name
          ? history(item, "상태명 변경", previous.name, item.name)
          : history(item, "설정 수정");
      setStatuses((current) =>
        current.map((s) => (s.id === item.id ? saved : s)),
      );
      setDrawerItem(saved);
  ...
  const deactivate = (item: OrderStatusEntry) => {
    if (item.orderCount > 0)
      return toastBriefly(
        `현재 ${item.orderCount.toLocaleString()}건의 주문이 이 상태입니다. 처리 방안을 확인한 후 비활성화해 주세요.`,
      );
  ...
  const confirmAction = () => {
    ...
    if (confirm.kind === "delete") {
      ...
      setTransitions((current) =>
        current.filter(
          (t) => t.from !== confirm.item.id && t.to !== confirm.item.id,
        ),
      );
      setDrawerItem(null);
      ...
      setDrawerItem(null);
      toastBriefly(
        "상태를 비활성화했습니다. 신규 주문은 이 상태로 진입할 수 없습니다.",
      );
    }
    setConfirm(null);
  };
  ...
  const saveTransition = (
    ...
    if (existing) {
      setTransitions((current) =>
        current.map((t) => (t.id === existing.id ? { ...t, ...fields } : t)),
      );
    } else {
      setTransitions((current) => [
        ...current,
        { id: `T-${Date.now()}`, from, to, ...fields },
      ]);
    }
    setTransitionEdit(null);
    toastBriefly(
      `${statusName(from)} → ${statusName(to)} 전환을 저장했습니다.`,
    );
  };

  const deleteTransition = (from: string, to: string) => {
    setTransitions((current) =>
      current.filter((t) => !(t.from === from && t.to === to)),
    );
    setTransitionEdit(null);
  ...
  const rows: GridRow[] = filtered.map((s) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: s.userVisible ? "노출" : "비노출",
          bg: s.userVisible ? "#eff6ff" : "#f4f4f5",
          fg: s.userVisible ? "#2563eb" : "#71717a",
        },
        {
          kind: "statusDot",
          text: s.active ? "사용" : "비활성",
          dot: s.active ? "#10b981" : "#a1a1aa",
          fg: s.active ? "#047857" : "#71717a",
        },
        {
          kind: "text",
          text: `${s.orderCount.toLocaleString()}건`,
          size: "12px",
          align: "right",
          numeric: true,
        },
    ...
    <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
      <header className={shared.header}>
        ...
        <div className={shared.quickFilters}>
          <button
            type="button"
            className={`${shared.qfBtn} ${view === "list" ? shared.quickActive : ""}`}
            onClick={() => setView("list")}
          >
            <span className={shared.qfLabel}>상태 목록</span>
          </button>
          <button
            type="button"
            className={`${shared.qfBtn} ${view === "transitions" ? shared.quickActive : ""}`}
            onClick={() => setView("transitions")}
          >
            <span className={shared.qfLabel}>상태 전환 설정</span>
          </button>
          <button
            type="button"
            className={`${shared.qfBtn} ${view === "preview" ? shared.quickActive : ""}`}
            onClick={() => setView("preview")}
          >
            <span className={shared.qfLabel}>
              Workflow 미리보기
                  ...
                  {QUICK_FILTERS.map((filter) => (
                    ...
                    >
                      ...
                      <span className={shared.qfCount}>
                        {
                          statuses.filter((s) =>
                            matchesQuickFilter(s, filter, warnings),
                          ).length
                        }
                      </span>
                    </button>
              ...
              <div className={shared.filterRow2}>
                ...
                <label className="globalFilterField">
                  ...
                  <select
                    ...
                    value={modeFilter}
                    onChange={(e) =>
                      setModeFilter(e.target.value as ChangeMode | "")
                    }
                  >
      ...
      {view === "transitions" && (
        <div className={styles.matrixSection}>
          <div className={styles.matrixHead}>
            <h2>전환 매트릭스</h2>
            <p>
              행(From)에서 열(To)로의 전환 가능 여부입니다. 셀을 클릭해 전환
              조건을 설정하세요.
            </p>
          </div>
          <div className={styles.matrixWrap}>
            <table className={styles.matrixTable}>
              <thead>
                <tr>
                  ...
                  {sorted.map((to) => (
                    <th key={to.id} className={styles.matrixColHead}>
                      {to.name}
                    </th>
                  ))}
                </tr>
                      ...
                      if (from.id === to.id)
                        return (
                          <td key={to.id} className={styles.matrixCell}>
                            <div className={styles.matrixCellDiag} />
                          </td>
                        );
                      const t = findTransition(from.id, to.id, transitions);
                      const cellClass = !t
                        ? styles.matrixCellBtn
                        : t.mode === "수동"
                          ? `${styles.matrixCellBtn} ${styles.matrixCellManual}`
                          : t.mode === "자동"
                            ? `${styles.matrixCellBtn} ${styles.matrixCellAuto}`
                            : `${styles.matrixCellBtn} ${styles.matrixCellBoth}`;
                      return (
                        <td key={to.id} className={styles.matrixCell}>
                          <button
                            type="button"
                            className={cellClass}
                            onClick={() =>
                              setTransitionEdit({ from: from.id, to: to.id })
                            }
                            title={
                              t
                                ? `${from.name} → ${to.name} · ${t.mode}`
                                : `${from.name} → ${to.name} 전환 설정`
                            }
                          >
                            {t ? "✓" : ""}
                          </button>
            ...
            <div className={styles.matrixLegend}>
              <span>
                <i
                  className={styles.matrixLegendDot}
      ...
      {view === "preview" && (
        <div className={styles.previewSection}>
          <div className={styles.previewHead}>
            <h2>주문 Workflow</h2>
            <p>
              취소·예외 상태를 제외한 정상 처리 흐름입니다. 상태를 클릭하면
              상세로 이동합니다.
            </p>
          </div>
          <div className={styles.flowWrap}>
            <div className={styles.flowRow}>
              {sorted
                .filter((s) => !s.isCancelled && !s.isException)
                .map((s, index, arr) => (
                  <FlowNodeWithArrow
                    key={s.id}
                    status={s}
              ...
              .map((s) => {
                ...
                return (
                  <div key={s.id} className={styles.branchRow}>
                    <span>
                      {inn.map((t) => statusName(t.from)).join(", ") ||
                        "진입 경로 없음"}
                    </span>
                    <span className={styles.branchArrow}>→</span>
                    <strong>{s.name}</strong>
                    {out.length === 0 && !s.isTerminal && (
                      <span
                        className={`${styles.branchTag} ${styles.branchTagWarn}`}
                      >
                        다음 상태 없음
                      </span>
                    )}
          ...
          <div className={styles.warningPanel}>
            <h3>설정 검증</h3>
            {warnings.length === 0 && (
              <div className={styles.warningOk}>
                ✓ 발견된 설정 문제가 없습니다.
              </div>
            )}
            {warnings.map((w) => (
              <div key={w.id} className={styles.warningItem}>
                ...
                <button
                  ...
                  onClick={() => openDetail(STATUS_MAP[w.statusId])}
                >
                  상태 보기
                </button>
              </div>
      ...
      {transitionEdit && (
        <TransitionEditDialog
          ...
          onSave={(fields) =>
            saveTransition(transitionEdit.from, transitionEdit.to, fields)
          }
          onDelete={
            findTransition(transitionEdit.from, transitionEdit.to, transitions)
              ? () => deleteTransition(transitionEdit.from, transitionEdit.to)
              : undefined
          }
        />
      )}
        ...
        >
          <div className={`${shared.dialogBox} ${styles.orderDialog}`}>
            ...
            <p className={shared.dialogBody}>
              목록에 표시되는 순서를 조정합니다. Workflow 전환 순서와는
              별개입니다.
            </p>
            <div className={styles.orderList}>
              {sorted.map((s) => (
                <div key={s.id} className={styles.orderRow}>
                  ...
                  <strong>{s.name}</strong>
                  <span>{s.stage}</span>
                  <button type="button" onClick={() => move(s, -1)}>
                    ↑
                  </button>
                  <button type="button" onClick={() => move(s, 1)}>
                    ↓
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogSummary}>
              ...
              <div className={shared.dialogSummaryRow}>
                <span>현재 주문수</span>
                <strong>{confirm.item.orderCount.toLocaleString()}건</strong>
              </div>
            </div>
            <div className={shared.dialogActions}>
              <button
...
function FlowNodeWithArrow({
  ...
  return (
    <>
      <button
        type="button"
        className={`${styles.flowNode} ${status.isSuccess ? styles.flowNodeTerminalSuccess : ""}`}
        onClick={onClick}
      >
        <span className={styles.flowNodeStage}>{status.stage}</span>
        <span className={styles.flowNodeName} style={{ color: tone.fg }}>
          {status.name}
        </span>
        <span className={styles.flowNodeCount}>
          {status.orderCount.toLocaleString()}건
        </span>
      </button>
