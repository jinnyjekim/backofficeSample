...
export function RefundReasonEditDialog({ initial, onClose, onSave }: Props) {
  ...
  const set = <K extends keyof RefundReason>(key: K, value: RefundReason[K]) =>
    ...
    >
      <div className={`${shared.dialogBox} ${styles.reasonDialog}`}>
        ...
        <label className={styles.formField}>
          ...
          <select
            value={draft.type}
            onChange={(e) => set('type', e.target.value as RefundType)}
          >
            {REFUND_TYPES.map((t) => (
        ...
        <label className={styles.toggleField}>
          ...
          <button
            type="button"
            className={`${styles.switch} ${draft.active ? styles.switchOn : ''}`}
        ...
        <label className={styles.toggleField}>
          <span>
            상세 사유 입력 필수
            <small>'기타' 등 자유 서술이 필요한 사유에 사용</small>
          </span>
          <button
            type="button"
            className={`${styles.switch} ${draft.requiresDetail ? styles.switchOn : ''}`}
            onClick={() => set('requiresDetail', !draft.requiresDetail)}
          >
            <i />
          </button>
        </label>
