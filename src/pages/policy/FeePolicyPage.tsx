...
export function FeePolicyPage() {
  ...
  const save = (item: FeePolicy) => {
    if (isNew) {
      ...
      const saved =
        previous && previous.rate !== item.rate && item.calcMethod === "정률"
          ? history(item, "수수료율 변경", `${previous.rate}%`, `${item.rate}%`)
          : previous &&
              previous.fixedAmount !== item.fixedAmount &&
              item.calcMethod === "정액"
            ? history(
                item,
                "정액 수수료 변경",
                `${previous.fixedAmount.toLocaleString()}원`,
          ...
          <div className={shared.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value as FeeType | "")}
              >
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={bearerFilter}
                onChange={(e) =>
                  setBearerFilter(e.target.value as FeeBearer | "")
                }
              >
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={methodFilter}
                onChange={(e) =>
                  setMethodFilter(e.target.value as CalcMethod | "")
                }
              >
