...
import { formatWon, INITIAL_EXCHANGES, matchesExchangeKeyword, type ExchangeItem } from './exchangeData';

const COLUMNS: GridColumn[] = [{ label: '교환번호' }, { label: '주문번호' }, { label: '신청고객' }, { label: '교환사유' }, { label: '상품 / 옵션 변경' }, { label: '교환금액', align: 'right' }, { label: '신청일시' }];
...
export function ExchangeRequestsPage() {
  ...
  const requests = useMemo(() => items.filter((item) => item.stage === '교환 요청'), [items]);
  const filtered = useMemo(() => requests.filter((item) => (reason === '전체' || item.reason.includes(reason)) && matchesExchangeKeyword(item, keyword)), [keyword, reason, requests]);
