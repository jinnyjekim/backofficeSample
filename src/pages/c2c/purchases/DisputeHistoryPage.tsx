...
export function DisputeHistoryPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><select className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>분쟁번호</option><option>구매번호</option><option>처리자</option></select><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="로그 / 분쟁 / 구매번호 또는 처리 사유"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={action} onChange={(event)=>setAction(event.target.value)}><option value="">전체 처리 유형</option>{[...new Set(DISPUTE_AUDIT_LOGS.map((item)=>item.action))].map((item)=><option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={actor} onChange={(event)=>setActor(event.target.value)}><option value="">전체 처리 주체</option><option value="관리자">관리자</option><option>SYSTEM</option></select><span>처리일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-19"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={()=>downloadCsv('c2c-dispute-history.csv',['처리일','로그 ID','분쟁번호','구매번호','처리','변경 전','변경 후','처리자','증빙 변경','사유'],filtered.map((item)=>[item.occurredAt,item.id,item.disputeId,item.purchaseId,item.action,item.before,item.after,item.actor,item.evidenceChange,item.reason]))}>다운로드</button></ResultBar><DataGrid columns={[{label:'처리일 / 로그 ID'},{label:'분쟁 / 구매번호'},{label:'구매자 / 판매자'},{label:'처리'},{label:'변경 전'},{label:'변경 후'},{label:'처리자 / 증빙'},{label:'처리 사유'},{label:'관리'}]} rows={rows} gridTemplate="160px 145px 130px 125px 85px 85px 145px minmax(220px,1.4fr) 55px" minWidth="1100px" empty={!filtered.length} emptyText="조건에 맞는 분쟁 처리 이력이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0건'}/></GridArea>
