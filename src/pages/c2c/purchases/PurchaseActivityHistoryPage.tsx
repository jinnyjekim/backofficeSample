...
import {
  ...
  buyerById,
  purchaseById,
  type PurchaseActivityLog,
} from "./purchaseActivityData";

const CATEGORY_META: Record<
  PurchaseActivityLog["category"],
  { bg: string; fg: string }
> = {
  구매: { bg: "#eff6ff", fg: "#1d4ed8" },
  결제: { bg: "#ecfdf5", fg: "#047857" },
  배송: { bg: "#f5f3ff", fg: "#6d28d9" },
  취소: { bg: "#fff7ed", fg: "#c2410c" },
  분쟁: { bg: "#fef2f2", fg: "#dc2626" },
  환불: { bg: "#ecfeff", fg: "#0e7490" },
};
...
export function PurchaseActivityHistoryPage() {
        ...
        (item) =>
          ...
          (!actor ||
            (actor === "SYSTEM"
              ...
              : item.actor !== "SYSTEM")) &&
          (!search ||
            `${item.id} ${item.action} ${item.target} ${item.reason} ${item.actor} ${buyerById(item.buyerId)?.nickname ?? ""}`
              .toLowerCase()
  ...
  const selected =
    PURCHASE_ACTIVITY_LOGS.find((item) => item.id === selectedId) ?? null;
  const buyer = selected ? buyerById(selected.buyerId) : null;
  const purchase = selected ? purchaseById(selected.target) : null;
  const rows: GridRow[] = filtered.map((item) => {
    const itemBuyer = buyerById(item.buyerId);
    return {
      id: item.id,
      ...
      cells: [
        ...
        {
          kind: "avatarText",
          title: itemBuyer?.nickname ?? item.buyerId,
    ...
    <div className={shared.page}>
        ...
        <FilterBox>
          ...
          >
            <input
              ...
              placeholder="로그 ID / 구매번호 / 구매자 / 처리자 / 사유"
            />
      ...
      <GridArea>
        ...
        <DataGrid
          ...
          rows={rows}
          gridTemplate="128px 58px minmax(155px,1fr) 198px 68px 88px 83px minmax(160px,1.2fr) 55px"
          minWidth="1060px"
          empty={!filtered.length}
          emptyText="조건에 맞는 구매 활동 이력이 없습니다."
          showPagination
          pages={pages}
          rangeLabel={
            filtered.length
      ...
      {selected && buyer && (
        <DetailDrawer
          ...
          fields={[
            ...
            { label: "구매자 ID", value: selected.buyerId },
            { label: "구매번호", value: selected.target },
            { label: "거래번호", value: purchase?.tradeId ?? "-" },
        ...
        >
          ...
          <div className={drawer.sectionTitleLoose}>감사 흐름</div>
          <div className={base.timeline}>
            <div className={base.timelineItem}>
              <strong>요청 또는 이벤트 발생</strong>
              <p>{selected.reason}</p>
              <time>{selected.occurredAt}</time>
            </div>
            <div className={base.timelineItem}>
              <strong>구매 상태 반영</strong>
              <p>
