...
export function SanctionProcessingPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{QUICKS.map((item)=><button type="button" key={item} className={`${shared.qfBtn} ${quick===item?base.quickActive:''}`} onClick={()=>updateParams(typeFilter,item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{cases.filter((entry)=>matchesQuick(entry,item)).length}</span></button>)}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><select className={shared.selectSm}><option>통합 검색</option><option>제재번호</option><option>회원</option><option>근거번호</option><option>제재 사유</option></select><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder={memberFilter?`${memberFilter} 회원의 제재만 표시 중`:'제재번호 / 회원 / 위반 사유 / 신고·분쟁·탐지 번호'}/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={typeFilter} onChange={(event)=>updateParams(event.target.value as SanctionType|'',quick,'')}><option value="">전체 제재 유형</option>{TYPES.map((item)=><option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={termFilter} onChange={(event)=>updateParams(typeFilter,quick,event.target.value)}><option value="">전체 적용 기간</option><option value="temporary">기간 제재</option><option value="permanent">영구 정지</option></select><select className={shared.selectSm} value={severity} onChange={(event)=>setSeverity(event.target.value as SanctionSeverity|'')}><option value="">전체 위험도</option><option>긴급</option><option>높음</option><option>보통</option><option>낮음</option></select><select className={shared.selectSm} value={assignee} onChange={(event)=>setAssignee(event.target.value)}><option value="">전체 배정 상태</option><option>미배정</option><option value="배정">담당자 배정</option></select><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"/><DataGrid columns={[{label:'제재번호 / 근거'},{label:'대상 회원'},{label:'제재 유형'},{label:'위반 / 제재 사유'},{label:'기간 / 누적'},{label:'적용 기간'},{label:'처리 상태'},{label:'담당자 / 승인'},{label:'관리'}]} rows={rows} gridTemplate="160px 155px 112px minmax(220px,1.4fr) 92px 145px 82px 125px 55px" minWidth="1160px" empty={!filtered.length} emptyText="조건에 맞는 제재 건이 없습니다." emptySubtext="제재 유형이나 처리 상태 필터를 변경해 주세요." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0건'}/></GridArea>
...
export function SanctionPolicyPage() {
  ...
  const rows:GridRow[]=filtered.map((item)=>({id:item.id,onClick:()=>setSelectedId(item.id),cells:[{kind:'stack',title:item.id,subtitle:item.updatedAt},{kind:'stack',title:item.name,subtitle:item.violation},{kind:'text',text:item.firstAction},{kind:'text',text:item.secondAction},{kind:'text',text:item.repeatAction,weight:600},{kind:'stack',title:item.approval,subtitle:`이의제기 ${item.appealDays}일`},{kind:'stack',title:item.owner,subtitle:item.automation},{kind:'badge',text:item.status,...policyMeta[item.status]},{kind:'link',text:'상세'}]}));
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><select className={shared.selectSm}><option>통합 검색</option><option>정책 ID</option><option>정책명</option><option>위반 기준</option><option>담당 조직</option></select><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="정책 ID / 정책명 / 위반 기준 / 담당 조직"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={status} onChange={(event)=>setStatus(event.target.value)}><option value="">전체 정책 상태</option><option>사용</option><option>검토 필요</option><option>중지</option></select><span>기준일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="개"/><DataGrid columns={[{label:'정책 ID / 수정일'},{label:'정책 / 위반 기준'},{label:'1차 조치'},{label:'2차 조치'},{label:'반복 위반'},{label:'승인 / 이의제기'},{label:'담당 / 자동화'},{label:'상태'},{label:'관리'}]} rows={rows} gridTemplate="125px minmax(220px,1.35fr) 120px 125px 135px 150px 155px 75px 55px" minWidth="1130px" empty={!filtered.length} emptyText="조건에 맞는 제재 정책이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></GridArea>
...
export function SanctionHistoryPage() {
  ...
  const rows:GridRow[]=filtered.map((item)=>({id:item.id,onClick:()=>setSelectedId(item.id),cells:[{kind:'stack',title:item.occurredAt,subtitle:item.id},{kind:'stack',title:item.sanctionId,subtitle:item.source},{kind:'avatarText',title:item.nickname,subtitle:item.memberId,avatarChar:item.nickname[0],avatarBg:'#f4f4f5',avatarFg:'#52525b'},{kind:'pillText',text:item.type,...SANCTION_TYPE_META[item.type]},{kind:'text',text:item.action,weight:600},{kind:'text',text:item.before,color:'#71717a'},{kind:'text',text:item.after,weight:600},{kind:'stack',title:item.actor,subtitle:item.reason},{kind:'link',text:'상세'}]}));
  return <section className={shared.page}>
    <PageHeading title="제재 처리 이력" subtitle="제재 검토·승인·적용·해제의 변경 전후 값과 처리 근거를 수정 불가능한 감사 로그로 조회합니다."/>
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><select className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>제재번호</option><option>회원</option><option>근거번호</option></select><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="로그 / 제재번호 / 회원 / 신고·분쟁·탐지 번호"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={type} onChange={(event)=>setType(event.target.value as SanctionType|'')}><option value="">전체 제재 유형</option>{TYPES.map((item)=><option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={action} onChange={(event)=>setAction(event.target.value)}><option value="">전체 처리 유형</option>{[...new Set(SANCTION_AUDIT_LOGS.map((item)=>item.action))].map((item)=><option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={actor} onChange={(event)=>setActor(event.target.value)}><option value="">전체 처리 주체</option><option value="관리자">관리자</option><option>SYSTEM</option></select><span>처리일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-24"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={()=>downloadCsv('c2c-sanction-history.csv',['처리일','로그 ID','제재번호','회원','유형','처리','변경 전','변경 후','처리자','근거','사유'],filtered.map((item)=>[item.occurredAt,item.id,item.sanctionId,item.memberId,item.type,item.action,item.before,item.after,item.actor,item.source,item.reason]))}>다운로드</button></ResultBar><DataGrid columns={[{label:'처리일 / 로그 ID'},{label:'제재번호 / 근거'},{label:'대상 회원'},{label:'제재 유형'},{label:'처리'},{label:'변경 전'},{label:'변경 후'},{label:'처리자 / 사유'},{label:'관리'}]} rows={rows} gridTemplate="160px 150px 145px 112px 115px 85px 85px minmax(220px,1.35fr) 55px" minWidth="1120px" empty={!filtered.length} emptyText="조건에 맞는 제재 처리 이력이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0건'}/></GridArea>
