...
export function TradeRiskMonitoringPage() {
  ...
  return <section className={shared.page}>
    ...
    <GridArea>
      ...
      <DataGrid columns={[{ label: '위험 건 / 탐지일' }, { label: '탐지 유형' }, { label: '대상' }, { label: '탐지 요약' }, { label: '위험도' }, { label: '점수', align: 'right' }, { label: '검토 상태' }, { label: '담당자' }, { label: '관리' }]} rows={rows} gridTemplate="145px 95px 130px minmax(230px,1.5fr) 72px 65px 80px 75px 55px" minWidth="1080px" empty={!filtered.length} emptyText="조건에 맞는 위험 건이 없습니다." emptySubtext="탐지 유형이나 위험도 필터를 변경해 주세요." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} />
    </GridArea>
