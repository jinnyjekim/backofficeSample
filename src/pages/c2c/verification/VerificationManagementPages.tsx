...
export function VerificationReviewPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{QUICKS.map((item)=><button type="button" key={item} className={`${shared.qfBtn} ${quick===item?base.quickActive:''}`} onClick={()=>updateParams(purpose,item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{cases.filter((entry)=>matchesQuick(entry,item)).length}</span></button>)}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>인증번호</option><option>회원</option><option>사업자 거래번호</option><option>실패 코드</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder={memberFilter?`${memberFilter} 회원의 인증만 표시 중`:'인증번호 / 회원 / 인증 사업자 거래번호 / 실패 사유'}/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>인증 목적</span><select aria-label="인증 목적" className={shared.selectSm} value={purpose} onChange={(event)=>updateParams(event.target.value as VerificationPurpose|'',quick)}><option value="">전체 인증 목적</option>{PURPOSES.map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>위험도</span><select aria-label="위험도" className={shared.selectSm} value={risk} onChange={(event)=>setRisk(event.target.value as VerificationRisk|'')}><option value="">전체 위험도</option><option>긴급</option><option>높음</option><option>주의</option><option>정상</option></select></label><label className="globalFilterField"><span>배정 상태</span><select aria-label="배정 상태" className={shared.selectSm} value={assignee} onChange={(event)=>setAssignee(event.target.value)}><option value="">전체 배정 상태</option><option>미배정</option><option value="배정">담당자 배정</option></select></label><span>접수일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-20"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
...
export function VerificationPolicyPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>정책 ID</option><option>정책명</option><option>인증 사업자</option><option>담당 조직</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="정책 ID / 정책명 / 인증 사업자 / 담당 조직"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>인증 목적</span><select aria-label="인증 목적" className={shared.selectSm} value={purpose} onChange={(event)=>setPurpose(event.target.value as VerificationPurpose|'')}><option value="">전체 인증 목적</option>{PURPOSES.map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>정책 상태</span><select aria-label="정책 상태" className={shared.selectSm} value={status} onChange={(event)=>setStatus(event.target.value)}><option value="">전체 정책 상태</option><option>사용</option><option>검토 필요</option><option>중지</option></select></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
...
export function VerificationHistoryPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>인증번호</option><option>회원</option><option>사업자 거래번호</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="로그 / 인증번호 / 회원 / 인증 사업자 거래번호"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>인증 목적</span><select aria-label="인증 목적" className={shared.selectSm} value={purpose} onChange={(event)=>setPurpose(event.target.value as VerificationPurpose|'')}><option value="">전체 인증 목적</option>{PURPOSES.map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>처리 결과</span><select aria-label="처리 결과" className={shared.selectSm} value={result} onChange={(event)=>setResult(event.target.value)}><option value="">전체 처리 결과</option><option>성공</option><option>실패</option><option>보류</option></select></label><label className="globalFilterField"><span>처리 주체</span><select aria-label="처리 주체" className={shared.selectSm} value={actor} onChange={(event)=>setActor(event.target.value)}><option value="">전체 처리 주체</option><option value="관리자">관리자</option><option>SYSTEM</option></select></label><span>처리일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-26"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
