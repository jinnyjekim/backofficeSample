...
export function VerificationPolicyPage() {
  ...
  return <section className={shared.page}>
    ...
    <GridArea><div className={styles.queueCard}><div className={styles.queueHeader}><div><strong>인증 정책</strong><span>전체 {filtered.length}개</span></div><span>정렬 · 최근 수정순</span></div><DataGrid columns={[{label:'정책 ID / 수정일'},{label:'정책 / 인증 목적'},{label:'필수 단계 / 사업자'},{label:'유효기간'},{label:'재시도'},{label:'수동 심사 기준'},{label:'보관 / 담당'},{label:'상태'}]} rows={rows} gridTemplate="125px 155px minmax(220px,1.3fr) 90px 95px minmax(175px,1fr) 155px 75px" minWidth="1025px" empty={!filtered.length} emptyText="조건에 맞는 인증 정책이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></div></GridArea>
