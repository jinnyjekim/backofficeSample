import { CommonButton, CommonSelect, ExcelDownloadButton, showToast } from '../../../components/common';
    ...
    <header className={shared.header}>
      <div className={shared.headerTop}>
        <div>
          ...
          <div className={shared.subtitle}>기본·판매자·고액 거래·재인증·계정 복구 인증을 하나의 심사 큐에서 자동 검증과 수동 판단으로 처리합니다.</div>
        </div>
        <button type="button" className={styles.reviewPrimaryBtn} onClick={autoAssign}>미배정 자동 배정</button>
      </div>
      ...
      <div className={shared.filterBox}>
        <form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}>
          <input aria-label="인증 심사 검색" className={shared.searchInput} value={keyword} onChange={(event) => { setKeyword(event.target.value); setSearch(event.target.value.trim()); }} placeholder={memberFilter ? `${memberFilter} 회원의 인증만 표시 중` : '인증번호 / 회원 / 사업자 / 실패 사유'} />
          <button type="submit" className={shared.searchBtn}>검색</button>
        </form>
        <div className={shared.filterRow2}>
          <label className="globalFilterField"><span>인증 목적</span><CommonSelect aria-label="인증 목적" size="md" value={purpose} placeholder="전체 인증 목적" options={[{ label: '전체 인증 목적', value: '' }, ...PURPOSES.map((value) => ({ label: value, value }))]} onChange={(value) => updateParams(value as VerificationPurpose | '', quick)} /></label>
          <label className="globalFilterField"><span>위험도</span><CommonSelect aria-label="위험도" size="md" value={risk} placeholder="전체 위험도" options={['', '긴급', '높음', '주의', '정상'].map((value) => ({ label: value || '전체 위험도', value }))} onChange={(value) => setRisk(value as VerificationRisk | '')} /></label>
          <label className={shared.dateFilterField}><span>접수일</span><span className={shared.dateRange}><DatePicker key={submittedRange[0] ?? 'submitted-from-empty'} aria-label="접수 시작 날짜" value={submittedRange[0] ?? undefined} placeholder="시작 날짜" onChange={(event) => setSubmittedRange([event.target.value || null, submittedRange[1]])} /><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker key={submittedRange[1] ?? 'submitted-to-empty'} aria-label="접수 종료 날짜" value={submittedRange[1] ?? undefined} placeholder="종료 날짜" onChange={(event) => setSubmittedRange([submittedRange[0], event.target.value || null])} /></span></label>
      ...
      <div className={shared.resultRow}>
        <span className={shared.resultLabel}>총 {filtered.length}건 · SLA 초과 {slaExceeded}건 · 고위험 {highRiskCount}건</span>
        <div className={shared.resultActions}><ExcelDownloadButton type="button" data-grid-download onClick={downloadCases} /></div>
      </div>
    </header>
    ...
    <header className={shared.header}>
      <div className={shared.headerTop}>
        <div>
          ...
          <div className={shared.subtitle}>인증 목적별 필수 단계, 인증 사업자, 유효기간, 재시도 제한, 수동 심사와 개인정보 보관 기준을 관리합니다.</div>
        </div>
        <button type="button" className={styles.reviewPrimaryBtn} onClick={createDraft}>정책 초안 등록</button>
      </div>
      ...
      <div className={shared.filterBox}>
        <form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}>
          <input aria-label="인증 정책 검색" className={shared.searchInput} value={keyword} onChange={(event) => { setKeyword(event.target.value); setSearch(event.target.value.trim()); }} placeholder="정책 ID / 정책명 / 사업자 / 담당 조직" />
          <button type="submit" className={shared.searchBtn}>검색</button>
        </form>
        <div className={shared.filterRow2}>
          <label className="globalFilterField"><span>인증 목적</span><CommonSelect aria-label="인증 목적" size="md" value={purpose} placeholder="전체 인증 목적" options={[{ label: '전체 인증 목적', value: '' }, ...PURPOSES.map((value) => ({ label: value, value }))]} onChange={(value) => setPurpose(value as VerificationPurpose | '')} /></label>
          <label className={shared.dateFilterField}><span>수정일</span><span className={shared.dateRange}><DatePicker key={updatedRange[0] ?? 'updated-from-empty'} aria-label="수정 시작 날짜" value={updatedRange[0] ?? undefined} placeholder="시작 날짜" onChange={(event) => setUpdatedRange([event.target.value || null, updatedRange[1]])} /><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker key={updatedRange[1] ?? 'updated-to-empty'} aria-label="수정 종료 날짜" value={updatedRange[1] ?? undefined} placeholder="종료 날짜" onChange={(event) => setUpdatedRange([updatedRange[0], event.target.value || null])} /></span></label>
          <span className={shared.rowSpacer} />
          <button type="button" className={shared.resetBtn} onClick={reset}>초기화</button>
        </div>
      </div>
      ...
      <div className={shared.resultRow}>
        <span className={shared.resultLabel}>총 {filtered.length}개 · 검토 필요 {reviewCount}건</span>
        <div className={shared.resultActions}><ExcelDownloadButton type="button" data-grid-download onClick={downloadPolicies} /></div>
      </div>
    </header>

    <div className={shared.gridWrap}>
      <DataGrid columns={[{label:'정책 ID / 수정일'},{label:'정책 / 인증 목적'},{label:'필수 단계 / 사업자'},{label:'유효기간'},{label:'재시도'},{label:'수동 심사 기준'},{label:'보관 / 담당'},{label:'상태'}]} rows={rows} gridTemplate="76px 124px minmax(220px,1.3fr) 60px 72px minmax(175px,1fr) 155px 75px" minWidth="1000px" empty={!filtered.length} emptyText="조건에 맞는 인증 정책이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/>
    </div>
    ...
    <header className={shared.header}>
      <div className={shared.headerTop}>
        <div>
          ...
          <div className={shared.subtitle}>본인 인증 접수·자동 검증·수동 심사·보완·승인·실패의 상태 변경과 근거를 마스킹된 감사 로그로 조회합니다.</div>
        </div>
      </div>
      ...
      <div className={shared.filterBox}>
        <form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}>
          <input aria-label="인증 처리 이력 검색" className={shared.searchInput} value={keyword} onChange={(event) => { setKeyword(event.target.value); setSearch(event.target.value.trim()); }} placeholder="로그 ID / 인증번호 / 회원 / 사업자 거래번호" />
          <button type="submit" className={shared.searchBtn}>검색</button>
        </form>
        <div className={shared.filterRow2}>
          <label className="globalFilterField"><span>인증 목적</span><CommonSelect aria-label="인증 목적" size="md" value={purpose} placeholder="전체 인증 목적" options={[{ label: '전체 인증 목적', value: '' }, ...PURPOSES.map((value) => ({ label: value, value }))]} onChange={(value) => setPurpose(value as VerificationPurpose | '')} /></label>
          <label className="globalFilterField"><span>처리 주체</span><CommonSelect aria-label="처리 주체" size="md" value={actor} placeholder="전체 처리 주체" options={[{ label: '전체 처리 주체', value: '' }, { label: '관리자', value: '관리자' }, { label: 'SYSTEM', value: 'SYSTEM' }]} onChange={(value) => setActor(String(value))} /></label>
          <label className={shared.dateFilterField}><span>처리일</span><span className={shared.dateRange}><DatePicker key={occurredRange[0] ?? 'occurred-from-empty'} aria-label="처리 시작 날짜" value={occurredRange[0] ?? undefined} placeholder="시작 날짜" onChange={(event) => setOccurredRange([event.target.value || null, occurredRange[1]])} /><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker key={occurredRange[1] ?? 'occurred-to-empty'} aria-label="처리 종료 날짜" value={occurredRange[1] ?? undefined} placeholder="종료 날짜" onChange={(event) => setOccurredRange([occurredRange[0], event.target.value || null])} /></span></label>
          <span className={shared.rowSpacer} />
          <button type="button" className={shared.resetBtn} onClick={reset}>초기화</button>
        </div>
      </div>
      ...
      <div className={shared.resultRow}>
        <span className={shared.resultLabel}>총 {filtered.length}건 · 실패 이력 {failureCount}건</span>
        <div className={shared.resultActions}><ExcelDownloadButton type="button" data-grid-download onClick={downloadHistory} /></div>
      </div>
    </header>

    <div className={shared.gridWrap}>
      <DataGrid columns={[{label:'처리일 / 로그 ID'},{label:'인증번호 / 사업자 거래'},{label:'대상 회원'},{label:'인증 목적'},{label:'처리'},{label:'변경 전'},{label:'변경 후'},{label:'결과'},{label:'처리자 / 사유'}]} rows={rows} gridTemplate="128px 124px 185px 78px 82px 68px 68px 54px minmax(210px,1.25fr)" minWidth="1040px" empty={!filtered.length} emptyText="조건에 맞는 인증 처리 이력이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0건'}/>
    </div>
