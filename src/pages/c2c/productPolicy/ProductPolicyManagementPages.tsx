...
export function ProductPolicyPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><div className={shared.filterHeadRow}><div className={shared.quickFilters}>{(['전체',...LEVELS] as const).map((item) => {
    ...
    })}</div><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>정책 ID</option><option>정책명</option><option>판단 기준</option><option>법적 근거</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="정책 ID / 정책명 / 판단 기준 / 예시 / 법적 근거"/><button className={shared.searchBtn}>조회</button></form></div><FilterBox><div className={shared.filterRow2}><label className="globalFilterField"><span>정책 분류</span><select aria-label="정책 분류" className={shared.selectSm} value={category} onChange={(event)=>setCategory(event.target.value)}><option value="">전체 정책 분류</option>{[...new Set(policies.map((item)=>item.category))].map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>정책 상태</span><select aria-label="정책 상태" className={shared.selectSm} value={status} onChange={(event)=>setStatus(event.target.value as ProductPolicyStatus|'')}><option value="">전체 정책 상태</option><option>사용</option><option>검토 필요</option><option>중지</option></select></label><label className="globalFilterField"><span>기준일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="개"><ExcelDownloadButton type="button" data-grid-download onClick={()=>downloadCsv('c2c-product-policies.csv',['정책 ID','정책명','분류','등급','판단 기준','기본 조치','증빙','법적 근거','상태','버전'],filtered.map((item)=>[item.id,item.name,item.category,item.level,item.criteria,item.defaultAction,item.evidence,item.legalBasis,item.status,item.version]))}/></ResultBar><DataGrid columns={[{label:'정책 ID / 버전'},{label:'정책 / 분류'},{label:'등급'},{label:'판단 기준 / 예시'},{label:'기본 조치'},{label:'필요 증빙'},{label:'담당 / 지역'},{label:'상태'},{label:'관리'}]} rows={rows} gridTemplate="106px 140px 85px minmax(245px,1.4fr) 165px 188px 100px 75px 55px" minWidth="1190px" empty={!filtered.length} emptyText="조건에 맞는 상품 정책이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></GridArea>
