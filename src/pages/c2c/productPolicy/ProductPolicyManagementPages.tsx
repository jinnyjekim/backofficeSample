...
export function ProductPolicyPage() {
  ...
  const toggle=()=>{if(!selected)return;const next:ProductPolicyStatus=selected.status==='중지'?'사용':'중지';setPolicies((items)=>items.map((item)=>item.id===selected.id?{...item,status:next,updatedAt:'2026-08-27'}:item));setToast(`${selected.id} 정책을 ${next} 상태로 변경했습니다.`);window.setTimeout(()=>setToast(''),2200);};
...
export function DetectionRulePage() {
  ...
  const toggle=()=>{if(!selected)return;const next:ProductPolicyStatus=selected.status==='중지'?'사용':'중지';setRules((items)=>items.map((item)=>item.id===selected.id?{...item,status:next,updatedAt:'2026-08-27'}:item));setToast(`${selected.id} 규칙을 ${next} 상태로 변경했습니다.`);window.setTimeout(()=>setToast(''),2200);};
