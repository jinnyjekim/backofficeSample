...
export function SellerProductsPage() {
  ...
  const filtered = useMemo(
    () =>
      products.filter((product) => {
        ...
        return (
          !search ||
          `${product.id} ${product.title} ${product.category} ${seller?.nickname ?? ""}`
            .toLowerCase()
  ...
  const updateStatus = (
    ...
    exposure = status === "숨김" ? ("비노출" as const) : ("노출" as const),
  ) => {
    if (!selected) return;
    setProducts((items) =>
      items.map((product) =>
        product.id === selected.id
          ? { ...product, status, exposure, updatedAt: "2026-08-26 15:40" }
          : product,
      ),
    );
    notify(`${selected.title} 상태를 ${status}(으)로 변경했습니다.`);
  };
  const rows: GridRow[] = filtered.map((product) => {
    ...
    return {
      ...
      cells: [
        {
          kind: "thumbTitle",
          thumb: product.status === "숨김" ? "#fecaca" : "#e0e7ff",
          title: product.title,
          id: product.id,
          onClick: () => setSelectedId(product.id),
        },
        ...
        {
          kind: "text",
          text: product.wishes.toLocaleString(),
          align: "right",
          numeric: true,
        },
        {
          kind: "text",
          text: String(product.reportCount),
          align: "right",
          numeric: true,
          color: product.reportCount ? "#dc2626" : "#71717a",
        },
        { kind: "link", text: "상세" },
      ],
    };
  ...
  return (
    <div className={shared.page}>
      <PageHeading
        title="사용자 상품 목록"
        subtitle="C2C 판매자가 등록한 전체 상품을 판매 상태, 노출 상태와 신고 현황으로 조회합니다."
      />
      <Metrics
        items={[
          {
          ...
          {
            label: "현재 판매중",
            value: `${products.filter((p) => p.status === "판매중").length}개`,
            note: "전체의 40.0%",
            dot: "#10b981",
          },
          {
            label: "검수 필요",
            value: `${products.filter((p) => p.status === "검수중" || p.reportCount > 0).length}개`,
            note: "검수중 또는 신고 접수",
            tone: "down",
            dot: "#f59e0b",
          },
          {
            label: "총 상품 가치",
            value: formatWon(products.reduce((sum, p) => sum + p.price, 0)),
            note: "등록가 합계",
            dot: "#8b5cf6",
          },
        ]}
      ...
      {selected && selectedSeller && (
        <DetailDrawer
          eyebrow={`판매 상품 상세 · ${selected.id}`}
          title={selected.title}
          status={selected.status}
          statusMeta={PRODUCT_STATUS_META[selected.status]}
          subtitle={`${selectedSeller.nickname} · ${selected.category}`}
          ...
          actions={
            <>
              <button
                ...
                onClick={() =>
                  updateStatus(
                    selected.status === "판매중" ? "예약중" : "판매중",
                  )
                }
              >
                {selected.status === "판매중" ? "예약 처리" : "판매 재개"}
              </button>
          ...
          stats={[
            { label: "조회", value: selected.views.toLocaleString() },
            { label: "관심", value: selected.wishes.toLocaleString() },
            { label: "채팅", value: selected.chats.toLocaleString() },
          ]}
          fields={[
            {
              label: "판매자",
            ...
            { label: "신고", value: `${selected.reportCount}건` },
          ]}
        >
          <div className={drawer.sectionTitleLoose}>검수 참고</div>
          <div className={drawer.bodyText}>
            {selected.reportCount
              ? `신고 ${selected.reportCount}건이 접수되었습니다. 상품 설명, 이미지와 판매자의 소명 자료를 확인하세요.`
              : "현재 접수된 신고가 없습니다. 상품 정보와 카테고리 적합성을 확인할 수 있습니다."}
          </div>
        </DetailDrawer>
      )}
    </div>
