...
export function SellerProductsPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{QUICK.map((item)=><button type="button" key={item} className={`${shared.qfBtn} ${quick===item?styles.quickActive:''}`} onClick={()=>setQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{products.filter((product)=>item==='전체'||(item==='신고 상품'?product.reportCount>0:product.status===item)).length}</span></button>)}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><select className={shared.selectSm}><option>통합 검색</option><option>상품 ID</option><option>상품명</option><option>판매자</option></select><input className={shared.searchInput} value={keyword} onChange={(e)=>setKeyword(e.target.value)} placeholder="상품 ID / 상품명 / 판매자 닉네임"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={category} onChange={(e)=>setCategory(e.target.value)}><option value="">전체 카테고리</option>{categories.map((item)=><option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={condition} onChange={(e)=>setCondition(e.target.value)}><option value="">전체 상품 상태</option><option>새상품</option><option>사용감 없음</option><option>사용감 적음</option><option>사용감 많음</option></select><input type="number" className={shared.selectSm} placeholder="최소 가격"/><span>~</span><input type="number" className={shared.selectSm} placeholder="최대 가격"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={()=>{setKeyword('');setSearch('');setCategory('');setCondition('');}}>초기화</button></div></FilterBox></ControlArea>
