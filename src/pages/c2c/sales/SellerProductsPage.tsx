    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><input className={shared.searchInput} value={keyword} onChange={(e)=>setKeyword(e.target.value)} placeholder="상품 ID / 상품명 / 판매자 닉네임"/><button className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>{QUICK.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{products.filter((product) => item === '전체' || (item === '신고 상품' ? product.reportCount > 0 : product.status === item)).length}</span>
        </CommonButton>
      );
    })}</div></form><div className={shared.filterRow2}><label className="globalFilterField"><span>카테고리</span><select aria-label="카테고리" className={shared.selectSm} value={category} onChange={(e)=>setCategory(e.target.value)}><option value="">전체 카테고리</option>{categories.map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>상품 상태</span><select aria-label="상품 상태" className={shared.selectSm} value={condition} onChange={(e)=>setCondition(e.target.value)}><option value="">전체 상품 상태</option><option>새상품</option><option>사용감 없음</option><option>사용감 적음</option><option>사용감 많음</option></select></label><label className={shared.dateFilterField}><span>가격 범위</span><div className={shared.dateRange}><input type="number" className={shared.selectSm} placeholder="최소 가격"/><span className={shared.dateSeparator}>~</span><input type="number" className={shared.selectSm} placeholder="최대 가격"/></div></label><span className={shared.rowSpacer}/><button type="button" className="detailFilterBtn">상세 필터</button><button type="button" className={shared.resetBtn} onClick={()=>{setKeyword('');setSearch('');setCategory('');setCondition('');}}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="개"><select className={shared.pageSizeSelect}><option>20개씩</option><option>50개씩</option></select></ResultBar><DataGrid columns={[{label:'상품'},{label:'판매자'},{label:'카테고리'},{label:'판매가',align:'right'},{label:'판매 상태'},{label:'상품 상태'},{label:'조회',align:'right'},{label:'관심',align:'right'},{label:'신고',align:'right'},{label:'관리'}]} rows={rows} gridTemplate="minmax(220px,1.6fr) 84px 102px 90px 80px 82px 55px 44px 48px 55px" minWidth="1065px" empty={!filtered.length} emptyText="조건에 맞는 판매 상품이 없습니다." showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></GridArea>
