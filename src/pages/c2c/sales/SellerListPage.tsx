...
export function SellerListPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{QUICK.map((item) => <button key={item} type="button" className={`${shared.qfBtn} ${quick === item ? styles.quickActive : ''}`} onClick={() => setQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{sellers.filter((seller) => item === '전체' || (item === '신고 있음' ? seller.reportCount > 0 : seller.status === item)).length}</span></button>)}</div>
      <FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>통합 검색</option><option>판매자 ID</option><option>닉네임</option><option>이름</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 닉네임 / 이름 / 연락처"/><button type="submit" className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={grade} onChange={(event) => setGrade(event.target.value)}><option value="">전체 등급</option><option>일반</option><option>우수</option><option>파워</option></select><select className={shared.selectSm} value={verification} onChange={(event) => setVerification(event.target.value)}><option value="">전체 인증</option><option>완료</option><option>미완료</option><option>재인증 필요</option></select><select className={shared.selectSm} value={region} onChange={(event) => setRegion(event.target.value)}><option value="">전체 지역</option>{[...new Set(sellers.map((seller) => seller.region))].map((item) => <option key={item}>{item}</option>)}</select><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
