...
export function SellerListPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea>
      <FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 닉네임 / 이름 / 연락처"/><button type="submit" className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>{QUICK.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{sellers.filter((seller) => item === '전체' || (item === '신고 있음' ? seller.reportCount > 0 : seller.status === item)).length}</span>
        </CommonButton>
      );
    })}</div></form><div className={shared.filterRow2}><label className="globalFilterField"><span>등급</span><select aria-label="등급" className={shared.selectSm} value={grade} onChange={(event) => setGrade(event.target.value)}><option value="">전체 등급</option><option>일반</option><option>우수</option><option>파워</option></select></label><label className="globalFilterField"><span>인증</span><select aria-label="인증" className={shared.selectSm} value={verification} onChange={(event) => setVerification(event.target.value)}><option value="">전체 인증</option><option>완료</option><option>미완료</option><option>재인증 필요</option></select></label><label className="globalFilterField"><span>지역</span><select aria-label="지역" className={shared.selectSm} value={region} onChange={(event) => setRegion(event.target.value)}><option value="">전체 지역</option>{[...new Set(sellers.map((seller) => seller.region))].map((item) => <option key={item}>{item}</option>)}</select></label><span className={shared.rowSpacer}/><button type="button" className="detailFilterBtn">상세 필터</button><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length}><PageSizeSelect className={shared.pageSizeSelect}><option>20개씩</option><option>50개씩</option></PageSizeSelect></ResultBar><DataGrid columns={[{ label:'판매자'},{label:'등급'},{label:'판매 상태'},{label:'본인 인증'},{label:'상품(판매중/완료)',align:'right'},{label:'거래',align:'right'},{label:'누적 판매액',align:'right'},{label:'취소율',align:'right'},{label:'신고',align:'right'},{label:'관리'}]} rows={rows} gridTemplate="minmax(180px,1.5fr) 52px 82px 76px 115px 44px 94px 56px 52px 55px" minWidth="980px" empty={!filtered.length} emptyText="조건에 맞는 판매자가 없습니다." showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0명'}/></GridArea>
