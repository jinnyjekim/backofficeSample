...
export function SalesStatusPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>통합 검색</option><option>판매자</option><option>변경 사유</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 닉네임 / 상태 변경 사유"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={status} onChange={(event) => setStatus(event.target.value as SellerStatus | '전체')}><option>전체</option>{statuses.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={reasonType} onChange={(event) => setReasonType(event.target.value)}><option value="">전체 변경 원인</option><option value="직접">판매자 직접</option><option value="신고">신고/정책</option><option value="활동 없음">장기 미활동</option><option value="승급">자동 승급</option></select><input type="date" className={shared.selectSm} defaultValue="2026-08-01"/><span>~</span><input type="date" className={shared.selectSm} defaultValue="2026-08-26"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={() => { setStatus('전체'); setReasonType(''); setSearch(''); setKeyword(''); }}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length}/><DataGrid columns={[{label:'판매자'},{label:'현재 상태'},{label:'상태 변경 사유'},{label:'최근 활동'},{label:'판매중 상품',align:'right'},{label:'취소율',align:'right'},{label:'신고',align:'right'},{label:'관리'}]} rows={rows} gridTemplate="minmax(175px,1.2fr) 82px minmax(220px,1.5fr) 126px 84px 64px 55px 55px" minWidth="1030px" empty={!filtered.length} emptyText="해당 상태의 판매자가 없습니다." showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0명'}/></GridArea>
