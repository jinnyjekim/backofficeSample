...
const STATUS_TABS: Array<SellerStatus | '전체'> = ['전체', '판매중', '판매중지', '판매제한', '휴면'];
...
export function SalesStatusPage() {
  ...
  return <div className={shared.page}>
    ...
    <div className={styles.riskHead}><div><h2>우선 검토 판매자</h2><p>신고와 취소율을 합산한 위험 순위</p></div></div>
    <div className={styles.riskGrid}>{riskSellers.map((seller,index) => <button type="button" key={seller.id} className={styles.riskCard} onClick={() => setSelectedId(seller.id)}><span className={styles.riskLabel}>{index+1}위 · {seller.nickname}</span><strong>{seller.reportCount * 10 + Math.round(seller.cancelRate)}점</strong><em>신고 {seller.reportCount}건 · 취소 {seller.cancelRate.toFixed(1)}%</em></button>)}</div>
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 닉네임 / 상태 변경 사유"/><button className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>{STATUS_TABS.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{item === '전체' ? sellers.length : sellers.filter((seller) => seller.status === item).length}</span>
        </CommonButton>
      );
    })}</div></form><div className={shared.filterRow2}><label className="globalFilterField"><span>판매자 상태</span><select aria-label="판매자 상태" className={shared.selectSm} value={status} onChange={(event) => setStatus(event.target.value as SellerStatus | '전체')}><option>전체</option>{statuses.map((item) => <option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>변경 원인</span><select aria-label="변경 원인" className={shared.selectSm} value={reasonType} onChange={(event) => setReasonType(event.target.value)}><option value="">전체 변경 원인</option><option value="직접">판매자 직접</option><option value="신고">신고/정책</option><option value="활동 없음">장기 미활동</option><option value="승급">자동 승급</option></select></label><label className={shared.dateFilterField}><span>변경일</span><div className={shared.dateRange}><DatePicker className={shared.selectSm} defaultValue="2026-08-01"/><span className={shared.dateSeparator}>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-26"/></div></label><span className={shared.rowSpacer}/><button type="button" className="detailFilterBtn">상세 필터</button><button type="button" className={shared.resetBtn} onClick={() => { setStatus('전체'); setReasonType(''); setSearch(''); setKeyword(''); }}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length}/><DataGrid columns={[{label:'판매자'},{label:'현재 상태'},{label:'상태 변경 사유'},{label:'최근 활동'},{label:'판매중 상품',align:'right'},{label:'취소율',align:'right'},{label:'신고',align:'right'}]} rows={rows} gridTemplate="minmax(190px,1.25fr) 88px minmax(240px,1.55fr) 132px 90px 70px 52px" minWidth="965px" empty={!filtered.length} emptyText="해당 상태의 판매자가 없습니다." showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0명'}/></GridArea>
