...
export function SalesPerformancePage(){
  ...
  return <section className={shared.page}>
    ...
    <GridArea><ResultBar count={ranked.length} unit="명"><select className={shared.selectSm} value={grade} onChange={(e)=>setGrade(e.target.value)}><option value="">전체 판매자 등급</option><option>일반</option><option>우수</option><option>파워</option></select></ResultBar><DataGrid columns={[{label:'순위',align:'right'},{label:'판매자'},{label:'거래수',align:'right'},{label:'누적 판매액',align:'right'},{label:'예상 수수료',align:'right'},{label:'취소율',align:'right'},{label:'상대 실적'}]} rows={rows} gridTemplate="52px minmax(180px,1.2fr) 56px 94px 88px 56px minmax(155px,1fr)" minWidth="770px" showPagination pages={pages} rangeLabel={`1–${ranked.length} / ${ranked.length}`}/></GridArea>
  </section>;
}
