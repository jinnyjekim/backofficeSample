...
export function SalesTradesPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(e)=>{e.preventDefault();setSearch(keyword.trim());}}><input className={shared.searchInput} value={keyword} onChange={(e)=>setKeyword(e.target.value)} placeholder="거래번호 / 상품 / 판매자 / 구매자"/><button className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>{QUICK.map((item) => {
    ...
    })}</div></form><div className={shared.filterRow2}><label className="globalFilterField"><span>결제수단</span><select aria-label="결제수단" className={shared.selectSm} value={payment} onChange={(e)=>setPayment(e.target.value)}><option value="">전체 결제수단</option><option>안전결제</option><option>카드</option><option>간편결제</option></select></label><label className="globalFilterField"><span>거래금액</span><select aria-label="거래금액" className={shared.selectSm} value={amount} onChange={(e)=>setAmount(e.target.value)}><option value="">전체 거래금액</option><option>10만 미만</option><option>100만 이상</option></select></label><label className={shared.dateFilterField}><span>결제일</span><div className={shared.dateRange}><DatePicker className={shared.selectSm} defaultValue="2026-08-01"/><span className={shared.dateSeparator}>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-26"/></div></label><span className={shared.rowSpacer}/><button type="button" className="detailFilterBtn">상세 필터</button><button type="button" className={shared.resetBtn} onClick={()=>{setKeyword('');setSearch('');setPayment('');setAmount('');}}>초기화</button></div></FilterBox></ControlArea>
