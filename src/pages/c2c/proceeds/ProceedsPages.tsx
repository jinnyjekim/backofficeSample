...
export function ProceedsOverviewPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><div className={shared.filterHeadRow}><div className={shared.quickFilters}>{quicks.map((item) => {
    ...
    })}</div><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>판매자 ID</option><option>판매자명</option><option>보류 근거</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 판매자명 / 보류 근거" /><button className={shared.searchBtn}>조회</button></form></div><FilterBox><div className={shared.filterRow2}><DateRangeFilter label="최근 정산일"/><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
...
export function WithdrawalManagementPage() {
  ...
  return <div className={shared.page}>
    ...
    })}</div><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>출금번호</option><option>판매자</option><option>계좌</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="출금번호 / 판매자 / 계좌" /><button className={shared.searchBtn}>조회</button></form></div><FilterBox><div className={shared.filterRow2}><DateRangeFilter label="요청일"/><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
