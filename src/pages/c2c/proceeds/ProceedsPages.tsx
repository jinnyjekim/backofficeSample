...
function DateRangeFilter({ label, from = '2026-08-20', to = '2026-08-27' }: { label: string; from?: string; to?: string }) {
  return <label className={styles.dateFilterField}><span>{label}</span><span className={styles.dateRange}><DatePicker defaultValue={from}/><span className={styles.dateSeparator} aria-hidden="true">~</span><DatePicker defaultValue={to}/></span></label>;
}
    ...
    <ControlArea><div className={shared.quickFilters}>{quicks.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{balances.filter((balance) => matchesBalanceQuick(balance, item)).length}</span>
        </CommonButton>
      );
    })}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>판매자 ID</option><option>판매자명</option><option>보류 근거</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="판매자 ID / 판매자명 / 보류 근거" /><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><DateRangeFilter label="최근 정산일"/><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="명"><ExcelDownloadButton data-grid-download onClick={() => downloadCsv('c2c-proceeds-overview.csv', ['판매자', '정산 예정', '출금 가능', '지급 보류', '출금 요청', '상태'], filtered.map((item) => [item.sellerName, item.scheduled, item.available, item.held, item.withdrawalRequested, item.status]))}/></ResultBar><DataGrid columns={[{ label: '판매자' }, { label: '정산 예정', align: 'right' }, { label: '출금 가능', align: 'right' }, { label: '지급 보류', align: 'right' }, { label: '출금 요청', align: 'right' }, { label: '계정 상태' }, { label: '출금계좌 / 최근 정산' }]} rows={rows} gridTemplate="95px 84px 88px 88px 74px 74px minmax(170px,1fr)" minWidth="765px" empty={!filtered.length} emptyText="조건에 맞는 판매대금이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
...
export function C2CSettlementManagementPage() {
  ...
  const rows: GridRow[] = filtered.map((item) => ({ id: item.id, onClick: () => setSelectedId(item.id), bg: item.status === '검토보류' ? '#fffafa' : undefined, cells: [{ kind: 'stack', title: item.id, subtitle: item.period }, { kind: 'stack', title: item.sellerName, subtitle: item.sellerId }, { kind: 'text', text: `${item.tradeCount}건`, align: 'right', numeric: true }, { kind: 'text', text: formatMoney(item.gross), align: 'right', numeric: true }, { kind: 'text', text: formatMoney(item.cancelRefund + item.fee), align: 'right', numeric: true, color: '#dc2626' }, { kind: 'text', text: `${item.adjustment >= 0 ? '+' : ''}${formatMoney(item.adjustment)}`, align: 'right', numeric: true }, { kind: 'text', text: formatMoney(item.net), align: 'right', numeric: true, weight: 700 }, { kind: 'badge', text: item.status, ...SETTLEMENT_STATUS_META[item.status] }, { kind: 'stack', title: item.assignee, subtitle: item.holdType }] }));
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{quicks.map((item) => {
      ...
      return (
        ...
        >
          <span className={shared.qfLabel}>{item}</span>
          <span className={shared.qfCount}>{settlements.filter((settlement) => item === '전체' || settlement.status === item).length}</span>
        </CommonButton>
      );
    })}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>정산번호</option><option>판매자</option><option>검토 이슈</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="정산번호 / 판매자 / 검토 이슈" /><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><DateRangeFilter label="정산 생성일"/><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><ExcelDownloadButton data-grid-download onClick={() => downloadCsv('c2c-settlements.csv', ['정산번호', '판매자', '거래수', '거래금액', '차감', '조정', '최종금액', '상태'], filtered.map((item) => [item.id, item.sellerName, item.tradeCount, item.gross, item.cancelRefund + item.fee, item.adjustment, item.net, item.status]))}/></ResultBar><DataGrid columns={[{ label: '정산번호 / 기간' }, { label: '판매자' }, { label: '거래', align: 'right' }, { label: '거래금액', align: 'right' }, { label: '환불·수수료', align: 'right' }, { label: '조정', align: 'right' }, { label: '최종 정산', align: 'right' }, { label: '상태' }, { label: '담당자 / 보류 구분' }]} rows={rows} gridTemplate="138px 90px 40px 84px 74px 72px 88px 74px 100px" minWidth="815px" empty={!filtered.length} emptyText="조건에 맞는 C2C 정산이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
...
export function WithdrawalManagementPage() {
  ...
  const reset = () => { setQuick('전체'); setKeyword(''); setSearch(''); };
  const rows: GridRow[] = filtered.map((item) => ({ id: item.id, onClick: () => setSelectedId(item.id), bg: item.status === '출금실패' ? '#fffafa' : undefined, cells: [{ kind: 'stack', title: item.id, subtitle: item.requestedAt }, { kind: 'stack', title: item.sellerName, subtitle: item.sellerId }, { kind: 'text', text: formatMoney(item.amount), align: 'right', numeric: true, weight: 700 }, { kind: 'text', text: item.bankAccount }, { kind: 'badge', text: item.status, ...WITHDRAWAL_STATUS_META[item.status] }, { kind: 'text', text: item.handler, color: item.handler === '미배정' ? '#dc2626' : '#52525b' }, { kind: 'stack', title: item.processedAt, subtitle: item.reason || '-' }] }));
  return <div className={shared.page}>
...
export function ProceedsLedgerPage() {
  ...
  const rows: GridRow[] = filtered.map((item) => ({ id: item.id, onClick: () => setSelectedId(item.id), cells: [{ kind: 'stack', title: item.occurredAt, subtitle: item.id }, { kind: 'stack', title: item.sellerName, subtitle: item.sellerId }, { kind: 'pillText', text: item.type, bg: item.type === '지급보류' ? '#fef2f2' : item.type === '정산확정' || item.type === '보류해제' ? '#ecfdf5' : '#f4f4f5', fg: item.type === '지급보류' ? '#dc2626' : item.type === '정산확정' || item.type === '보류해제' ? '#047857' : '#52525b' }, { kind: 'text', text: `${item.amount > 0 ? '+' : ''}${formatMoney(item.amount)}`, align: 'right', numeric: true, weight: 800, color: item.amount > 0 ? '#047857' : item.amount < 0 ? '#dc2626' : '#52525b' }, { kind: 'text', text: formatMoney(item.scheduledAfter), align: 'right', numeric: true }, { kind: 'text', text: formatMoney(item.availableAfter), align: 'right', numeric: true, weight: 700 }, { kind: 'text', text: formatMoney(item.heldAfter), align: 'right', numeric: true, color: item.heldAfter ? '#dc2626' : '#a1a1aa' }, { kind: 'stack', title: item.reference, subtitle: item.actor }, { kind: 'titleWarn', title: item.note }] }));
  return <div className={shared.page}>
    <PageHeading title="판매대금 원장" subtitle="정산 예정부터 확정·보류·출금까지 모든 판매대금 증감을 변경 불가능한 원장으로 조회합니다." />
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>원장 ID</option><option>판매자</option><option>연결 근거</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="원장 ID / 판매자 / 정산·출금·보류 번호" /><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>변동 유형</span><select aria-label="변동 유형" className={shared.selectSm} value={type} onChange={(event) => setType(event.target.value as LedgerType | '')}><option value="">전체 변동 유형</option>{types.map((item) => <option key={item}>{item}</option>)}</select></label><DateRangeFilter label="발생일"/><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><ExcelDownloadButton data-grid-download onClick={() => downloadCsv('c2c-proceeds-ledger.csv', ['발생일', '원장 ID', '판매자', '유형', '증감', '정산 예정 잔액', '출금 가능 잔액', '보류 잔액', '근거'], filtered.map((item) => [item.occurredAt, item.id, item.sellerName, item.type, item.amount, item.scheduledAfter, item.availableAfter, item.heldAfter, item.reference]))}/></ResultBar><DataGrid columns={[{ label: '발생일 / 원장 ID' }, { label: '판매자' }, { label: '변동 유형' }, { label: '증감', align: 'right' }, { label: '정산 예정 잔액', align: 'right' }, { label: '출금 가능 잔액', align: 'right' }, { label: '보류 잔액', align: 'right' }, { label: '연결 근거 / 처리자' }, { label: '사유' }]} rows={rows} gridTemplate="128px 90px 70px 96px 86px 88px 84px 135px minmax(180px,1fr)" minWidth="1005px" empty={!filtered.length} emptyText="조건에 맞는 판매대금 원장이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
