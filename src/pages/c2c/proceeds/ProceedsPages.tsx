...
export function WithdrawalManagementPage() {
  ...
  return <section className={shared.page}>
    ...
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={() => downloadCsv('c2c-withdrawals.csv', ['출금번호', '판매자', '금액', '계좌', '상태', '담당자'], filtered.map((item) => [item.id, item.sellerName, item.amount, item.bankAccount, item.status, item.handler]))}>다운로드</button></ResultBar><DataGrid columns={[{ label: '출금번호 / 요청일' }, { label: '판매자' }, { label: '출금 금액', align: 'right' }, { label: '출금계좌' }, { label: '상태' }, { label: '담당자' }, { label: '처리일 / 사유' }, { label: '관리' }]} rows={rows} gridTemplate="120px 90px 88px 98px 74px 62px minmax(175px,1fr) 55px" minWidth="810px" empty={!filtered.length} emptyText="조건에 맞는 출금 요청이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
