...
export function ChatAccessHistoryPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>관리자</option><option>채팅방</option><option>업무 번호</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="로그 / 관리자 / 채팅방 / 거래번호 / 연결 업무 번호"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>열람 사유</span><select aria-label="열람 사유" className={shared.selectSm} value={reason} onChange={(event)=>setReason(event.target.value)}><option value="">전체 열람 사유</option>{[...new Set(CHAT_ACCESS_LOGS.map((item)=>item.reason))].map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>접근 결과</span><select aria-label="접근 결과" className={shared.selectSm} value={result} onChange={(event)=>setResult(event.target.value)}><option value="">전체 접근 결과</option><option>허용</option><option>거부</option><option>만료</option></select></label><label className="globalFilterField"><span>내보내기</span><select aria-label="내보내기" className={shared.selectSm} value={exported} onChange={(event)=>setExported(event.target.value)}><option value="">전체 내보내기</option><option value="yes">내보내기 있음</option><option value="no">내보내기 없음</option></select></label><span>조회일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-26"/><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27"/><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
