    ...
    <ControlArea><div className={shared.filterHeadRow}><div className={shared.quickFilters}>{QUICKS.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{rooms.filter((room) => matchesQuick(room, item)).length}</span>
        </CommonButton>
      );
    })}</div><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>채팅방 ID</option><option>거래번호</option><option>상품</option><option>참여 회원</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="채팅방 / 거래번호 / 상품 / 구매자 / 판매자"/><button className={shared.searchBtn}>조회</button></form></div><FilterBox><div className={shared.filterRow2}><label className="globalFilterField"><span>위험도</span><select aria-label="위험도" className={shared.selectSm} value={risk} onChange={(event)=>setRisk(event.target.value as ChatRisk|'')}><option value="">전체 위험도</option><option>긴급</option><option>높음</option><option>주의</option><option>정상</option></select></label><label className="globalFilterField"><span>참여자</span><select aria-label="참여자" className={shared.selectSm} value={participant} onChange={(event)=>setParticipant(event.target.value)}><option value="">전체 참여자</option><option>구매자</option><option>판매자</option></select></label><label className={styles.dateFilterField}><span>최근 메시지</span><span className={styles.dateRange}><DatePicker controlSize="sm" defaultValue="2026-08-20"/><span className={styles.dateSeparator} aria-hidden="true">~</span><DatePicker controlSize="sm" defaultValue="2026-08-27"/></span></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="개"/><DataGrid columns={[{label:'채팅방 / 거래번호'},{label:'상품 / 최근 메시지'},{label:'구매자'},{label:'판매자'},{label:'메시지',align:'right'},{label:'위험 / 신고·제한'},{label:'거래 상태'},{label:'최근 메시지'}]} rows={rows} gridTemplate="138px minmax(220px,1.35fr) 92px 92px 46px 78px 72px 116px" minWidth="895px" empty={!filtered.length} emptyText="조건에 맞는 거래 채팅이 없습니다." emptySubtext="상태나 위험도 필터를 변경해 주세요." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></GridArea>
...
export function MessagePolicyPage() {
  ...
  const rows:GridRow[]=filtered.map((item)=>({id:item.id,onClick:()=>setSelectedId(item.id),cells:[{kind:'stack',title:item.id,subtitle:item.updatedAt},{kind:'stack',title:item.name,subtitle:item.category},{kind:'stack',title:item.detection,subtitle:item.patterns.join(' · ')},{kind:'pillText',text:item.action,bg:item.action==='발송 차단'?'#fef2f2':item.action==='마스킹'?'#f5f3ff':'#eff6ff',fg:item.action==='발송 차단'?'#dc2626':item.action==='마스킹'?'#6d28d9':'#1d4ed8'},{kind:'badge',text:item.severity,...CHAT_RISK_META[item.severity]},{kind:'text',text:item.exceptions},{kind:'stack',title:item.owner,subtitle:item.updatedAt},{kind:'badge',text:item.status,...statusMeta[item.status]}]}));
  return <div className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>정책 ID</option><option>정책명</option><option>탐지 패턴</option><option>담당 조직</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="정책 ID / 정책명 / 탐지 패턴 / 담당 조직"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>정책 분류</span><select aria-label="정책 분류" className={shared.selectSm} value={category} onChange={(event)=>setCategory(event.target.value)}><option value="">전체 정책 분류</option>{[...new Set(policies.map((item)=>item.category))].map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>처리 방식</span><select aria-label="처리 방식" className={shared.selectSm} value={action} onChange={(event)=>setAction(event.target.value)}><option value="">전체 처리 방식</option><option>표시</option><option>마스킹</option><option>발송 차단</option><option>신고 큐 생성</option></select></label><label className="globalFilterField"><span>정책 상태</span><select aria-label="정책 상태" className={shared.selectSm} value={status} onChange={(event)=>setStatus(event.target.value)}><option value="">전체 정책 상태</option><option>사용</option><option>검토 필요</option><option>중지</option></select></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="개"/><DataGrid columns={[{label:'정책 ID / 수정일'},{label:'정책 / 분류'},{label:'탐지 방식 / 패턴'},{label:'처리 방식'},{label:'위험도'},{label:'예외 조건'},{label:'담당 / 수정일'},{label:'상태'}]} rows={rows} gridTemplate="76px 100px minmax(215px,1.35fr) 84px 54px minmax(170px,1fr) 100px 75px" minWidth="915px" empty={!filtered.length} emptyText="조건에 맞는 메시지 정책이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0개'}/></GridArea>
...
export function ChatAccessHistoryPage() {
  ...
  return <div className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event)=>{event.preventDefault();setSearch(keyword.trim());}}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>관리자</option><option>채팅방</option><option>업무 번호</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="로그 / 관리자 / 채팅방 / 거래번호 / 연결 업무 번호"/><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><label className="globalFilterField"><span>열람 사유</span><select aria-label="열람 사유" className={shared.selectSm} value={reason} onChange={(event)=>setReason(event.target.value)}><option value="">전체 열람 사유</option>{[...new Set(CHAT_ACCESS_LOGS.map((item)=>item.reason))].map((item)=><option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>접근 결과</span><select aria-label="접근 결과" className={shared.selectSm} value={result} onChange={(event)=>setResult(event.target.value)}><option value="">전체 접근 결과</option><option>허용</option><option>거부</option><option>만료</option></select></label><label className="globalFilterField"><span>내보내기</span><select aria-label="내보내기" className={shared.selectSm} value={exported} onChange={(event)=>setExported(event.target.value)}><option value="">전체 내보내기</option><option value="yes">내보내기 있음</option><option value="no">내보내기 없음</option></select></label><label className={shared.dateFilterField}><span>조회일</span><span className={shared.dateRange}><DatePicker defaultValue="2026-08-26"/><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker defaultValue="2026-08-27"/></span></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><ExcelDownloadButton type="button" data-grid-download onClick={()=>downloadCsv('c2c-chat-access-audit.csv',['조회일','로그 ID','관리자','채팅방','거래번호','사유','업무 번호','범위','결과','접속 IP','세션','내보내기'],filtered.map((item)=>[item.accessedAt,item.id,item.adminId,item.roomId,item.tradeId,item.reason,item.ticketId,item.scope,item.result,item.ip,item.duration,item.exported?'있음':'없음']))}/></ResultBar><DataGrid columns={[{label:'조회일 / 로그 ID'},{label:'관리자 / 접속 IP'},{label:'채팅방 / 거래번호'},{label:'열람 사유 / 업무 번호'},{label:'접근 범위'},{label:'결과'},{label:'세션'},{label:'내보내기'}]} rows={rows} gridTemplate="128px 76px 138px 104px minmax(150px,1fr) 54px 62px 60px" minWidth="815px" empty={!filtered.length} emptyText="조건에 맞는 운영 조회 이력이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length?`1–${filtered.length} / ${filtered.length}`:'0건'}/></GridArea>
