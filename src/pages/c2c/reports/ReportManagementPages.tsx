    ...
    <ControlArea><div className={shared.filterHeadRow}><div className={shared.quickFilters}>{quicks.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span className={shared.qfCount}>{reports.filter((report) => matchesQuick(report, item)).length}</span>
        </CommonButton>
      );
    })}</div><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><label className="globalFilterField"><span>검색 범위</span><select aria-label="검색 범위" className={shared.selectSm}><option>통합 검색</option><option>신고번호</option><option>신고 대상</option><option>신고자 / 피신고자</option></select></label><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="신고번호 / 대상 / 신고자 / 피신고자 / 사유" /><button className={shared.searchBtn}>조회</button></form></div><FilterBox><div className={shared.filterRow2}><label className="globalFilterField"><span>신고 대상</span><select aria-label="신고 대상" className={shared.selectSm} value={targetType} onChange={(event) => selectTarget(event.target.value as ReportTargetType | '')}><option value="">전체 신고 대상</option>{TARGETS.map((item) => <option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>우선순위</span><select aria-label="우선순위" className={shared.selectSm} value={priority} onChange={(event) => setPriority(event.target.value as ReportPriority | '')}><option value="">전체 우선순위</option><option>긴급</option><option>높음</option><option>보통</option><option>낮음</option></select></label><label className="globalFilterField"><span>배정 상태</span><select aria-label="배정 상태" className={shared.selectSm} value={assignee} onChange={(event) => setAssignee(event.target.value)}><option value="">전체 배정 상태</option><option>미배정</option><option value="배정">담당자 배정</option></select></label><label className={shared.dateFilterField}><span>접수일</span><span className={shared.dateRange}><DatePicker defaultValue="2026-08-20" /><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker defaultValue="2026-08-27" /></span></label><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={() => downloadCsv('c2c-reports.csv', ['신고번호', '접수일', '대상 유형', '대상', '사유', '우선순위', '상태', '담당자'], filtered.map((item) => [item.id, item.receivedAt, item.targetType, item.targetId, item.category, item.priority, item.status, item.assignee]))}>다운로드</button></ResultBar><DataGrid columns={[{ label: '신고번호 / 접수일' }, { label: '대상 유형' }, { label: '신고 대상' }, { label: '신고 사유 / 내용' }, { label: '신고자 / 피신고자' }, { label: '우선순위' }, { label: '처리 상태' }, { label: '담당자 / 기한' }]} rows={rows} gridTemplate="122px 58px 164px minmax(240px,1.45fr) 102px 58px 74px 78px" minWidth="945px" empty={!filtered.length} emptyText="조건에 맞는 신고가 없습니다." emptySubtext="신고 대상이나 상태 필터를 변경해 주세요." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
