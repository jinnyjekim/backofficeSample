...
export function ReportProcessingPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><div className={shared.quickFilters}>{quicks.map((item) => <button type="button" key={item} className={`${shared.qfBtn} ${quick === item ? base.quickActive : ''}`} onClick={() => selectQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{reports.filter((report) => matchesQuick(report, item)).length}</span></button>)}</div><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>통합 검색</option><option>신고번호</option><option>신고 대상</option><option>신고자 / 피신고자</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="신고번호 / 대상 / 신고자 / 피신고자 / 사유" /><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={targetType} onChange={(event) => selectTarget(event.target.value as ReportTargetType | '')}><option value="">전체 신고 대상</option>{TARGETS.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={priority} onChange={(event) => setPriority(event.target.value as ReportPriority | '')}><option value="">전체 우선순위</option><option>긴급</option><option>높음</option><option>보통</option><option>낮음</option></select><select className={shared.selectSm} value={assignee} onChange={(event) => setAssignee(event.target.value)}><option value="">전체 배정 상태</option><option>미배정</option><option value="배정">담당자 배정</option></select><span>접수일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-20" /><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27" /><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={() => downloadCsv('c2c-reports.csv', ['신고번호', '접수일', '대상 유형', '대상', '사유', '우선순위', '상태', '담당자'], filtered.map((item) => [item.id, item.receivedAt, item.targetType, item.targetId, item.category, item.priority, item.status, item.assignee]))}>다운로드</button></ResultBar><DataGrid columns={[{ label: '신고번호 / 접수일' }, { label: '대상 유형' }, { label: '신고 대상' }, { label: '신고 사유 / 내용' }, { label: '신고자 / 피신고자' }, { label: '우선순위' }, { label: '처리 상태' }, { label: '담당자 / 기한' }, { label: '관리' }]} rows={rows} gridTemplate="150px 75px 145px minmax(240px,1.45fr) 135px 82px 85px 115px 55px" minWidth="1120px" empty={!filtered.length} emptyText="조건에 맞는 신고가 없습니다." emptySubtext="신고 대상이나 상태 필터를 변경해 주세요." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
...
export function ReportHistoryPage() {
  ...
  return <section className={shared.page}>
    ...
    <ControlArea><FilterBox><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>통합 검색</option><option>로그 ID</option><option>신고번호</option><option>대상 ID</option><option>처리자</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="로그 / 신고 / 대상 ID 또는 처리 사유" /><button className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={targetType} onChange={(event) => setTargetType(event.target.value as ReportTargetType | '')}><option value="">전체 신고 대상</option>{TARGETS.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={actor} onChange={(event) => setActor(event.target.value)}><option value="">전체 처리 주체</option><option value="관리자">관리자</option><option>SYSTEM</option></select><span>처리일</span><DatePicker className={shared.selectSm} defaultValue="2026-08-20" /><span>~</span><DatePicker className={shared.selectSm} defaultValue="2026-08-27" /><span className={shared.rowSpacer} /><button type="button" className={shared.resetBtn} onClick={reset}>초기화</button></div></FilterBox></ControlArea>
    <GridArea><ResultBar count={filtered.length} unit="건"><button type="button" className={shared.downloadBtn} onClick={() => downloadCsv('c2c-report-history.csv', ['처리일', '로그 ID', '신고번호', '대상', '처리', '변경 전', '변경 후', '처리자', '사유'], filtered.map((item) => [item.occurredAt, item.id, item.reportId, `${item.targetType}:${item.targetId}`, item.action, item.before, item.after, item.actor, item.reason]))}>다운로드</button></ResultBar><DataGrid columns={[{ label: '처리일 / 로그 ID' }, { label: '신고 / 대상' }, { label: '처리' }, { label: '변경 전' }, { label: '변경 후' }, { label: '처리자 / 연계' }, { label: '처리 사유' }, { label: '관리' }]} rows={rows} gridTemplate="160px 150px 125px 90px 90px 155px minmax(230px,1.4fr) 55px" minWidth="1050px" empty={!filtered.length} emptyText="조건에 맞는 신고 처리 이력이 없습니다." emptyActionLabel="필터 초기화" emptyActionClick={reset} showPagination pages={pages} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'} /></GridArea>
