...
export function PointLedgerPage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = entries.filter((e) => matchesQuickFilter(e, f, entries)).length;
    });
    return c;
  }, [entries]);
  ...
  const rows: GridRow[] = filtered.map((e) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: `${e.delta > 0 ? "+" : ""}${fmtPoint(e.delta)}`,
        color: increase ? "#059669" : "#dc2626",
        size: "12.5px",
        weight: 700,
        align: "right",
        numeric: true,
      ...
      {
        kind: "text",
        text: fmtPoint(e.after),
        color: e.after < 0 ? "#dc2626" : "#18181b",
        size: "12.5px",
        weight: 700,
        align: "right",
        numeric: true,
        ...
        <div className={styles.headRow}>
          <div>
            ...
            <div className={styles.subtitle}>
              회원별 포인트/적립금 지급, 사용, 차감, 소멸 및 복원 내역을
              조회합니다.
            </div>
          </div>
        </div>
        ...
        <div className={styles.filterBox}>
          ...
          >
            <input
              ...
              placeholder="내역번호, 회원 또는 Source 번호 검색"
            />
          ...
          <div className={styles.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                value={typeFilter}
                onChange={(e) =>
                  setTypeFilter(e.target.value as LedgerType | "")
                }
              >
        ...
        <DataGrid
          ...
          emptyText={
            entries.length === 0
              ? "포인트 변동 내역이 없습니다."
              : quickFilter === "확인 필요"
                ? "현재 확인이 필요한 포인트 내역이 없습니다."
                : "검색 조건에 해당하는 포인트 내역이 없습니다."
          }
          emptySubtext={
            entries.length > 0
              ? "검색어나 필터 조건을 변경해 주세요."
          ...
          emptyActionLabel={entries.length > 0 ? "초기화" : undefined}
          emptyActionClick={entries.length > 0 ? resetFilters : undefined}
        />
      </div>

      {selected && (
        <PointLedgerDetailDrawer
          key={selected.id}
          entry={selected}
          all={entries}
