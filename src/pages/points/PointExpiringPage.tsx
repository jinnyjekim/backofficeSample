import { CommonButton, CommonStatCard, CommonStatGrid, ExcelDownloadButton, PageSizeSelect } from '../../components/common';
...
export function PointExpiringPage() {
  ...
  const rows = useMemo<GridRow[]>(() => {
    ...
    return ENTRIES
      ...
      .map((entry) => {
        const days = Math.max(0, Math.ceil((new Date(entry.expiresAt).getTime() - today.getTime()) / (1000 * 60 * 60 * 24)));
        return { id: entry.id, cells: [
          { kind: 'text', text: entry.id, weight: 650, numeric: true },
          { kind: 'text', text: entry.expiresAt, color: '#dc2626', weight: 650, numeric: true },
          { kind: 'text', text: entry.member, weight: 650 },
          { kind: 'badge', text: entry.status, bg: entry.status === '정상' ? '#ecfdf5' : '#fef2f2', fg: entry.status === '정상' ? '#047857' : '#b91c1c' },
  ...
  return <div className={styles.page}>
    ...
    <CommonStatGrid columns={3} style={{ marginBottom: 14 }}>
      <CommonStatCard label="소멸 예정 회원" value={`${new Set(ENTRIES.map((entry) => entry.member)).size}명`} />
      <CommonStatCard label="소멸 예정 건수" value={`${ENTRIES.length}건`} />
      <CommonStatCard label="소멸 예정 포인트" value={fmtPoint(ENTRIES.reduce((sum, entry) => sum + entry.amount, 0))} tone="warn" />
    </CommonStatGrid>
