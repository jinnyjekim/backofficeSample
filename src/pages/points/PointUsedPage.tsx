...
export function PointUsedPage() {
  ...
  return <div className={styles.page}>
    ...
    <CommonStatGrid columns={3} style={{ marginBottom: 14 }}>
      <CommonStatCard label="총 사용 건수" value={`${ENTRIES.length}건`} />
      <CommonStatCard label="총 사용 포인트" value={fmtPoint(ENTRIES.reduce((sum, entry) => sum + Math.abs(entry.delta), 0))} />
      <CommonStatCard label="출처 확인 필요" value={`${ENTRIES.filter((entry) => !entry.sourceId).length}건`} tone={ENTRIES.some((entry) => !entry.sourceId) ? 'warn' : 'neutral'} />
    </CommonStatGrid>
