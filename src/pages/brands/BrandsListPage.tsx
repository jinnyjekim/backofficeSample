...
type ConfirmState =
  | { kind: "toggleStatus"; item: Brand }
  | { kind: "delete"; item: Brand }
...
export function BrandsListPage() {
  ...
  const issuesMap = useMemo(() => {
    ...
    brands.forEach((b) => {
      map[b.id] = computeIssues(b, brands);
    });
    return map;
  }, [brands]);
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = brands.filter((b) => matchesQuickFilter(b, f)).length;
    });
    return c;
  }, [brands]);
  ...
  const selected = drawerId
    ? (brands.find((b) => b.id === drawerId) ?? null)
    : null;
  ...
  function toggleExposure(item: Brand) {
    const updated = history(
      ...
      "노출 상태 변경",
      item.exposure ? "노출" : "비노출",
      item.exposure ? "비노출" : "노출",
    );
    setBrands((prev) => prev.map((b) => (b.id === updated.id ? updated : b)));
    toastBriefly(
      item.exposure ? "비노출로 전환했습니다." : "노출로 전환했습니다.",
    );
  }
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "toggleStatus") {
      const updated = history(
        {
          ...confirm.item,
          status: confirm.item.status === "사용중" ? "미사용" : "사용중",
        },
        confirm.item.status === "사용중" ? "미사용 처리" : "사용 재개",
      );
      setBrands((prev) => prev.map((b) => (b.id === updated.id ? updated : b)));
      toastBriefly(
        confirm.item.status === "사용중"
          ? "브랜드를 미사용 처리했습니다."
          : "브랜드를 사용 재개했습니다.",
      );
  ...
  function rowMenuItems(b: Brand, status: BrandStatus) {
    ...
    items.push({ sep: true });
    items.push({
      label: status === "사용중" ? "미사용 처리" : "사용 재개",
      click: () => setConfirm({ kind: "toggleStatus", item: b }),
    });
    if (b.productCodes.length === 0)
      items.push({
        label: "삭제",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "delete", item: b }),
      });
    return items;
  }
  ...
  const rows: GridRow[] = filtered.map((b) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: b.productCodes.length > 0 ? `${b.productCodes.length}개` : "-",
        color: "#3f3f46",
        size: "12px",
        weight: 600,
        align: "right",
      ...
      {
        kind: "badge",
        text: b.exposure ? "노출" : "비노출",
        bg: EXPOSURE_META[b.exposure ? "노출" : "비노출"].bg,
        fg: EXPOSURE_META[b.exposure ? "노출" : "비노출"].fg,
      },
      {
        kind: "text",
        text: b.updatedAt.slice(5).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
    ...
    <div className={styles.page} onClick={() => menuId && setMenuId(null)}>
        ...
        <DataGrid
          ...
          emptyText={
            brands.length === 0
              ? "등록된 브랜드가 없습니다."
              : quickFilter === "상품 미연결"
                ? "현재 상품이 연결되지 않은 브랜드가 없습니다."
                : "검색 조건에 해당하는 브랜드가 없습니다."
          }
          emptySubtext={
            brands.length === 0
              ? "상품에 사용할 브랜드를 등록해 주세요."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={brands.length === 0 ? "+ 브랜드 등록" : "초기화"}
          emptyActionClick={
            brands.length === 0 ? () => openEditor("new") : resetFilters
          }
        />
      </div>
      ...
      {selected && (
        <BrandDetailDrawer
          ...
          onToggleStatus={() =>
            setConfirm({ kind: "toggleStatus", item: selected })
          }
      ...
      {showOrder && (
        <BrandExposureOrderDrawer
          brands={brands}
          onCancel={() => setShowOrder(false)}
          onSave={saveExposureOrder}
        />
      )}
      ...
      {confirm?.kind === "delete" && (
        ...
        >
          <div className={styles.dialogBox}>
            ...
            >{`'${confirm.item.name}'이(가) 삭제됩니다. 연결된 상품이 없는 브랜드만 삭제할 수 있습니다.`}</div>
            <div className={styles.dialogActions}>
              <button
                type="button"
                className={styles.dialogBtn}
        ...
        >
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogBody}>
              선택한 브랜드를 모두 {confirm.expose ? "노출" : "비노출"} 상태로
              변경합니다.
            </div>
            <div className={styles.dialogActions}>
              <button
