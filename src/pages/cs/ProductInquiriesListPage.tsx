...
function matchesSearch(
  ...
  if (scope === "문의 제목")
    return (
      q.title.toLowerCase().includes(k) || q.content.toLowerCase().includes(k)
    );
  if (scope === "작성자") return q.member.toLowerCase().includes(k);
  if (scope === "상품")
    return (
      q.productCode.toLowerCase().includes(k) ||
      productName(q.productCode).toLowerCase().includes(k)
    );
  return (
    q.id.toLowerCase().includes(k) ||
    q.title.toLowerCase().includes(k) ||
    q.member.toLowerCase().includes(k) ||
    q.productCode.toLowerCase().includes(k) ||
    productName(q.productCode).toLowerCase().includes(k)
  );
}
...
export function ProductInquiriesListPage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = inquiries.filter(
        (q) => matchesQuickFilter(q, f) && !q.hidden,
      ).length;
    });
    return c;
  }, [inquiries]);
  ...
  const rows: GridRow[] = filtered.map((q) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: productName(q.productCode),
        color: "#3f3f46",
        size: "12px",
        weight: 500,
      },
      {
        kind: "titleWarn",
        title: `${q.visibility === "비공개" ? "🔒 " : ""}${q.title}`,
      ...
      {
        kind: "text",
        text: q.createdAt.slice(5, 10).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              상품 문의를 하나의 업무 큐에서 조회하고 답변·노출 상태와 처리
              기록을 관리합니다.
            </div>
          </div>
        </div>
