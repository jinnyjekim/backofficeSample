...
const COLUMNS = [{ label: '문의 유형명' }, { label: '적용 범위' }, { label: '상위 유형' }, { label: '유형 코드' }, { label: '기본 담당팀' }, { label: '우선순위' }, { label: 'SLA' }, { label: '신규 접수' }, { label: '사용자 노출' }, { label: '상태' }, { label: '최근 30일' }, { label: '수정일' }];
...
export function InquiryTypesPage() {
  ...
  return <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
    <header className={shared.header}>
      <div className={shared.headerTop}><div><div className={shared.title}>문의 유형 관리</div><div className={shared.subtitle}>1:1 문의의 분류, 자동 Routing, 우선순위, SLA와 입력 항목을 관리합니다.</div></div><button type="button" className={shared.createBtn} onClick={() => openCreate()}>+ 문의 유형 등록</button></div>
        ...
        {QUICK_FILTERS.map((filter) => {
          ...
          return (
            ...
            >
              ...
              <span className={shared.qfCount}>{types.filter((item) => matchesConfigScope(inquiryTypeScopes(item), scopeFilter) && filterMatch(item, filter, types)).length}</span>
            </CommonButton>
          );
        })}
      </div>
