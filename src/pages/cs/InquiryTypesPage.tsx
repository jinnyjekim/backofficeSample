...
import {
  ...
  newInquiryType,
  typeErrors,
  type InquiryTypeEntry,
  type TypeStatus,
} from "./inquiryTypesData";
...
type QuickFilter =
  ...
  | "사용자 숨김";
type ConfirmState =
  | { kind: "deactivate"; item: InquiryTypeEntry }
  | { kind: "delete"; item: InquiryTypeEntry }
  | null;
...
function filterMatch(
  item: InquiryTypeEntry,
  filter: QuickFilter,
  all: InquiryTypeEntry[],
) {
...
export function InquiryTypesPage() {
  ...
  const [drawerMode, setDrawerMode] = useState<"view" | "edit" | "preview">(
    "view",
  );
  ...
  const filtered = useMemo(
    () =>
      types.filter((item) => {
        ...
        if (!matchesConfigScope(inquiryTypeScopes(item), scopeFilter))
          return false;
        if (
          search &&
          !`${item.name} ${item.code} ${item.parent ?? ""}`
            .toLowerCase()
            .includes(search.toLowerCase())
        )
          return false;
        if (status && item.status !== status) return false;
        if (team === "미설정" ? item.team !== null : team && item.team !== team)
          return false;
  ...
  const openCreate = (seed = newInquiryType()) => {
    setDrawerItem(seed);
    setIsNew(true);
    setDrawerMode("edit");
  };
  ...
  const save = (item: InquiryTypeEntry) => {
    if (isNew) {
      const saved = history(
        { ...item, status: item.status, history: [] },
        "문의 유형 생성",
      );
      setTypes((current) => [...current, saved]);
      setDrawerItem(saved);
      setIsNew(false);
      toastBriefly("문의 유형을 등록했습니다.");
    } else {
  ...
  const duplicate = (item: InquiryTypeEntry) =>
    openCreate({
      ...item,
      id: `TYPE-${Date.now()}`,
      name: `[복사본] ${item.name}`,
  ...
  const deactivate = (item: InquiryTypeEntry) => {
    const activeChildren = types.filter(
      (child) => child.parent === item.name && child.status === "사용",
    );
    if (item.depth === 1 && activeChildren.length)
      return toastBriefly(
        `사용 중인 하위 유형 ${activeChildren.length}개를 먼저 비활성화하거나 이동해 주세요.`,
      );
  ...
  const confirmAction = () => {
    ...
    if (confirm.kind === "delete") {
      ...
      setDrawerItem(null);
      toastBriefly("사용 이력이 없는 유형을 삭제했습니다.");
    } else {
      const updated = history(
        { ...confirm.item, status: "비활성", visible: false, intake: "중지" },
        "문의 유형 비활성",
        "기존 문의와 통계는 유지",
      );
      setTypes((current) =>
      ...
      setDrawerItem(null);
      toastBriefly("신규 접수를 중지하고 유형을 비활성화했습니다.");
    }
    setConfirm(null);
  };

  const move = (item: InquiryTypeEntry, direction: -1 | 1) => {
    const siblings = types
      .filter((value) => value.parent === item.parent)
    ...
    setTypes((current) =>
      current.map((value) =>
        value.id === item.id
          ? { ...value, displayOrder: swap.displayOrder }
          : value.id === swap.id
  ...
  const rows: GridRow[] = filtered.map((item) => {
    ...
    return {
      ...
      bg: errors.length ? "#fffdf8" : undefined,
      cells: [
        {
          kind: "titleWarn",
          title: `${item.depth === 2 ? "└ " : ""}${item.name}`,
          hasIssue: errors.length > 0,
          issueTitle: errors.join(" · "),
        },
        {
          kind: "stack",
          title: inquiryTypeScopes(item).join(" · "),
          subtitle: inquiryTypeScopes(item).includes("공통")
            ? "전 서비스"
            : `${inquiryTypeScopes(item).length}개 서비스`,
        },
        {
          kind: "text",
          text: item.parent ?? "대분류",
          size: "12px",
          color: item.parent ? "#52525b" : "#a1a1aa",
        },
        {
          kind: "text",
          text: item.code,
        ...
        },
        {
          ...
          text: item.team ?? "미설정",
          size: "12px",
          color: item.team ? "#3f3f46" : "#dc2626",
          weight: item.team ? 500 : 700,
        },
        {
          kind: "pillText",
          text: item.priority,
        ...
        {
          kind: "stack",
          title: item.firstResponseHours
            ? `첫 답변 ${item.firstResponseHours}h`
            : "첫 답변 미설정",
          subtitle: item.resolutionHours
            ? `처리 ${item.resolutionHours}h · ${item.businessHours ? "영업시간" : "24시간"}`
            : "처리 SLA 미설정",
        },
        {
          kind: "badge",
        ...
        {
          kind: "badge",
          text: item.visible ? "노출" : "숨김",
          bg: item.visible ? "#eff6ff" : "#f4f4f5",
          fg: item.visible ? "#2563eb" : "#71717a",
        },
        {
          kind: "statusDot",
          text: item.status,
        ...
        {
          kind: "text",
          text: `${item.recentCount.toLocaleString()}건`,
          size: "12px",
          numeric: true,
        },
    ...
    <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
      <header className={shared.header}>
        <div className={shared.headerTop}>
          <div>
            <div className={shared.title}>문의 유형 관리</div>
            <div className={shared.subtitle}>
              1:1 문의의 분류, 자동 Routing, 우선순위, SLA와 입력 항목을
              관리합니다.
            </div>
          </div>
          <button
                    ...
                    <span className={shared.qfCount}>
                      {
                        types.filter(
                          (item) =>
                            matchesConfigScope(
                              inquiryTypeScopes(item),
                              scopeFilter,
                            ) && filterMatch(item, filter, types),
                        ).length
                      }
                    </span>
                  </CommonButton>
      ...
      {selected.length > 0 && (
        <div className={shared.bulkBar}>
          ...
          <select
            ...
            defaultValue=""
            onChange={(e) => {
              const value = e.target.value as TypeStatus;
              if (!value) return;
              setTypes((current) =>
                current.map((item) =>
                  selected.includes(item.id)
          ...
          <button
            ...
            onClick={() => {
              ...
              toastBriefly("선택 유형을 사용자 화면에 노출합니다.");
            }}
          >
            사용자 노출
          </button>
          <span className={styles.bulkGuard}>
            SLA는 영향 범위가 커서 개별 유형에서만 변경할 수 있습니다.
          </span>
        </div>
      )}
      ...
      <div className={shared.gridWrap}>
        ...
        <DataGrid
          columns={COLUMNS}
          rows={rows}
          gridTemplate="100px 110px 72px 132px 68px 54px 106px 76px 56px 60px 52px 96px"
          minWidth="1145px"
          selectable
          allSelected={
            filtered.length > 0 &&
      ...
      {orderOpen && (
        ...
        >
          <div className={`${shared.dialogBox} ${styles.orderDialog}`}>
            <h2 className={shared.dialogTitle}>문의 유형 노출 순서</h2>
            <p className={shared.dialogBody}>
              검색·필터와 분리된 전체 목록에서 같은 Depth의 순서를 조정합니다.
            </p>
            <div className={styles.orderList}>
              {types
                ...
                .sort((a, b) => a.displayOrder - b.displayOrder)
                .map((parent) => (
                  <div key={parent.id} className={styles.orderGroup}>
                    <OrderRow item={parent} onMove={move} />
                    <div>
                      {types
      ...
      {confirm && (
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <p className={shared.dialogBody}>
              {confirm.kind === "delete"
                ? "사용 및 노출 이력이 없는 유형입니다. 삭제하면 복구할 수 없습니다."
                : "신규 접수와 사용자 노출이 중지됩니다. 기존 문의 데이터와 접수 당시 분류·SLA는 그대로 유지됩니다."}
            </p>
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>문의 유형</span>
                <strong>
                  {confirm.item.parent ? `${confirm.item.parent} > ` : ""}
                  {confirm.item.name}
                </strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>전체 연결 문의</span>
                <strong>{confirm.item.totalCount.toLocaleString()}건</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>진행중 문의</span>
                <strong>{confirm.item.openCount}건</strong>
              </div>
            </div>
            <div className={shared.dialogActions}>
              <button
...
function OrderRow({
  ...
  item: InquiryTypeEntry;
  child?: boolean;
  onMove: (item: InquiryTypeEntry, direction: -1 | 1) => void;
}) {
  return (
    <div className={`${styles.orderRow} ${child ? styles.orderChild : ""}`}>
      <span className={styles.dragHandle}>☰</span>
      <strong>{item.name}</strong>
      <span>{item.status}</span>
      <button type="button" onClick={() => onMove(item, -1)}>
        ↑
      </button>
      <button type="button" onClick={() => onMove(item, 1)}>
        ↓
