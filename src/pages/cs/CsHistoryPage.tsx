  ...
  const filtered = useMemo(
    () =>
      CS_AUDIT_LOGS.filter((log) => {
        ...
        if (search) {
          const haystack =
            `${log.targetId} ${log.customerId ?? ""} ${log.customerName ?? ""} ${log.actorId} ${log.id}`.toLowerCase();
    ...
    <div className={shared.page}>
        ...
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              고객센터에서 발생한 관리자 및 시스템 처리 내역을 조회합니다.
            </div>
          </div>
        </div>
      ...
      {selectedLogId &&
        (() => {
          ...
          return log ? (
            <CsHistoryDetailDrawer
              key={log.id}
              log={log}
              all={CS_AUDIT_LOGS}
              onClose={() => setSelectedLogId(null)}
              onJump={setSelectedLogId}
            />
          ) : null;
        })()}
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <p className={shared.dialogBody}>
              {selected.length > 0
                ? `선택한 ${selected.length}건을 다운로드합니다.`
                : `현재 검색 결과 ${filtered.length}건을 다운로드합니다.`}
            </p>
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>포함 항목</span>
                <strong>
                  처리일시 · 대상 · 작업유형 · 변경내용 · 처리자 · 담당팀 ·
                  Source · 결과
                </strong>
              </div>
            </div>
            <p className={styles.privacyNote}>
              CS 처리 이력 다운로드는 별도 작업 이력으로 기록됩니다.
            </p>
