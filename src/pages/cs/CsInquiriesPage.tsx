      ...
      <header className={shared.header}>
        ...
        <div className={shared.filterBox}>
          ...
          <div className={shared.filterRow2}>
            <label className="globalFilterField"><span>비즈니스 범위</span><select className={shared.selectSm} value={businessScope} onChange={(event) => { setBusinessScope(event.target.value as BusinessScope); setSelected([]); }} aria-label="비즈니스 범위">{BUSINESS_SCOPES.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label className="globalFilterField"><span>상태</span><select aria-label="상태" className={shared.selectSm} value={status} onChange={(event) => setStatus(event.target.value as InquiryStatus | '')}><option value="">전체 상태</option>{INQUIRY_STATUSES.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label className="globalFilterField"><span>유형</span><select aria-label="유형" className={shared.selectSm} value={category} onChange={(event) => setCategory(event.target.value)}><option value="">전체 유형</option>{INQUIRY_CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label className="globalFilterField"><span>담당자</span><select aria-label="담당자" className={shared.selectSm} value={manager} onChange={(event) => setManager(event.target.value)}><option value="">전체 담당자</option><option>미배정</option>{INQUIRY_MANAGERS.map((item) => <option key={item}>{item}</option>)}</select></label>
            <label className="globalFilterField"><span>우선순위</span><select aria-label="우선순위" className={shared.selectSm} value={priority} onChange={(event) => setPriority(event.target.value as InquiryPriority | '')}><option value="">전체 우선순위</option>{INQUIRY_PRIORITIES.map((item) => <option key={item}>{item}</option>)}</select></label>
      ...
      <div className={shared.splitRow}>
        ...
        <div className={shared.splitDetailCol}>
          {currentInquiry ? (
            ...
            <div className={shared.splitEmpty}>왼쪽 목록에서 문의를 선택하면<br />문의 상세와 답변 작성이 여기에 표시됩니다.</div>
          )}
        </div>
      </div>
