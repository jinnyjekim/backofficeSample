...
export function CsInquiriesPage() {
  ...
  const [quickFilter, setQuickFilter] =
    useState<InquiryQuickFilter>("처리 필요");
  ...
  const scopeItems = useMemo(
    () =>
      inquiries.filter(
        (inquiry) =>
          businessScope === "통합" || inquiry.businessType === businessScope,
      ),
    [inquiries, businessScope],
  );
  const filtered = useMemo(
    () =>
      scopeItems.filter((inquiry) => {
        if (!matchesQuickFilter(inquiry, quickFilter)) return false;
        const haystack =
          `${inquiry.id} ${inquiry.title} ${inquiry.body} ${inquiry.customer.id} ${inquiry.customer.name} ${inquiry.relatedItems.map((item) => item.id).join(" ")}`.toLowerCase();
        ...
        if (
          manager === "미배정"
            ...
            : manager && inquiry.assignee !== manager
        )
          return false;
        if (priority && inquiry.priority !== priority) return false;
        if (receivedFrom && inquiry.receivedAt.slice(0, 10) < receivedFrom)
          return false;
        if (receivedTo && inquiry.receivedAt.slice(0, 10) > receivedTo)
          return false;
  ...
  const patchInquiry = (
    id: string,
    transform: (item: InquiryEntry) => InquiryEntry,
  ) => {
  ...
  const assign = () => {
    ...
    setInquiries((current) =>
      current.map((item) =>
        dialog.ids.includes(item.id)
          ? appendHistory(
              {
                ...item,
                assignee: assignManager,
                team: TEAM_BY_MANAGER[assignManager],
              },
              item.assignee ? "담당자 변경" : "담당자 지정",
              `${assignManager} · ${assignMemo || TEAM_BY_MANAGER[assignManager]}`,
            )
          : item,
      ),
    );
          ...
          ? appendHistory(
              {
                ...item,
                status: "처리 완료",
                completionReason: completeReason,
              },
              "처리 완료",
              `${completeReason}${completeMemo ? ` · ${completeMemo}` : ""}`,
            )
          : item,
      ),
    );
  ...
  const start = (id: string) =>
    patchInquiry(id, (item) =>
      appendHistory(
        {
          ...item,
          status: "처리중",
          assignee: item.assignee ?? "admin01",
          team: item.team ?? TEAM_BY_MANAGER.admin01,
        },
        "처리 시작",
        item.assignee ? undefined : "admin01 자동 배정",
      ),
    );

  const hold = (id: string) =>
    patchInquiry(id, (item) =>
      appendHistory({ ...item, status: "보류" }, "문의 보류", "내부 확인 필요"),
    );
  const reopen = (id: string) =>
    patchInquiry(id, (item) =>
      appendHistory(
        { ...item, status: "처리중", reopened: true },
        "문의 재오픈",
        "추가 확인 및 답변 필요",
      ),
    );
  ...
  const download = (items: InquiryEntry[]) => {
    const header =
      "문의번호,서비스,유형,제목,고객,접수일,기한,담당자,상태,우선순위";
  ...
  const rows: GridRow[] = filtered.map((inquiry) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: inquiry.businessType,
          ...BUSINESS_BADGE_META[inquiry.businessType],
        },
        {
          kind: "pillText",
          text: inquiry.category,
          sub: inquiry.subcategory,
          bg: "#f4f4f5",
          fg: "#52525b",
        },
        {
        ...
        },
        {
          ...
          subtitle: inquiry.customer.id,
          avatarChar: inquiry.customer.name.slice(0, 1),
          avatarBg: "#eef2ff",
          avatarFg: "#4f46e5",
        },
        related
        ...
        {
          kind: "text",
          text: inquiry.assignee ?? "미배정",
          size: "12px",
          color: inquiry.assignee ? "#3f3f46" : "#dc2626",
          weight: inquiry.assignee ? 500 : 700,
        },
        {
          kind: "badge",
          text: inquiry.status,
    ...
    <div className={shared.page}>
      <header className={shared.header}>
        <div className={shared.headerTop}>
          <div>
            <div className={shared.title}>1:1 문의</div>
            <div className={shared.subtitle}>
              접수부터 답변, 완료까지 문의 처리 흐름과 SLA를 한 화면에서
              관리합니다.
            </div>
          </div>
        ...
        <div className={shared.filterBox}>
          ...
          >
            <input
              ...
              placeholder="문의번호, 제목, 고객, 주문·결제번호 검색"
            />
            ...
            <div className={shared.quickFilters}>
              {QUICK_FILTERS.map((filter) => {
                const count = scopeItems.filter((item) =>
          ...
          <div className={shared.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                value={businessScope}
                onChange={(event) => {
                  setBusinessScope(event.target.value as BusinessScope);
                  setSelected([]);
                }}
                aria-label="비즈니스 범위"
              >
                {BUSINESS_SCOPES.map((item) => (
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={priority}
                onChange={(event) =>
                  setPriority(event.target.value as InquiryPriority | "")
                }
              >
      ...
      {selected.length > 0 && (
        <div className={shared.bulkBar}>
          ...
          <button
            ...
            onClick={() => setDialog({ kind: "assign", ids: selected })}
          >
            담당자 일괄 지정
          </button>
          <select
          ...
          <ExcelDownloadButton
            data-grid-download
            onClick={() =>
              download(inquiries.filter((item) => selected.includes(item.id)))
            }
          />
          <span className={styles.bulkGuard}>
            답변·완료 처리는 개별 문의에서만 가능합니다.
          </span>
        </div>
      )}
      ...
      <div className={shared.gridWrap}>
        ...
        <DataGrid
          columns={COLUMNS}
          rows={rows}
          gridTemplate="128px 56px 84px minmax(236px,1.7fr) 126px 96px 88px 122px 66px 88px 52px"
          minWidth="1275px"
          selectable
          allSelected={
            filtered.length > 0 &&
          ...
          emptySubtext="빠른 필터나 검색 조건을 변경해 보세요."
      ...
      {currentInquiry && (
        <InquiryDetailDrawer
          ...
          onAssign={() =>
            setDialog({ kind: "assign", ids: [currentInquiry.id] })
          }
          onStart={() => start(currentInquiry.id)}
          onHold={() => hold(currentInquiry.id)}
          onComplete={() =>
            setDialog({ kind: "complete", ids: [currentInquiry.id] })
          }
          onReopen={() => reopen(currentInquiry.id)}
          onSaveDraft={(body) =>
            patchInquiry(currentInquiry.id, (item) =>
              appendHistory(
                { ...item, replyDraft: body, draftSavedAt: "2026-08-24 14:00" },
                "답변 임시저장",
              ),
            )
          }
          onSendReply={(body, channels) =>
            patchInquiry(currentInquiry.id, (item) =>
              appendHistory(
                {
                  ...item,
                  ...
                  messages: [
                    ...item.messages,
                    {
                      ...
                      role: "admin",
                      author: item.assignee ?? "admin01",
                      sentAt: "2026-08-24 14:00",
                      body,
                      notificationResult: `${channels.join(" / ") || "서비스 내"} 발송 완료`,
                    },
                  ],
                },
                "고객 답변 발송",
        ...
        >
          <div className={shared.dialogBox}>
            <h2 className={shared.dialogTitle}>
              {dialog.kind === "assign" ? "담당자 지정" : "처리 완료"}
            </h2>
            <p className={shared.dialogBody}>
              {dialog.ids.length}건의 문의에 변경 사항을 적용합니다. 모든 변경은
              처리 이력에 기록됩니다.
            </p>
            {dialog.kind === "assign" ? (
              <>
                <label className={drawer.formLabel}>담당자</label>
                <select
                ...
                >
                  {INQUIRY_MANAGERS.map((item) => (
                    <option key={item} value={item}>
                      {item} · {TEAM_BY_MANAGER[item]}
                    </option>
                  ))}
                </select>
                <label className={drawer.formLabel}>인수인계 메모 (선택)</label>
                <textarea
              ...
              <>
                <label className={drawer.formLabel}>완료 사유</label>
                <select
                  className={styles.dialogControl}
                ...
                >
                  <option>안내 완료</option>
                  <option>조치 완료</option>
                  <option>고객 확인 완료</option>
                  <option>중복 문의</option>
                  <option>처리 불가</option>
                </select>
                <label className={drawer.formLabel}>완료 메모 (선택)</label>
                <textarea
                  className={styles.dialogTextarea}
                  value={completeMemo}
                  onChange={(event) => setCompleteMemo(event.target.value)}
                  placeholder="완료 판단 근거를 입력하세요."
                />
              </>
            )}
            <div className={shared.dialogActions}>
              ...
              <button
                ...
                onClick={dialog.kind === "assign" ? assign : complete}
              >
                {dialog.kind === "assign" ? "지정" : "처리 완료"}
              </button>
            </div>
          </div>
        </div>
