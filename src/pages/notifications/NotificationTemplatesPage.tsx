      ...
      <div className={shared.headTop}>
        ...
        <div className={shared.filterBox}>
          ...
          <div className={shared.filterRow2}>
            <select className={shared.selectSm} value={moduleFilter} onChange={(event) => setModuleFilter(event.target.value)}><option value="">사용 모듈 전체</option>{TEMPLATE_MODULES.map((item) => <option key={item}>{item}</option>)}</select>
            <select className={shared.selectSm} value={purposeFilter} onChange={(event) => setPurposeFilter(event.target.value)}><option value="">사용 목적 전체</option>{TEMPLATE_PURPOSES.map((item) => <option key={item}>{item}</option>)}</select>
            <select className={shared.selectSm} value={businessFilter} onChange={(event) => setBusinessFilter(event.target.value)}><option value="">연결 업무 전체</option>{TEMPLATE_BUSINESSES.map((item) => <option key={item}>{item}</option>)}</select>
            <select className={shared.selectSm} value={sendTypeFilter} onChange={(event) => setSendTypeFilter(event.target.value)}><option value="">발송 유형 전체</option>{TEMPLATE_SEND_TYPES.map((item) => <option key={item}>{item}</option>)}</select>
            <select className={shared.selectSm} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">사용 상태 전체</option>{TEMPLATE_STATUSES.map((item) => <option key={item}>{item}</option>)}</select>
