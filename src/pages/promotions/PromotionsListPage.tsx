...
const GRID_TEMPLATE = "1.5fr 44px 100px 64px 150px 40px 76px 62px 44px";
...
export function PromotionsListPage() {
  ...
  const [discountMethodFilter, setDiscountMethodFilter] = useState<
    DiscountMethod | ""
  >("");
  ...
  const issuesMap = useMemo(() => {
    ...
    promotions.forEach((p) => {
      map[p.id] = computeIssues(p, promotions);
    });
    return map;
  }, [promotions]);
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = promotions.filter((p) =>
        matchesQuickFilter(p, f, issuesMap),
      ).length;
    });
    return c;
  ...
  const filtered = useMemo(
    () =>
      promotions.filter((p) => {
        ...
        if (discountMethodFilter && p.discountMethod !== discountMethodFilter)
          return false;
  ...
  const selected = drawerId
    ? (promotions.find((p) => p.id === drawerId) ?? null)
    : null;
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "end") {
      ...
      const updated = history(
        ...
        confirm.item.active ? "프로모션 비활성화" : "프로모션 활성화",
      );
      setPromotions((prev) =>
        prev.map((p) => (p.id === updated.id ? updated : p)),
      );
      ...
      setPromotions((prev) =>
        prev.map((p) =>
          selectedIds.includes(p.id)
            ? history({ ...p, active: false }, "프로모션 비활성화 (일괄)")
            : p,
        ),
      );
  ...
  function rowMenuItems(
    ...
    if (status !== "종료")
      items.push({ label: "수정", click: () => openEditor(p) });
    ...
    if (status === "진행중")
      items.push({
        label: "종료",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "end", item: p }),
      });
    if (status === "진행 예정" || status === "비활성")
      items.push({
        label: p.active ? "비활성화" : "활성화",
        click: () => setConfirm({ kind: "toggleActive", item: p }),
      });
    if (p.appliedCount === 0)
      items.push({
        label: "삭제",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "delete", item: p }),
      });
    return items;
  }
  ...
  const rows: GridRow[] = filtered.map((p) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: p.stackPromotion,
        color: p.stackPromotion === "가능" ? "#059669" : "#a1a1aa",
        size: "11.5px",
        weight: 600,
      },
      ...
      {
        kind: "text",
        text: p.appliedCount > 0 ? p.appliedCount.toLocaleString("ko-KR") : "-",
        color: "#3f3f46",
        size: "12px",
        weight: 600,
        align: "right",
    ...
    <div className={styles.page} onClick={() => menuId && setMenuId(null)}>
        ...
        <div className={styles.headRow}>
          <div>
            ...
            <div className={styles.subtitle}>
              상품, 주문 및 회원을 대상으로 적용되는 할인 프로모션을 관리합니다.
            </div>
          </div>
          <button
            type="button"
          ...
          <div className={styles.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                value={applyUnitFilter}
                onChange={(e) =>
                  setApplyUnitFilter(e.target.value as ApplyUnit | "")
                }
              >
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                onChange={(e) =>
                  setDiscountMethodFilter(e.target.value as DiscountMethod | "")
                }
              >
        ...
        <div className={styles.resultRow}>
          ...
          <div className={styles.resultActions}>
            <ExcelDownloadButton
              type="button"
              data-grid-download
              onClick={() => toastBriefly("다운로드를 준비했습니다.")}
            />
      ...
      {selectedIds.length > 0 && (
        <div className={styles.bulkBar}>
          ...
          <button
            ...
            onClick={() => setConfirm({ kind: "bulkOwner" })}
          >
            담당자 변경
          </button>
          <button
            type="button"
            className={styles.bulkBtn}
            onClick={() => setConfirm({ kind: "bulkDeactivate" })}
          >
            비활성화
          </button>
          <button
            ...
            onClick={() =>
              toastBriefly(`${selectedIds.length}건을 다운로드했습니다.`)
            }
          >
            다운로드
          </button>
      ...
      <div className={styles.gridWrap}>
        <DataGrid
          ...
          emptyActionLabel={
            promotions.length === 0 ? "+ 프로모션 등록" : "초기화"
          }
          emptyActionClick={
            promotions.length === 0 ? () => openEditor("new") : resetFilters
          }
        />
      </div>
      ...
      {selected && (
        <PromotionDetailDrawer
          ...
          onToggleActive={() =>
            setConfirm({ kind: "toggleActive", item: selected })
          }
          ...
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogSummary}>
              <div className={styles.dialogSummaryRow}>
                <span>현재 적용</span>
                <span>
                  {confirm.item.appliedCount.toLocaleString("ko-KR")}건
                </span>
              </div>
              <div className={styles.dialogSummaryRow}>
                <span>총 할인</span>
                <span>
                  {confirm.item.appliedAmount.toLocaleString("ko-KR")}원
                </span>
              </div>
            </div>
            <div className={styles.dialogBody} style={{ marginBottom: 0 }}>
              종료 후 신규 주문에는 적용되지 않습니다.
            </div>
      ...
      {confirm?.kind === "delete" && (
        ...
        >
          <div className={styles.dialogBox}>
            ...
            >{`'${confirm.item.name}'이(가) 삭제됩니다. 적용 이력이 없는 프로모션만 삭제할 수 있습니다.`}</div>
        ...
        >
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogBody}>
              {confirm.item.active
                ? "비활성화하면 적용기간과 관계없이 즉시 적용이 중단됩니다."
                : "활성화하면 적용기간 조건에 따라 다시 적용될 수 있습니다."}
            </div>
            <div className={styles.dialogActions}>
              <button
                type="button"
      ...
      {confirm?.kind === "bulkDeactivate" && (
        ...
        >
          <div className={styles.dialogBox}>
            <div className={styles.dialogTitle}>
              {selectedIds.length}건 비활성화
            </div>
            <div className={styles.dialogBody}>
              선택한 프로모션을 모두 비활성화합니다.
            </div>
            <div className={styles.dialogActions}>
              <button
                type="button"
