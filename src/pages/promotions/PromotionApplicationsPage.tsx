...
export function PromotionApplicationsPage() {
  ...
  const [applications, setApplications] =
    useState<PromotionApplication[]>(APPLICATIONS);
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = applications.filter((a) => matchesQuickFilter(a, f)).length;
    });
    return c;
  }, [applications]);
  ...
  const filtered = useMemo(
    () =>
      applications.filter((a) => {
        ...
        if (
          search &&
          !`${a.orderId} ${a.member} ${a.promotionNameSnapshot} ${a.promotionCode}`
            .toLowerCase()
            .includes(search.toLowerCase())
        )
          return false;
        if (promotionFilter && a.promotionCode !== promotionFilter)
          return false;
  ...
  const rows: GridRow[] = filtered.map((a) => {
    ...
    const cells: Cell[] = [
      {
        kind: "text",
        text: a.appliedAt.replace(/^\d{4}-/, "").replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
      ...
      {
        kind: "titleWarn",
        title: `${a.promotionNameSnapshot} · ${a.id}`,
      ...
      },
      {
        kind: "text",
        text:
          a.currentDiscountAmount < 0
            ? fmtWon(a.currentDiscountAmount)
            : `-${fmtWon(a.currentDiscountAmount)}`,
        color: a.currentDiscountAmount < 0 ? "#dc2626" : "#18181b",
        size: "12.5px",
        weight: 700,
        align: "right",
        numeric: true,
        ...
        <div className={styles.filterBox}>
          ...
          >
            <input
              ...
              placeholder="주문번호, 회원, 프로모션명 또는 코드 검색"
            />
          ...
          <div className={styles.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as ApplyStatus | "")
                }
              >
        ...
        <DataGrid
          ...
          emptyText={
            applications.length === 0
              ? "프로모션 적용 이력이 없습니다."
              : quickFilter === "확인 필요"
                ? "현재 확인이 필요한 프로모션 적용 이력이 없습니다."
                : "검색 조건에 해당하는 프로모션 적용 이력이 없습니다."
          }
          emptySubtext={
            applications.length === 0
              ? "프로모션이 실제 주문에 적용되면 이곳에서 확인할 수 있습니다."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={applications.length > 0 ? "초기화" : undefined}
