...
export function FreeShippingConditionPage() {
  ...
  const save = (item: FreeShippingPolicy) => {
    if (isNew) {
      ...
      const saved =
        previous && previous.threshold !== item.threshold
          ? history(
              item,
              "무료배송 기준금액 변경",
              fmtWon(previous.threshold),
              fmtWon(item.threshold),
            )
          : history(item, "정책 수정");
      setPolicies((current) =>
      ...
      {view === "list" && (
        <div className={shared.gridWrap}>
          ...
          <DataGrid
            ...
            emptyText={
              quickFilter === "확인 필요"
                ? "현재 확인이 필요한 무료배송 조건이 없습니다."
                : "검색 결과가 없습니다."
            }
            emptySubtext={
              filtered.length === 0 && policies.length === 0
                ? "현재 모든 주문에는 기본 배송비 정책이 적용됩니다."
                : "검색어나 필터 조건을 변경해 주세요."
            }
        ...
        <div className={shared.gridWrap} style={{ marginTop: 0 }}>
          <div className={styles.previewGrid}>
              ...
              <div className={styles.orderPick}>
                {TEST_ORDERS.map((o) => (
                  <button
                    key={o.id}
                    type="button"
                    className={`${styles.orderOption} ${previewOrderId === o.id ? styles.orderOptionActive : ""}`}
                    onClick={() => setPreviewOrderId(o.id)}
                  >
                    <span>
                      <strong>{o.target}</strong> · {o.deliveryMethod}
                    </span>
            ...
            <div className={styles.previewCard}>
              ...
              <div
                className={`${styles.resultHero} ${previewResult.match.tie ? styles.resultHeroWarn : !previewResult.match.matched ? styles.resultHeroPaid : ""}`}
              >
                <span>
                  {previewOrder.target} · {previewOrder.deliveryMethod} ·{" "}
                  {fmtWon(previewOrder.productAmount)}
                </span>
                <strong>
              ...
              {!previewResult.match.matched &&
                previewResult.match.shortfall && (
                  <div className={styles.noteList}>
                    <div>
                      ℹ {fmtWon(previewResult.match.shortfall.amount)} 더
                      구매하면 '{previewResult.match.shortfall.policy.name}'
                      조건으로 무료배송이 적용됩니다.
                    </div>
                  </div>
                )}
              <div className={styles.resultRow}>
                <span>판정 기준금액</span>
                <strong>{fmtWon(previewResult.basisAmount)}</strong>
              </div>
              <div className={styles.resultRow}>
                <span>매칭 정책</span>
                <strong>{previewResult.match.matched?.name ?? "없음"}</strong>
              </div>
              {previewResult.match.matched && (
                <div className={styles.resultRow}>
                  <span>면제 범위</span>
                  <strong>{previewResult.match.matched.exemptionScope}</strong>
                </div>
              )}
            </div>
          </div>
