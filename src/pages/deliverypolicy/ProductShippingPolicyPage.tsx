...
function history(
  ...
  return {
    ...
    history: [
      ...item.history,
      {
        id: `H-${item.productCode}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        at: `${TODAY} 14:00`,
        by: "admin01",
        action,
        before,
...
export function ProductShippingPolicyPage() {
  ...
  const filtered = useMemo(
    () =>
      PRODUCTS.filter((product) => {
        ...
        if (!matchesQuickFilter(product, override, quickFilter, warnings))
          return false;
        if (
          search &&
          ...
          return false;
        if (methodFilter && override.deliveryMethod !== methodFilter)
          return false;
        return true;
      }),
  ...
  const runBulkApply = () => {
    ...
    setOverrides((current) => {
      ...
      selected.forEach((code) => {
        ...
        next[code] = history(
          { ...source, productCode: code, history: next[code].history },
          `정책 일괄 적용 (기준: ${PRODUCTS.find((p) => p.code === bulkSource)?.name})`,
        );
      });
      return next;
    });
  ...
  const rows: GridRow[] = filtered.map((product) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: override.usesOverride ? "별도 정책" : "기본 정책",
          bg: override.usesOverride ? "#eef2ff" : "#f4f4f5",
          fg: override.usesOverride ? "#4338ca" : "#71717a",
        },
        {
          kind: "text",
          text: fmtFee(override),
          size: "12px",
          weight: 600,
          color: "#18181b",
        },
        {
          kind: "text",
          text: override.usesOverride ? override.deliveryMethod : "-",
          size: "12px",
          color: "#3f3f46",
        },
        {
          kind: "statusDot",
          text: !override.usesOverride
            ? "기본"
            : override.active
              ? "적용중"
              : "비활성",
          dot: !override.usesOverride
            ? "#a1a1aa"
            : override.active
              ? "#10b981"
              : "#d4d4d8",
          fg: !override.usesOverride
            ? "#71717a"
            : override.active
              ? "#047857"
              : "#a1a1aa",
  ...
  const drawerProduct = drawerCode
    ? (PRODUCTS.find((p) => p.code === drawerCode) ?? null)
    : null;
    ...
    <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
        ...
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              기본 배송 정책과 다르게 적용해야 하는 상품의 배송비·배송 조건을
              개별적으로 설정합니다.
            </div>
          </div>
        </div>
            ...
            {selected.size > 0 && (
              <>
                <button
                  ...
                  onClick={() => setBulkModal("apply")}
                >
                  정책 일괄 적용 ({selected.size})
                </button>
                <button
                  type="button"
                  className={styles.bulkBtn}
                  onClick={() => setBulkModal("reset")}
                >
                  기본 정책으로 변경 ({selected.size})
                </button>
              </>
            )}
      ...
      <div className={shared.gridWrap}>
        ...
        <DataGrid
          ...
          emptyText={
            quickFilter === "설정 확인 필요"
              ? "현재 확인이 필요한 상품 정책이 없습니다."
              : "검색 결과가 없습니다."
          }
            ...
            {bulkModal === "apply" ? (
              <>
                <p className={shared.dialogBody}>
                  선택한 {selected.size}개 상품에 기준 상품의 배송 정책을 그대로
                  복사해서 적용합니다.
                </p>
                <label className={styles.formField}>
                  ...
                  >
                    ...
                    {PRODUCTS.filter((p) => selected.has(p.code)).map((p) => (
              ...
              <p className={shared.dialogBody}>
                선택한 {selected.size}개 상품의 배송 정책을 기본 배송 정책
                사용으로 초기화합니다. 개별 설정 값은 사라집니다.
              </p>
            )}
