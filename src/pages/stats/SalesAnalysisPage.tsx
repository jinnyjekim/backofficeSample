    ...
    <section className={`${shared.page} ${styles.page}`}>
      <div className={shared.headTop}>
        ...
        <div className={styles.filterCard}>
          <div className={styles.filterGrid}>
            <label className={styles.filterField}><span>기간</span><CommonSelect className={styles.analysisSelect} size="sm" value={range} options={['오늘', '최근 7일', '최근 30일', '이번 달', '지난 달', '이번 분기', '직접 설정'].map((value) => ({ label: value, value }))} onChange={(value) => pickRange(String(value))} /></label>
            ...
            <label className={styles.filterField}><span>비교</span><CommonSelect className={styles.analysisSelect} size="sm" value={compare} options={['비교 없음', '이전 기간'].map((value) => ({ label: value, value }))} onChange={(value) => setCompare(String(value))} /></label>
            <label className={styles.filterField}><span>매출 기준</span><CommonSelect className={styles.analysisSelect} size="sm" value={salesBasis} options={['주문 기준', '결제 완료 기준', '거래 완료 기준', '매출 확정 기준'].map((value) => ({ label: value, value }))} onChange={(value) => setSalesBasis(String(value))} /></label>
            <label className={styles.filterField}><span>날짜 기준</span><CommonSelect className={styles.analysisSelect} size="sm" value={dateBasis} options={(mode === 'b2b' ? ['주문일', '납품 완료일', '매출 확정일'] : mode === 'c2c' ? ['결제일', '거래 완료일', '정산 확정일'] : ['주문일', '결제일', '배송 완료일', '매출 확정일']).map((value) => ({ label: value, value }))} onChange={(value) => setDateBasis(String(value))} /></label>
            <label className={styles.filterField}><span>금액 기준</span><CommonSelect className={styles.analysisSelect} size="sm" value={amountBasis} options={['세금 포함', '공급가액'].map((value) => ({ label: value, value }))} onChange={(value) => setAmountBasis(String(value))} /></label>
          ...
          {advanced && <div className={styles.advancedFilters}><label className={styles.filterField}><span>거래 상태</span><CommonSelect className={styles.analysisSelect} size="sm" value={tradeState} options={['정상 거래', '취소·환불 포함', '완료 거래만'].map((value) => ({ label: value, value }))} onChange={(value) => setTradeState(String(value))} /></label><label className={styles.filterField}><span>판매 채널</span><CommonSelect className={styles.analysisSelect} size="sm" value={channel} options={['전체 채널', '모바일 앱', '웹', '파트너 포털'].map((value) => ({ label: value, value }))} onChange={(value) => setChannel(String(value))} /></label><div className={styles.filterDescription}>상세 조건은 KPI, 추이, 구성 및 상세 표에 동일하게 적용됩니다.</div></div>}
      ...
      <div className={styles.body}>
        ...
        <section className={styles.card}>
          ...
          <div className={styles.chartToolbar}><label><span>지표</span><CommonSelect className={styles.analysisSelect} size="sm" value={chartMetric} options={chartOptions.map((item) => ({ label: item.label, value: item.key }))} onChange={(value) => setChartMetric(value as ChartMetric)} /></label><div className={styles.segmented}>{(['일별', '주별', '월별'] as Granularity[]).map((item) => <button key={item} type="button" className={granularity === item ? styles.segmentedActive : ''} onClick={() => setGranularity(item)}>{item}</button>)}</div></div>
