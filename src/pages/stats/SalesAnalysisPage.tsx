...
function metricsFor(
  ...
  const part = (business: Exclude<Mode, "all">): SalesMetrics => {
    if (business === "b2c") {
      ...
      return {
        volume: round(volume),
        revenue: round(volume - discount - cancelRefund),
        count: round(count),
        average: round(volume / Math.max(count, 1)),
        cancelRefund: round(cancelRefund),
    ...
    if (business === "c2c") {
      ...
      return {
        volume: round(volume),
        revenue: round(platformRevenue),
        count: round(count),
        average: round(volume / Math.max(count, 1)),
        cancelRefund: round(base.refundAmount * 0.21),
        discount: 0,
      };
    }
    ...
    return {
      volume: round(volume),
      revenue: round(volume - cancelRefund - adjustment),
      count: round(count),
      average: round(volume / Math.max(count, 1)),
      cancelRefund: round(cancelRefund),
      discount: round(adjustment),
    };
  };
...
function TrendChart({
  ...
  const point = (value: number, index: number, length: number) => {
    const x =
      length <= 1
        ? width / 2
        : padX + (index / (length - 1)) * (width - padX * 2);
  ...
  return (
    <div className={styles.trendChart}>
      ...
      >
        ...
        {previousPoints && (
          <polyline points={previousPoints} className={styles.previousLine} />
        )}
        <polyline points={currentPoints} className={styles.currentLine} />
        {current.map((value, index) => (
          <circle
            key={`${labels[index]}-${value}`}
            cx={point(value, index, current.length).split(",")[0]}
            cy={point(value, index, current.length).split(",")[1]}
            r="3"
...
function buildRows(
  ...
  allBase: PeriodAggregate,
): AnalysisRow[] {
  if (mode === "all" && dimension === "비즈니스별") {
    const unscaledTotal = metricsFor(allBase, "all");
    const appliedFactor = unscaledTotal.volume
      ? metrics.volume / unscaledTotal.volume
      : 1;
...
export function SalesAnalysisPage() {
  ...
  const metrics = useMemo(
    () => scaleMetrics(metricsFor(base, mode), basisFactor),
    [base, mode, basisFactor],
  );
  const previousMetrics = useMemo(
    () => scaleMetrics(metricsFor(previousBase, mode), basisFactor),
    [previousBase, mode, basisFactor],
  );
  const analysisRows = useMemo(
    () => buildRows(mode, dimension, metrics, base),
    [mode, dimension, metrics, base],
  );

  const currentBuckets = useMemo(
    () => bucketSeries(start, end, granularity),
  ...
  const previousBuckets = useMemo(
    () =>
      compare === "비교 없음"
        ? []
        : bucketSeries(prevStart, prevEnd, granularity),
    [compare, prevStart, prevEnd, granularity],
  );
  const chartCurrent = currentBuckets.map((bucket) =>
    metricValue(
      scaleMetrics(chartMetrics(bucket, mode), basisFactor),
      chartMetric,
    ),
  );
  const chartPrevious = previousBuckets.map((bucket) =>
    metricValue(
      scaleMetrics(chartMetrics(bucket, mode), basisFactor),
      chartMetric,
    ),
  );
  const highestIndex = chartCurrent.reduce(
    (best, value, index) =>
      value > (chartCurrent[best] ?? -Infinity) ? index : best,
    0,
  );
  const lowestIndex = chartCurrent.reduce(
    (best, value, index) =>
      value < (chartCurrent[best] ?? Infinity) ? index : best,
    0,
  );
  const formatChartValue = (value: number) =>
    chartMetric === "count"
      ? `${round(value).toLocaleString("ko-KR")}건`
      : fmtWon(value);
  ...
  function pickRange(value: string) {
    ...
    const mapped =
      value === "이번 분기"
        ? ["2026-07-01", TODAY]
        : quickRangeDates(value as QuickRange);
  ...
  function resetFilters() {
    setStart("2026-08-01");
    setEnd(TODAY);
    setDraftStart("2026-08-01");
    setDraftEnd(TODAY);
    setRange("이번 달");
    setCompare("이전 기간");
    setSalesBasis("결제 완료 기준");
    setDateBasis(
      mode === "b2b"
        ? "매출 확정일"
        : mode === "c2c"
  ...
  const kpis = [
    {
      key: "volume" as const,
      label: modeLabels.volume,
      value: fmtWon(metrics.volume),
      previous: previousMetrics.volume,
      definition:
        mode === "all"
          ? "B2C 판매액 + C2C 거래액(GMV) + B2B 주문금액"
          : "선택한 집계 기준의 전체 거래 발생 금액",
    },
    {
      key: "revenue" as const,
      label: modeLabels.revenue,
      value: fmtWon(metrics.revenue),
      previous: previousMetrics.revenue,
      definition:
        mode === "c2c"
          ? "거래액이 아닌 플랫폼 수수료 등 실제 귀속 매출"
          : "취소·환불·할인 및 조정을 반영한 통계 기준 매출",
    },
    {
      key: "count" as const,
      label: modeLabels.count,
      value: `${metrics.count.toLocaleString("ko-KR")}건`,
      previous: previousMetrics.count,
      definition: "선택한 날짜 및 상태 기준에 포함된 유효 거래 건수",
    },
    {
      key: "average" as const,
      label: modeLabels.average,
      value: fmtWon(metrics.average),
      previous: previousMetrics.average,
      definition: `${modeLabels.volume} ÷ ${modeLabels.count} 건수`,
    },
  ];
  ...
  const columns: GridColumn[] =
    mode === "b2c"
      ? [
          ...
          { label: "판매수량", align: "right" },
          { label: "총매출", align: "right" },
          { label: "할인", align: "right" },
          { label: "취소·환불", align: "right" },
      ...
      : mode === "c2c"
        ? [
            ...
            { label: "거래액", align: "right" },
            { label: "플랫폼 매출", align: "right" },
            { label: "취소·환불", align: "right" },
            { label: "판매자 정산대상", align: "right" },
            { label: "비중", align: "right" },
            { label: "전기 대비", align: "right" },
          ]
        : mode === "b2b"
          ? [
              { label: dimension.replace("별", "") },
              { label: "주문", align: "right" },
              { label: "주문금액", align: "right" },
              { label: "취소·조정", align: "right" },
              { label: "확정 매출", align: "right" },
          ...
          : [
              { label: dimension.replace("별", "") },
              { label: "거래·주문", align: "right" },
              { label: "사업 거래규모", align: "right" },
              { label: "플랫폼/서비스 매출", align: "right" },
              { label: "취소·환불", align: "right" },
              { label: "비중", align: "right" },
              { label: "전기 대비", align: "right" },
            ];
  ...
  const gridRows: GridRow[] = analysisRows.map((row) => {
    ...
    const commonEnd: Cell[] = [
      {
        kind: "text",
        text: `${row.share.toFixed(1)}%`,
        align: "right",
        numeric: true,
      },
      {
        kind: "text",
        text: `${row.change >= 0 ? "▲" : "▼"} ${Math.abs(row.change).toFixed(1)}%`,
        align: "right",
        color: changeColor,
        weight: 600,
        numeric: true,
        ...
        ? [
            {
              kind: "stack",
              title: row.name,
              subtitle: `${dimension} 매출 상세`,
            },
            {
              kind: "text",
              text: row.count.toLocaleString("ko-KR"),
              align: "right",
              numeric: true,
            },
            {
              kind: "text",
              text: row.quantity.toLocaleString("ko-KR"),
              align: "right",
              numeric: true,
            },
            {
              kind: "text",
              text: fmtWon(row.gross),
              align: "right",
              numeric: true,
            },
            {
              kind: "text",
              text: fmtWon(row.discount),
              align: "right",
              color: "#71717a",
              numeric: true,
            },
        ...
        : mode === "c2c"
          ...
          : mode === "b2b"
            ? [
                ...
                {
                  kind: "text",
                  text: fmtWon(row.refund + row.discount),
                  align: "right",
                  color: "#b45309",
                  numeric: true,
                },
    ...
    return {
      ...
      bg: selectedRow?.id === row.id ? "#f7f8ff" : undefined,
    };
  });
  ...
  const factors =
    mode === "b2c"
      ? ([
          ["프리미엄 무선 헤드폰", metrics.revenue * 0.057],
          ["신규 구매 증가", metrics.revenue * 0.032],
          ["환불 증가", -metrics.revenue * 0.014],
        ] as const)
      : mode === "c2c"
        ? ([
            ["디지털 기기 거래 증가", metrics.revenue * 0.061],
            ["안전결제 이용 증가", metrics.revenue * 0.028],
            ["거래 취소 증가", -metrics.revenue * 0.019],
          ] as const)
        : mode === "b2b"
          ? ([
              ["회사 01 주문 증가", metrics.revenue * 0.052],
              ["신규 계약 매출", metrics.revenue * 0.034],
              ["납품 조정 증가", -metrics.revenue * 0.012],
            ] as const)
          : ([
              ["B2C 상품 판매 증가", metrics.revenue * 0.041],
              ["B2B 회사 01 주문", metrics.revenue * 0.029],
              ["C2C 환불 증가", -metrics.revenue * 0.011],
            ] as const);
  ...
  const exportDefinitions = [
    {
      key: "target",
      label: dimension.replace("별", ""),
      value: (row: AnalysisRow) => row.name,
    },
    {
      key: "count",
      label: modeLabels.count,
      value: (row: AnalysisRow) => row.count,
    },
    {
      key: "gross",
      label: modeLabels.volume,
      value: (row: AnalysisRow) => row.gross,
    },
    {
      key: "discount",
      label: mode === "b2b" ? "조정" : "할인",
      value: (row: AnalysisRow) => row.discount,
    },
    {
      key: "refund",
      label: "취소·환불",
      value: (row: AnalysisRow) => row.refund,
    },
    {
      key: "net",
      label: modeLabels.revenue,
      value: (row: AnalysisRow) => row.net,
    },
    {
      key: "average",
      label: modeLabels.average,
      value: (row: AnalysisRow) => row.average,
    },
    {
      key: "share",
      label: "비중(%)",
      value: (row: AnalysisRow) => row.share.toFixed(1),
    },
    {
      key: "change",
      label: "전기 대비(%)",
      value: (row: AnalysisRow) => row.change.toFixed(1),
    },
  ];

  function runDownload() {
    const fields = exportDefinitions.filter((field) =>
      downloadFields.has(field.key),
    );
    const dimensionSheet = (name: string) => ({
      name,
      headers: fields.map((field) => field.label),
      rows: buildRows(mode, name, metrics, base).map((row) =>
        fields.map((field) => field.value(row)),
      ),
    });
    ...
    const metricRow = (label: string, current: number, previous: number) => {
      const change =
        compare === "비교 없음" ? undefined : delta(current, previous);
      return {
        label,
        current,
        previous: compare === "비교 없음" ? undefined : previous,
        change: change?.abs,
        changeRate: change ? `${change.pct.toFixed(1)}%` : undefined,
      };
    };
    downloadStatisticsReport({
      reportName: "매출 분석",
      mode: mode === "all" ? "통합" : mode.toUpperCase(),
      period: `${start}~${end}`,
      comparisonPeriod:
        compare === "비교 없음" ? undefined : `${prevStart}~${prevEnd}`,
      filters: [
        ["매출 기준", salesBasis],
        ["날짜 기준", dateBasis],
        ["금액 기준", amountBasis],
      ...
      summary: [
        metricRow(modeLabels.volume, metrics.volume, previousMetrics.volume),
        metricRow(modeLabels.revenue, metrics.revenue, previousMetrics.revenue),
        metricRow(modeLabels.count, metrics.count, previousMetrics.count),
        metricRow(modeLabels.average, metrics.average, previousMetrics.average),
        metricRow(
          "취소·환불액",
          metrics.cancelRefund,
          previousMetrics.cancelRefund,
        ),
      ],
      trend: {
        name: "02_매출추이",
      ...
      dimensions: [
        ...MODE_DIMENSIONS[mode].map(dimensionSheet),
        {
          name: "증감요인",
          headers: ["요인", "증감 금액"],
          rows: factors.map(([label, value]) => [label, Math.round(value)]),
        },
      ],
      definitions: [
        {
          term: modeLabels.revenue,
          description:
            mode === "c2c"
              ? "거래액이 아닌 플랫폼 수수료 등 실제 귀속 매출"
              : "취소·환불·할인 및 조정을 반영한 통계 기준 매출",
        ...
        {
          term: modeLabels.volume,
          description:
            mode === "all"
              ? "B2C 판매액, C2C 거래액, B2B 주문금액의 합계"
              : "선택한 집계 기준의 전체 거래 발생 금액",
        },
        {
          term: modeLabels.average,
          description: `${modeLabels.volume}을 ${modeLabels.count} 건수로 나눈 금액`,
        },
        {
          term: "취소·환불액",
          description: "조회 기간에 취소 또는 환불로 차감된 금액",
        },
      ],
    ...
    <section className={`${shared.page} ${styles.page}`}>
      <div className={shared.headTop}>
        <div className={shared.headRow}>
          <div>
            ...
            <p className={shared.subtitle}>
              B2C 판매 매출, C2C 거래액·플랫폼 매출, B2B 주문·확정 매출을 구분해
              비교합니다.
            </p>
          </div>
        ...
        <div className={styles.modeSwitch} aria-label="매출 분석 비즈니스 모드">
          {MODES.map((item) => (
            <button
              key={item.key}
              type="button"
              className={mode === item.key ? styles.modeActive : ""}
        ...
        {showBasis && (
          <div className={styles.basisPanel}>
            <Info size={16} />
            <div>
              <strong>통계 기준 매출</strong>
              <p>
                {mode === "c2c"
                  ? "C2C 거래액은 구매자와 판매자 사이의 GMV이며, 플랫폼 매출에는 수수료 등 실제 귀속 금액만 포함합니다."
                  : mode === "all"
                    ? "사업 거래규모에는 B2C 판매액, C2C GMV, B2B 주문금액이 포함됩니다. 플랫폼/서비스 매출에는 실제 귀속 금액만 합산합니다."
                    : `${mode.toUpperCase()} ${modeLabels.revenue}은(는) 취소·환불 및 할인·조정을 반영한 운영 통계 기준이며 회계 매출과 다를 수 있습니다.`}
              </p>
            </div>
            <button
              type="button"
      ...
      <div className={styles.body}>
        <div className={styles.kpiGrid}>
          {kpis.map((item) => {
            const change =
              compare === "비교 없음"
                ? null
                : delta(metricValue(metrics, item.key), item.previous);
            return (
              <article key={item.key} className={styles.kpiCard}>
                ...
                {change ? (
                  <div
                    className={
                      change.pct >= 0 ? styles.changeUp : styles.changeDown
                    }
                  >
                    {change.pct >= 0 ? (
                      <TrendingUp size={13} />
                    ...
                    <span>
                      · 이전{" "}
                      {item.key === "count"
                        ? `${round(item.previous).toLocaleString("ko-KR")}건`
                        : fmtWon(item.previous)}
                    </span>
                  </div>
                ) : (
                  <div className={styles.noCompare}>비교 기간 없음</div>
                )}
              </article>
            );
          })}
        ...
        <div className={styles.secondaryMetrics}>
          <div>
            <span>취소·환불액</span>
            <strong>{fmtWon(metrics.cancelRefund)}</strong>
            <em>
              {metrics.volume
                ? ((metrics.cancelRefund / metrics.volume) * 100).toFixed(1)
                : "0.0"}
              %
            </em>
          </div>
            ...
            <strong>
              {mode === "c2c"
                ? fmtWon(
                    Math.max(
                      0,
                      metrics.volume - metrics.revenue - metrics.cancelRefund,
                    ),
                  )
                : fmtWon(metrics.discount)}
            </strong>
            <em>{amountBasis}</em>
          </div>
          <div>
        ...
        <section className={styles.card}>
          <div className={styles.cardHead}>
            ...
            <div className={styles.legend}>
              ...
              {compare !== "비교 없음" && (
                <span>
                  <i className={styles.legendPrevious} />
                  비교 기간
                </span>
              )}
            </div>
          ...
          <div className={styles.chartToolbar}>
            ...
            <div className={styles.segmented}>
              {(["일별", "주별", "월별"] as Granularity[]).map((item) => (
                <button
                  key={item}
                  type="button"
                  className={granularity === item ? styles.segmentedActive : ""}
          ...
          {chartCurrent.length ? (
            <TrendChart
              ...
              previous={compare === "비교 없음" ? undefined : chartPrevious}
              labels={currentBuckets.map((bucket) => bucket.label)}
              formatCount={chartMetric === "count"}
            />
          ) : (
            <div className={styles.emptyState}>
              <strong>해당 기간의 매출 데이터가 없습니다.</strong>
              <span>기간 또는 필터 조건을 변경해 주세요.</span>
          ...
          <div className={styles.chartSummary}>
            <div>
              <span>기간 {modeLabels.revenue}</span>
              <strong>{fmtWon(metrics.revenue)}</strong>
              <em
                className={
                  revenueDelta.pct >= 0 ? styles.changeUp : styles.changeDown
                }
              >
                {signedWon(revenueDelta.abs)} /{" "}
                {revenueDelta.pct >= 0 ? "+" : ""}
                {revenueDelta.pct.toFixed(1)}%
              </em>
            </div>
            <div>
              <span>최고 구간</span>
              <strong>{currentBuckets[highestIndex]?.label ?? "-"}</strong>
              <em>{formatChartValue(chartCurrent[highestIndex] ?? 0)}</em>
            </div>
            <div>
              <span>최저 구간</span>
              <strong>{currentBuckets[lowestIndex]?.label ?? "-"}</strong>
              <em>{formatChartValue(chartCurrent[lowestIndex] ?? 0)}</em>
            </div>
          </div>
        </section>
        ...
        <div className={styles.insightGrid}>
          <section className={styles.card}>
            <div className={styles.cardHead}>
              <div>
                <h2>
                  {mode === "all"
                    ? "통합 매출 구성"
                    : `${modeLabels.volume} 구성`}
                </h2>
                <p>
                  {mode === "all"
                    ? "거래규모와 실제 귀속 매출을 함께 표시합니다."
                    : `${dimension} 상위 항목`}
                </p>
              </div>
            </div>
            <div className={styles.compositionList}>
              {compositionRows.map((row) => (
                <div key={row.id} className={styles.compositionItem}>
                  <div>
                    <strong>{row.name}</strong>
                    <span>
                      {fmtWon(row.gross)}
                      {mode === "all" && ` · 귀속 ${fmtWon(row.net)}`}
                    </span>
                  </div>
          ...
          <section className={styles.card}>
            <div className={styles.cardHead}>
              <div>
                <h2>증감 요인</h2>
                <p>이전 기간 대비 {signedWon(revenueDelta.abs)}</p>
              </div>
            </div>
            <div className={styles.factorList}>
              {factors.map(([label, value]) => (
                <div key={label}>
                  <span
                    className={
                      value >= 0 ? styles.factorPositive : styles.factorNegative
                    }
                  >
                    {value >= 0 ? (
                      <TrendingUp size={14} />
                  ...
                  <strong>{label}</strong>
                  <em
                    className={value >= 0 ? styles.changeUp : styles.changeDown}
                  >
                    {signedWon(value)}
                  </em>
                </div>
            ...
            <p className={styles.factorNote}>
              금액 차이가 큰 항목을 단순 Difference Ranking으로 표시합니다.
            </p>
          </section>
        </div>
        ...
        <section className={styles.card}>
          <div className={styles.analysisHead}>
            <div>
              <h2>상세 분석</h2>
              <p>
                행을 클릭하면 해당 항목의 구성과 실제 거래 이동 경로를 확인할 수
                있습니다.
              </p>
            </div>
            <button
          ...
          <div className={styles.dimensionTabs}>
            {MODE_DIMENSIONS[mode].map((item) => (
              <button
                key={item}
                type="button"
                className={dimension === item ? styles.dimensionActive : ""}
          ...
          <DataGrid
            ...
            minWidth={mode === "b2c" ? "880px" : "840px"}
            empty={!analysisRows.length}
            emptyText="해당 조건에 집계된 매출 데이터가 없습니다."
            emptySubtext="기간 또는 분석 기준을 변경해 주세요."
          />
        </section>
      </div>
      ...
      {selectedRow && (
        ...
        >
          <div className={styles.drawerHead}>
            <div>
              <span>
                {mode === "all" ? "통합" : mode.toUpperCase()} · {dimension}
              </span>
          ...
          <div className={styles.drawerBody}>
            <div className={styles.drawerHero}>
              <span>{modeLabels.revenue}</span>
              <strong>{fmtWon(selectedRow.net)}</strong>
              <em
                className={
                  selectedRow.change >= 0 ? styles.changeUp : styles.changeDown
                }
              >
                {selectedRow.change >= 0 ? "▲" : "▼"}{" "}
                {Math.abs(selectedRow.change).toFixed(1)}%
              </em>
            </div>
            <div className={styles.drawerFacts}>
              <div>
                <span>{modeLabels.count}</span>
                <strong>{selectedRow.count.toLocaleString("ko-KR")}건</strong>
              </div>
              <div>
                <span>{modeLabels.volume}</span>
                <strong>{fmtWon(selectedRow.gross)}</strong>
              </div>
              <div>
                <span>취소·환불</span>
                <strong>{fmtWon(selectedRow.refund)}</strong>
              </div>
              <div>
                <span>{modeLabels.average}</span>
                <strong>{fmtWon(selectedRow.average)}</strong>
              </div>
              <div>
                <span>매출 비중</span>
                <strong>{selectedRow.share.toFixed(1)}%</strong>
              </div>
              <div>
                <span>비교 기간 추정</span>
                <strong>
                  {fmtWon(selectedRow.net / (1 + selectedRow.change / 100))}
                </strong>
              </div>
            </div>
            <div className={styles.drawerSection}>
              <h3>집계 조건</h3>
              <p>
                {fmtDate(start)} ~ {fmtDate(end)} · {salesBasis} · {dateBasis} ·{" "}
                {amountBasis}
              </p>
            </div>
            <div className={styles.drawerSection}>
              <h3>금액 구성</h3>
              <div className={styles.amountFlow}>
                <span>
                  거래 발생 <b>{fmtWon(selectedRow.gross)}</b>
                </span>
                <span>
                  취소·환불 <b>-{fmtWon(selectedRow.refund)}</b>
                </span>
                <span>
                  할인·조정 <b>-{fmtWon(selectedRow.discount)}</b>
                </span>
                <span>
                  유효/귀속 매출 <b>{fmtWon(selectedRow.net)}</b>
                </span>
              </div>
            </div>
          </div>
        ...
        >
          <div className={styles.dialog}>
            ...
            <div className={styles.downloadSummary}>
              ...
              <span>
                매출 기준 <b>{salesBasis}</b>
              </span>
              <span>
                금액 기준 <b>{amountBasis}</b>
              </span>
            </div>
            <StatisticsDownloadFields
              className={styles.downloadFields}
              fields={exportDefinitions}
