  ...
  return (
    <section className={shared.page}>
      ...
      <StatisticsFilterToolbar
        ...
        onApply={applyCustom}
        summary={<>조회 기간 <b>{fmtDate(start)} ~ {fmtDate(end)}</b> ({agg.days}일){compare && <> · 비교 기간 <b>{fmtDate(prevStart)} ~ {fmtDate(prevEnd)}</b> ({prevAgg.days}일)</>} · 전환 목표 <b>{goal}</b></>}
      />
