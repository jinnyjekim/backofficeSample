...
function TrendChart({
  ...
  return <StatisticsLineChart values={primary} labels={labels} height={240} ariaLabel="판매대금 추이 차트" />;
...
export function SellerProceedsStatsPage() {
  ...
  const settlementGrowth = prevAgg.settlementFinal
    ? delta(agg.settlementFinal, prevAgg.settlementFinal).pct
    : 0;
  const feeGrowth = prevAgg.fee ? delta(agg.fee, prevAgg.fee).pct : 0;
  const payoutGrowth = prevAgg.payoutDone
    ? delta(agg.payoutDone, prevAgg.payoutDone).pct
    : 0;
  ...
  const kpis = [
    ...
    },
    {
      key: "settlement",
      label: "정산 대상금액",
      value: fmtWon(agg.settlementTarget),
      change: settlementGrowth,
      sub: "전기 대비",
    },
    {
      key: "fee",
      label: "플랫폼 수수료",
      value: fmtWon(agg.fee),
      change: feeGrowth,
      sub: `전기 대비 · 수수료율 ${fmtPct(agg.feeRate)}`,
    },
    {
      key: "payout",
      label: "출금 완료금액",
      value: fmtWon(agg.payoutDone),
      change: payoutGrowth,
      sub: "전기 대비",
    },
  ];
  ...
  const gridRows: GridRow[] = rows.map((row) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: fmtWon(row.settlementFinal),
        align: "right",
        numeric: true,
      },
      {
  ...
  const exportFields = [
    ...
    },
    {
      key: "dealCount",
      label: "거래건수",
      value: (row: DimensionRow) => row.dealCount,
    },
    ...
    { key: "fee", label: "수수료", value: (row: DimensionRow) => row.fee },
    {
      key: "settlementFinal",
      label: "정산금액",
      value: (row: DimensionRow) => row.settlementFinal,
    },
    {
      key: "payoutDone",
      label: "지급 완료",
      value: (row: DimensionRow) => row.payoutDone,
    },
    {
      key: "change",
      label: "전기 대비",
      value: (row: DimensionRow) =>
        row.prevGmv ? fmtSignedPct(delta(row.gmv, row.prevGmv).pct) : "-",
    },
  ];
  ...
  function download() {
    ...
    downloadStatisticsReport({
      ...
      summary: [
        {
          label: "판매대금",
          current: agg.gmv,
          previous: prevAgg.gmv,
          change: agg.gmv - prevAgg.gmv,
          changeRate: `${gmvGrowth.toFixed(1)}%`,
        },
        {
          label: "정산 대상금액",
          current: agg.settlementTarget,
          previous: prevAgg.settlementTarget,
          changeRate: `${settlementGrowth.toFixed(1)}%`,
        },
        {
          label: "플랫폼 수수료",
          current: agg.fee,
          previous: prevAgg.fee,
          changeRate: `${feeGrowth.toFixed(1)}%`,
        },
        {
          label: "출금 완료금액",
          current: agg.payoutDone,
          previous: prevAgg.payoutDone,
          changeRate: `${payoutGrowth.toFixed(1)}%`,
        },
        {
          label: "출금 예정금액",
          current: agg.payoutPending,
          previous: prevAgg.payoutPending,
        },
      ],
      trend: {
        name: "02_판매대금추이",
        headers: ["일자", "판매대금", "정산 확정금액", "지급 완료금액"],
      ...
      dimensions: [
        ...
        dimensionSheet("정산상태별", statusTabRows),
      ...
      definitions: [
        {
          term: "판매대금",
          description: "조회기간 완료된 거래의 총 판매금액(GMV)",
        },
        {
          term: "정산 대상금액",
          description: "판매대금에서 환불 등을 제외하고 정산 대상이 되는 금액",
        },
        {
          term: "플랫폼 수수료",
          description: "정산 대상금액에 수수료율을 적용해 차감하는 금액",
        },
        {
          term: "출금 완료금액",
          description: "판매자에게 실제로 지급이 완료된 금액",
        },
      ],
    ...
    <section className={`${shared.page} ${layout.page}`}>
      <div className={shared.headTop}>
        <div className={shared.headRow}>
          <div>
            <h1 className={shared.title}>판매대금 통계</h1>
            <p className={shared.subtitle}>
              판매자 판매대금이 얼마나 발생하고, 수수료 차감 후 정산·출금이
              어떻게 이뤄지는지 분석합니다.
            </p>
          </div>
        ...
        {showBasis && (
          <div className={layout.basisPanel}>
            <Info size={16} />
            <div>
              <strong>판매대금 통계 집계 기준</strong>
              <p>
                판매대금은 조회기간 완료된 거래의 총 판매금액(GMV)입니다. 정산
                대상금액은 판매대금에서 환불분을 제외한 금액이며, 플랫폼
                수수료를 차감한 나머지가 정산 확정금액입니다. 출금 완료금액은
                판매자에게 실제 지급이 끝난 금액이고, 나머지는 출금 예정으로
                분류합니다.
              </p>
            </div>
            <button
      ...
      <div className={layout.body}>
        ...
        <div className={layout.secondaryMetrics}>
          <div>
            <span>정산 확정금액</span>
            <strong>{fmtWon(agg.settlementFinal)}</strong>
            <em>수수료 차감 후</em>
          </div>
          <div>
            <span>출금 완료금액</span>
            <strong>{fmtWon(agg.payoutDone)}</strong>
            <em className={layout.changeUp}>정산 지급 완료</em>
          </div>
          <div>
            <span>출금 예정금액</span>
            <strong>{fmtWon(agg.payoutPending)}</strong>
            <em>아직 지급되지 않음</em>
          </div>
          <div>
            <span>평균 거래단가</span>
            <strong>{fmtWon(agg.avgDealValue)}</strong>
            <em>판매대금 ÷ 거래건수</em>
          </div>
        </div>
        ...
        <section className={layout.card}>
          ...
          {primaryTrend.length ? (
            ...
            <div className={layout.emptyState}>
              <strong>분석할 판매대금 데이터가 없습니다.</strong>
        ...
        <section className={layout.card}>
          <div className={layout.analysisHead}>
            <div>
              <h2>상세 분석</h2>
              <p>
                분석 결과에서 바로 처리하지 않고 판매대금 관리 화면으로
                이동합니다.
              </p>
            </div>
            <button
          ...
          <DataGrid
            columns={columns}
            rows={gridRows}
            gridTemplate="minmax(190px,1.5fr) 70px 114px 90px 98px 74px"
            minWidth="740px"
            empty={!rows.length}
            emptyText="현재 조건에 해당하는 판매대금 데이터가 없습니다."
        ...
        >
          ...
          <div className={layout.drawerBody}>
            ...
            <div className={layout.drawerFacts}>
              <div>
                <span>거래건수</span>
                <strong>{fmtCount(selected.dealCount)}</strong>
              </div>
              <div>
                <span>수수료</span>
                <strong>{fmtWon(selected.fee)}</strong>
              </div>
              <div>
                <span>정산금액</span>
                <strong>{fmtWon(selected.settlementFinal)}</strong>
              </div>
              <div>
                <span>지급 완료</span>
                <strong>{fmtWon(selected.payoutDone)}</strong>
              </div>
              <div>
                <span>이전 기간 대비</span>
                <strong>
                  {selected.prevGmv
                    ? fmtSignedPct(delta(selected.gmv, selected.prevGmv).pct)
                    : "데이터 없음"}
                </strong>
              </div>
            </div>
