...
export function CancelRateStatsPage() {
  ...
  const metricLabels: Record<TrendMetric, string> = {
    cancelCount: "취소 건수",
    sellerFaultCount: "판매자 귀책",
    buyerFaultCount: "구매자 귀책",
  };
  ...
  const cancelGrowth = prevAgg.cancelCount
    ? delta(agg.cancelCount, prevAgg.cancelCount).pct
    : 0;
  const rateChange = prevAgg.cancelRate
    ? agg.cancelRate - prevAgg.cancelRate
    : 0;
  const sellerFaultChange = prevAgg.sellerFaultRate
    ? agg.sellerFaultRate - prevAgg.sellerFaultRate
    : 0;
  ...
  const kpis = [
    {
      key: "cancels",
      label: "취소 건수",
      value: fmtCount(agg.cancelCount),
      change: cancelGrowth,
      sub: "전기 대비",
      badIsGood: true,
    },
    ...
    },
    {
      key: "sellerFault",
      label: "판매자 귀책률",
      value: fmtPct(agg.sellerFaultRate),
      change: sellerFaultChange,
      sub: "전기 대비 · 취소 대비",
      isPoint: true,
      badIsGood: true,
    },
    {
      key: "hours",
      label: "평균 처리 시간",
      value: `${agg.avgProcessHours.toFixed(1)}시간`,
      sub: "요청 → 처리 완료",
      noChange: true,
    },
  ];
  ...
  const gridRows: GridRow[] = rows.map((row) => {
    const change = row.prevCancelCount
      ? delta(row.cancelCount, row.prevCancelCount).pct
      : 0;
    const cells: Cell[] = [
      { kind: "stack", title: row.name, subtitle: row.subtitle },
      {
        kind: "text",
        text: fmtCount(row.cancelCount),
        align: "right",
        numeric: true,
        weight: 700,
      },
      ...
      {
        kind: "text",
        text: fmtCount(row.sellerFaultCount),
        align: "right",
        numeric: true,
      },
      {
        kind: "text",
        text: fmtPct(row.sellerFaultRate),
        align: "right",
        numeric: true,
        color: row.sellerFaultRate >= 60 ? "#dc2626" : undefined,
      },
      {
        kind: "text",
  ...
  const exportFields = [
    ...
    },
    {
      key: "cancelCount",
      label: "취소 건수",
      value: (row: DimensionRow) => row.cancelCount,
    },
    {
      key: "rate",
      label: "취소율",
      value: (row: DimensionRow) => fmtPct(row.cancelRate),
    },
    {
      key: "sellerFault",
      label: "판매자 귀책",
      value: (row: DimensionRow) => row.sellerFaultCount,
    },
    {
      key: "sellerFaultRate",
      label: "귀책률",
      value: (row: DimensionRow) => fmtPct(row.sellerFaultRate),
    },
    {
      key: "change",
      label: "전기 대비",
      value: (row: DimensionRow) =>
        row.prevCancelCount
          ? fmtSignedPct(delta(row.cancelCount, row.prevCancelCount).pct)
          : "-",
    },
  ];
  ...
  function download() {
    ...
    downloadStatisticsReport({
      ...
      summary: [
        {
          label: "취소 건수",
          current: agg.cancelCount,
          previous: prevAgg.cancelCount,
          change: agg.cancelCount - prevAgg.cancelCount,
          changeRate: `${cancelGrowth.toFixed(1)}%`,
        },
        {
          label: "거래 취소율(%)",
        ...
        {
          label: "판매자 귀책률(%)",
          current: Number(agg.sellerFaultRate.toFixed(2)),
          previous: Number(prevAgg.sellerFaultRate.toFixed(2)),
          change: `${sellerFaultChange.toFixed(2)}%p`,
        },
        {
          label: "평균 처리 시간(시간)",
          current: Number(agg.avgProcessHours.toFixed(1)),
          previous: Number(prevAgg.avgProcessHours.toFixed(1)),
        },
      ],
      trend: {
        name: "02_취소추이",
        headers: ["일자", "취소 건수", "판매자 귀책", "구매자 귀책"],
        rows: trendDays.map((row) => [
          row.date,
          row.cancelCount,
          row.sellerFaultCount,
          row.buyerFaultCount,
        ]),
      },
      dimensions: [
      ...
      definitions: [
        {
          term: "거래 취소율",
          description: "조회기간 발생 거래 대비 취소로 종료된 거래의 비율",
        },
        {
          term: "판매자 귀책률",
          description:
            "취소 건 중 품절·미응답 등 판매자 사유로 발생한 취소의 비율",
        },
        {
          term: "평균 처리 시간",
          description: "취소 요청부터 처리 완료까지 걸린 평균 시간",
        },
      ],
    ...
    <section className={`${shared.page} ${layout.page}`}>
      <div className={shared.headTop}>
        <div className={shared.headRow}>
          <div>
            <h1 className={shared.title}>거래 취소율</h1>
            <p className={shared.subtitle}>
              거래가 얼마나 자주 취소되고 있으며, 어느 쪽 귀책으로 발생하는지
              분석합니다.
            </p>
          </div>
        ...
        {showBasis && (
          <div className={layout.basisPanel}>
            <Info size={16} />
            <div>
              <strong>거래 취소율 집계 기준</strong>
              <p>
                거래 취소율은 조회기간 발생 거래 대비 취소로 종료된 거래의
                비율입니다. 판매자 귀책률은 취소 건 중 품절·판매자 미응답 등
                판매자 사유로 발생한 취소의 비율이며, 나머지는 구매자 귀책(단순
                변심·미입금 등)으로 분류합니다. 평균 처리 시간은 취소 요청부터
                처리 완료까지 걸린 평균 시간입니다.
              </p>
            </div>
            <button
        ...
        <section className={layout.card}>
          ...
          {primaryTrend.length ? (
            ...
            <div className={layout.emptyState}>
              <strong>분석할 취소 데이터가 없습니다.</strong>
        ...
        <section className={layout.card}>
          <div className={layout.analysisHead}>
            <div>
              <h2>상세 분석</h2>
              <p>
                분석 결과에서 바로 처리하지 않고 취소 관리 화면으로 이동합니다.
              </p>
            </div>
            <button
              type="button"
          ...
          <DataGrid
            columns={columns}
            rows={gridRows}
            gridTemplate="minmax(190px,1.5fr) 74px 56px 88px 58px 74px"
            minWidth="640px"
            empty={!rows.length}
            emptyText="현재 조건에 해당하는 취소 데이터가 없습니다."
        ...
        >
          ...
          <div className={layout.drawerBody}>
            <div className={layout.drawerHero}>
              <span>취소 건수</span>
              <strong>{fmtCount(selected.cancelCount)}</strong>
            </div>
            <div className={layout.drawerFacts}>
              ...
              <div>
                <span>판매자 귀책</span>
                <strong>{fmtCount(selected.sellerFaultCount)}</strong>
              </div>
              <div>
                <span>귀책률</span>
                <strong
                  className={
                    selected.sellerFaultRate >= 60
                      ? styles.warnValue
                      : undefined
                  }
                >
                  {fmtPct(selected.sellerFaultRate)}
                </strong>
              </div>
              <div>
                <span>이전 기간 대비</span>
                <strong>
                  {selected.prevCancelCount
                    ? fmtSignedPct(
                        delta(selected.cancelCount, selected.prevCancelCount)
                          .pct,
                      )
                    : "데이터 없음"}
                </strong>
