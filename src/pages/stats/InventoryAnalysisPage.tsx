...
export function InventoryAnalysisPage() {
  ...
  return <section className={`${shared.page} ${layout.page}`}>
    <div className={shared.headTop}>
      ...
      <div className={layout.filterCard}><div className={`${layout.filterGrid} ${styles.inventoryFilterGrid}`}><label className={layout.filterField}><span>재고 기준일</span><DatePicker controlSize="sm" value={snapshotDate} onChange={(event) => setSnapshotDate(event.target.value)} /></label><label className={layout.filterField}><span>{mode === 'b2b' ? '수요 분석 기간' : '판매속도 기준'}</span><select value={period} onChange={(event) => setPeriod(event.target.value)}>{Object.keys(ANALYSIS_DAYS).map((item) => <option key={item}>{item}</option>)}</select></label><label className={layout.filterField}><span>카테고리</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label><label className={layout.filterField}><span>창고</span><select value={warehouse} onChange={(event) => setWarehouse(event.target.value)}><option>전체 창고</option><option>수도권 센터</option><option>부산 센터</option></select></label><label className={layout.filterField}><span>재고 위험</span><select value={risk} onChange={(event) => setRisk(event.target.value)}><option>전체 상태</option>{Object.keys(RISK_META).map((item) => <option key={item}>{item}</option>)}</select></label><label className={layout.filterField}><span>재고 평가 기준</span><select value={valuation} onChange={(event) => setValuation(event.target.value)}><option>평균 원가</option><option>최근 매입가</option><option>표준 원가</option></select></label><label className={layout.filterField}><span>입고 예정 반영</span><select value={includeInbound} onChange={(event) => setIncludeInbound(event.target.value)}><option>미반영</option><option>7일 내 입고만</option><option>전체 입고 예정</option></select></label><div className={layout.filterActions}><button type="button" className={layout.resetButton} onClick={reset}>초기화</button><button type="button" className={layout.applyButton} onClick={() => flash('조회 조건을 적용했습니다.')}>조회</button></div></div><div className={layout.periodSummary}>재고 기준 <strong>{fmtDate(snapshotDate)} 23:59</strong> · {mode === 'b2b' ? '수요' : '판매'} 분석 <strong>{period}</strong> · 최근 집계 <strong>{refreshedAt}</strong> · C2C는 플랫폼 보유 재고가 없어 분석 대상에서 제외</div></div>
