...
export function DisputeRateStatsPage() {
  ...
  const disputeGrowth = prevAgg.disputeCount
    ? delta(agg.disputeCount, prevAgg.disputeCount).pct
    : 0;
  const rateChange = prevAgg.disputeRate
    ? agg.disputeRate - prevAgg.disputeRate
    : 0;
  const resolveChange = prevAgg.resolveRate
    ? agg.resolveRate - prevAgg.resolveRate
    : 0;
  ...
  const kpis = [
    {
      key: "disputes",
      label: "분쟁 발생 건수",
      value: fmtCount(agg.disputeCount),
      change: disputeGrowth,
      sub: "전기 대비",
      badIsGood: true,
    },
    {
    ...
    },
    {
      key: "days",
      label: "평균 해결 소요",
      value: `${agg.avgResolveDays.toFixed(1)}일`,
      sub: "접수 → 해결 완료",
      noChange: true,
    },
  ];
  ...
  const gridRows: GridRow[] = rows.map((row) => {
    const change = row.prevDisputeCount
      ? delta(row.disputeCount, row.prevDisputeCount).pct
      : 0;
    const cells: Cell[] = [
      { kind: "stack", title: row.name, subtitle: row.subtitle },
      {
        kind: "text",
        text: fmtCount(row.disputeCount),
        align: "right",
        numeric: true,
        weight: 700,
      },
      {
        ...
        align: "right",
        numeric: true,
        color: row.disputeRate >= 2 ? "#dc2626" : undefined,
      },
      {
        kind: "text",
        text: fmtCount(row.resolvedCount),
        align: "right",
        numeric: true,
      },
      {
        kind: "text",
        text: fmtPct(row.resolveRate),
        align: "right",
        numeric: true,
        color: row.resolveRate < 50 ? "#dc2626" : "#059669",
      },
      {
        kind: "text",
  ...
  const exportFields = [
    ...
    },
    {
      key: "disputeCount",
      label: "분쟁 건수",
      value: (row: DimensionRow) => row.disputeCount,
    },
    {
      key: "rate",
      label: "분쟁률",
      value: (row: DimensionRow) => fmtPct(row.disputeRate),
    },
    {
      key: "resolved",
      label: "해결 완료",
      value: (row: DimensionRow) => row.resolvedCount,
    },
    {
      key: "resolveRate",
      label: "해결율",
      value: (row: DimensionRow) => fmtPct(row.resolveRate),
    },
    {
      key: "change",
      label: "전기 대비",
      value: (row: DimensionRow) =>
        row.prevDisputeCount
          ? fmtSignedPct(delta(row.disputeCount, row.prevDisputeCount).pct)
          : "-",
    },
  ];
  ...
  function download() {
    ...
    downloadStatisticsReport({
      ...
      summary: [
        {
          label: "분쟁 발생 건수",
          current: agg.disputeCount,
          previous: prevAgg.disputeCount,
          change: agg.disputeCount - prevAgg.disputeCount,
          changeRate: `${disputeGrowth.toFixed(1)}%`,
        },
        {
          label: "분쟁률(%)",
          current: Number(agg.disputeRate.toFixed(2)),
          previous: Number(prevAgg.disputeRate.toFixed(2)),
          change: `${rateChange.toFixed(2)}%p`,
        },
        {
          label: "분쟁 해결율(%)",
          current: Number(agg.resolveRate.toFixed(2)),
          previous: Number(prevAgg.resolveRate.toFixed(2)),
          change: `${resolveChange.toFixed(2)}%p`,
        },
        {
          label: "평균 해결 소요(일)",
          current: Number(agg.avgResolveDays.toFixed(1)),
          previous: Number(prevAgg.avgResolveDays.toFixed(1)),
        },
      ],
      trend: {
        name: "02_분쟁추이",
        headers: ["일자", "분쟁 발생", "해결 완료"],
      ...
      dimensions: [
        dimensionSheet("사유별", reasonTabRows),
        dimensionSheet("판매자별", sellerTabRows),
        dimensionSheet("카테고리별", categoryTabRows),
        {
          name: "분쟁사유구성",
          headers: ["사유", "건수", "비중(%)"],
          rows: reasonComp.map((row) => [
            row.label,
            row.count,
      ...
      definitions: [
        {
          term: "분쟁률",
          description: "조회기간 완료 거래 대비 분쟁이 접수된 건의 비율",
        },
        {
          term: "분쟁 해결율",
          description:
            "접수된 분쟁 중 조정·환불·기각 등으로 해결이 끝난 건의 비율",
        },
        {
          term: "평균 해결 소요",
          description: "분쟁 접수부터 해결 완료까지 걸린 평균 일수",
        },
      ],
  ...
  function goToDisputes(row: DimensionRow) {
    navigate(
      `/c2c/disputes/processing?keyword=${encodeURIComponent(row.name)}`,
    );
  }
    ...
    <section className={`${shared.page} ${layout.page}`}>
      <div className={shared.headTop}>
        <div className={shared.headRow}>
          <div>
            <h1 className={shared.title}>분쟁률</h1>
            <p className={shared.subtitle}>
              거래 대비 분쟁이 얼마나 발생하고 있으며, 분쟁 해결은 얼마나 빠르게
              이뤄지는지 분석합니다.
            </p>
          </div>
        ...
        {showBasis && (
          <div className={layout.basisPanel}>
            <Info size={16} />
            <div>
              <strong>분쟁률 집계 기준</strong>
              <p>
                분쟁률은 조회기간 내 완료 거래 수 대비 분쟁이 접수된 건의
                비율입니다. 분쟁 해결율은 접수된 분쟁 중 조정·환불·기각 등으로
                처리가 끝난 건의 비율이며, 평균 해결 소요는 분쟁 접수부터 해결
                완료까지 걸린 평균 일수입니다. 단순 신고와 달리 분쟁은 양측
                주장이 상충하는 건만 집계합니다.
              </p>
            </div>
            <button
      ...
      <div className={layout.body}>
        ...
        <div className={layout.secondaryMetrics}>
          <div>
            <span>해결 완료 건수</span>
            <strong>{fmtCount(agg.resolvedCount)}</strong>
            <em>조회기간 해결 완료</em>
          </div>
          <div>
            <span>미해결 건수</span>
            <strong
              className={agg.pendingCount > 0 ? styles.warnValue : undefined}
            >
              {fmtCount(agg.pendingCount)}
            </strong>
            <em>진행중인 분쟁</em>
          </div>
          <div>
            <span>분쟁 해결률</span>
            <strong className={layout.changeUp}>
              {fmtPct(agg.resolveRate)}
            </strong>
            <em>접수 대비 해결율</em>
          </div>
          <div>
            <span>평균 해결기간</span>
            <strong>{agg.avgResolveDays.toFixed(1)}일</strong>
            <em>접수 → 해결 완료</em>
          </div>
        </div>
        ...
        <section className={layout.card}>
          ...
          {primaryTrend.length ? (
            ...
            <div className={layout.emptyState}>
              <strong>분석할 분쟁 데이터가 없습니다.</strong>
              <span>기간을 변경해 주세요.</span>
        ...
        <section className={layout.card}>
          <div className={layout.analysisHead}>
            <div>
              <h2>상세 분석</h2>
              <p>
                분석 결과에서 바로 처리하지 않고 분쟁 관리 화면으로 이동합니다.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className={layout.dimensionTabs}>
            ...
            <button
              type="button"
              className={dimension === "seller" ? layout.dimensionActive : ""}
              onClick={() => setDimension("seller")}
            >
              판매자별
            </button>
            <button
          ...
          <DataGrid
            columns={columns}
            rows={gridRows}
            gridTemplate="minmax(190px,1.5fr) 74px 56px 74px 56px 74px"
            minWidth="620px"
            empty={!rows.length}
            emptyText="현재 조건에 해당하는 분쟁 데이터가 없습니다."
        ...
        >
          ...
          <div className={layout.drawerBody}>
            <div className={layout.drawerHero}>
              <span>분쟁 건수</span>
              <strong>{fmtCount(selected.disputeCount)}</strong>
            </div>
            <div className={layout.drawerFacts}>
              <div>
                ...
                <strong
                  className={
                    selected.disputeRate >= 2 ? styles.warnValue : undefined
                  }
                >
              ...
              <div>
                <span>해결 완료</span>
                <strong>{fmtCount(selected.resolvedCount)}</strong>
              </div>
              <div>
                <span>해결율</span>
                <strong>{fmtPct(selected.resolveRate)}</strong>
              </div>
              <div>
                <span>이전 기간 대비</span>
                <strong>
                  {selected.prevDisputeCount
                    ? fmtSignedPct(
                        delta(selected.disputeCount, selected.prevDisputeCount)
                          .pct,
                      )
                    : "데이터 없음"}
                </strong>
