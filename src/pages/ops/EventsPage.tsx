...
export function EventsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"전체" | EventStatus>(
    "전체",
  );
  ...
  const counts = useMemo(() => {
    const result: Record<EventQuickFilter, number> = {
      전체: 0,
      진행중: 0,
      "진행 예정": 0,
      종료: 0,
    ...
    QUICK_FILTER_LABELS.forEach((key) => {
      result[key] = events.filter((event) =>
        matchesQuickFilter(event, key),
      ).length;
    });
    return result;
  }, [events]);
  ...
  const filtered = useMemo(
    () =>
      events.filter((event) => {
        ...
        if (targetFilter !== "전체" && event.target !== targetFilter)
          return false;
        if (
          statusFilter !== "전체" &&
          computeEventStatus(event) !== statusFilter
        )
          return false;
        if (managerFilter !== "전체" && event.manager !== managerFilter)
          return false;
        ...
        if (searchField === "이벤트번호")
          return event.id.toLowerCase().includes(query);
        if (searchField === "관리명")
          return event.managementName.toLowerCase().includes(query);
        if (searchField === "사용자 노출명")
          return event.displayName.toLowerCase().includes(query);
        return [event.id, event.managementName, event.displayName].some(
  ...
  const previewEvent = previewId
    ? (events.find((event) => event.id === previewId) ?? null)
    : null;
  const editingEvent =
    editorTarget && editorTarget !== "new"
      ? (events.find((event) => event.id === editorTarget) ?? null)
      : null;
  const confirmEvent = confirm
    ? (events.find((event) => event.id === confirm.id) ?? null)
    : null;

  function updateEvent(id: string, updater: (event: EventEntry) => EventEntry) {
    setEvents((prev) =>
      prev.map((event) => (event.id === id ? updater(event) : event)),
    );
  }
  ...
  function toggleActive(id: string) {
    updateEvent(id, (event) => ({
      ...
      history: [
        ...
        {
          when: "방금",
          title: event.isActive ? "비활성 처리" : "활성화",
          by: "관리자",
        },
      ],
    }));
  ...
  function addMemo(id: string, text: string) {
    updateEvent(id, (event) => ({
      ...event,
      memos: [...event.memos, { when: "방금", by: "관리자", text }],
    }));
  }
  ...
  function duplicateEvent(id: string) {
    ...
    const clone: EventEntry = {
      ...
      updatedAt: "방금",
      history: [
        { when: "방금", title: "이벤트 복제", by: "관리자", detail: source.id },
      ],
      memos: [],
  ...
  function submitEditor(values: EventFormValues, mode: "draft" | "publish") {
    ...
    const linkedBanners = values.bannerLinked
      ? current?.linkedBanners.length
        ? current.linkedBanners
        : [
            {
              id: "BNR-연결대기",
              name: "연결 배너",
              status: "노출예정" as const,
            },
          ]
      : [];
    const linkedPopups = values.popupLinked
      ? current?.linkedPopups.length
        ? current.linkedPopups
        : [
            {
              id: "POP-연결대기",
              name: "연결 팝업",
      ...
      : [];
    const linkedNotice = values.noticeLinked
      ? (current?.linkedNotice ?? {
          id: "NOTICE-연결대기",
          name: "관련 공지",
          status: "게시중" as const,
        })
    ...
    const next: EventEntry = {
      ...
      benefitName:
        values.benefitType === "혜택 없음" ? "-" : values.benefitName,
      ...
      winner:
        values.type === "응모형"
          ? {
              totalEntries:
                current?.winner?.totalEntries ?? current?.participants ?? 0,
      ...
      memos: values.memo.trim()
        ? [
            ...(current?.memos ?? []),
            { when: "방금", by: "관리자", text: values.memo.trim() },
          ]
        : (current?.memos ?? []),
    };
    setEvents((prev) =>
      current
        ? prev.map((event) => (event.id === current.id ? next : event))
        : [next, ...prev],
    );
    setEditorTarget(null);
    setSelectedId(id);
  ...
  function downloadCsv() {
    ...
    const rows = source.map((event) => [
      ...
      event.displayName,
      event.type,
      fmtCompactRange(event.eventStartAt, event.eventEndAt),
      fmtCompactRange(event.displayStartAt, event.displayEndAt),
      event.target,
      event.participants,
      event.benefitName,
    ...
    const cells: Cell[] = [
      {
        ...
        subtitle: `${event.managementName} · ${event.id}`,
      },
      { kind: "pillText", text: event.type, bg: "#f4f4f5", fg: "#52525b" },
      {
        kind: "stack",
        title: fmtCompactRange(event.eventStartAt, event.eventEndAt),
        subtitle: `노출 ${fmtCompactRange(event.displayStartAt, event.displayEndAt)}`,
      },
      {
        kind: "text",
        text: event.target.replace(" 사용자", ""),
        color: "#52525b",
        size: "11.5px",
        weight: 500,
      },
      {
        kind: "text",
        text:
          event.participationMethod === "참여 기능 없음"
            ? "-"
            : event.participants.toLocaleString("ko-KR"),
        color: "#27272a",
        size: "12px",
        weight: 700,
        align: "right",
      ...
      {
        kind: "stack",
        title: benefitSummary(event),
        subtitle: event.benefitType === "혜택 없음" ? "-" : event.benefitName,
      },
      {
        kind: "badgeSub",
        text: status,
      ...
      {
        kind: "text",
        text: linkedExposureSummary(event),
        color: "#52525b",
        size: "11.5px",
        weight: 500,
      },
      {
        ...
        subtitle:
          event.updatedAt === "방금"
            ? "방금"
            : event.updatedAt.slice(5, 10).replace("-", "."),
      },
    ];
    return {
      id: event.id,
    ...
    <div className={shared.page} onClick={() => menuId && setMenuId(null)}>
          ...
          <div className={shared.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as "전체" | EventStatus)
                }
              >
                <option value="전체">진행상태 전체</option>
                {Object.keys(EVENT_STATUS_META).map((item) => (
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={typeFilter}
                onChange={(e) =>
                  setTypeFilter(e.target.value as "전체" | EventType)
                }
              >
                <option value="전체">이벤트유형 전체</option>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={targetFilter}
                onChange={(e) =>
                  setTargetFilter(e.target.value as "전체" | EventTarget)
                }
              >
                <option value="전체">참여대상 전체</option>
                {EVENT_TARGETS.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
            </label>
      ...
      <div className={`${shared.gridWrap} ${styles.eventGrid}`}>
        <DataGrid
          ...
          allSelected={
            filtered.length > 0 &&
            filtered.every((event) => selectedIds.includes(event.id))
          }
          ...
          emptyText={
            filter === "진행중"
              ...
              : filter === "진행 예정"
                ? "예정된 이벤트가 없습니다."
                : filter === "확인 필요"
                  ? "현재 운영 확인이 필요한 이벤트가 없습니다."
                  : q || typeFilter !== "전체" || targetFilter !== "전체"
                    ? "검색 결과가 없습니다."
                    : "등록된 이벤트가 없습니다."
          }
          emptySubtext={
          ...
          emptyActionLabel={gridRows.length === 0 ? "초기화" : undefined}
      ...
      {selected && (
        <EventDetailDrawer
          ...
          onDuplicate={() => duplicateEvent(selected.id)}
          onStart={() => startEvent(selected.id)}
          onEnd={() => setConfirm({ kind: "end", id: selected.id })}
      ...
      {confirm?.kind === "end" && confirmEvent && (
        ...
        >
          <div className={shared.dialogBox}>
            <div className={shared.dialogTitle}>
              이벤트를 조기 종료하시겠습니까?
            </div>
            <div className={shared.dialogBody}>{confirmEvent.displayName}</div>
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>기존 종료일</span>
                <span>{confirmEvent.eventEndAt}</span>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>현재 참여</span>
                <span>
                  {confirmEvent.participants.toLocaleString("ko-KR")}명
                </span>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>혜택 지급 대기</span>
                <span>
                  {confirmEvent.benefitPending.toLocaleString("ko-KR")}건
                </span>
              </div>
            </div>
            <div className={styles.dialogNote}>
              종료 후 신규 참여는 불가하지만 기존 참여 데이터와 지급 대기 혜택은
              유지됩니다.
            </div>
            <div className={shared.dialogActions} style={{ marginTop: 16 }}>
              ...
              <button
                ...
                onClick={() => endEvent(confirmEvent.id)}
              >
                이벤트 종료
              </button>
            </div>
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogBody}>
              {confirmEvent.managementName}
              <br />
              참여자와 노출 이력이 없는 작성중 이벤트만 삭제할 수 있습니다.
            </div>
            <div className={shared.dialogActions}>
              ...
              <button
                ...
                className={shared.dialogBtn}
                style={{ border: 0, background: "#dc2626", color: "#fff" }}
                onClick={() => deleteEvent(confirmEvent.id)}
              >
                삭제
              </button>
            </div>
