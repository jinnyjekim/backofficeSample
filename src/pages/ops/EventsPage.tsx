...
export function EventsPage() {
  ...
  return (
    <div className={shared.page} onClick={() => menuId && setMenuId(null)}>
      ...
      {confirm?.kind === 'end' && confirmEvent && <div className={shared.dialogOverlay}><div className={shared.dialogBox}><div className={shared.dialogTitle}>이벤트를 조기 종료하시겠습니까?</div><div className={shared.dialogBody}>{confirmEvent.displayName}</div><div className={shared.dialogSummary}><div className={shared.dialogSummaryRow}><span>기존 종료일</span><span>{confirmEvent.eventEndAt}</span></div><div className={shared.dialogSummaryRow}><span>현재 참여</span><span>{confirmEvent.participants.toLocaleString('ko-KR')}명</span></div><div className={shared.dialogSummaryRow}><span>혜택 지급 대기</span><span>{confirmEvent.benefitPending.toLocaleString('ko-KR')}건</span></div></div><div className={styles.dialogNote}>종료 후 신규 참여는 불가하지만 기존 참여 데이터와 지급 대기 혜택은 유지됩니다.</div><div className={shared.dialogActions} style={{ marginTop: 16 }}><button type="button" className={shared.dialogBtn} style={{ border: '1px solid rgba(0,0,0,.12)', background: '#fff', color: '#52525b' }} onClick={() => setConfirm(null)}>취소</button><button type="button" className={shared.dialogBtn} style={{ border: 0, background: '#dc2626', color: '#fff' }} onClick={() => endEvent(confirmEvent.id)}>이벤트 종료</button></div></div></div>}
      {confirm?.kind === 'delete' && confirmEvent && <div className={shared.dialogOverlay}><div className={shared.dialogBox}><div className={shared.dialogTitle}>이벤트 초안을 삭제하시겠습니까?</div><div className={shared.dialogBody}>{confirmEvent.managementName}<br />참여자와 노출 이력이 없는 작성중 이벤트만 삭제할 수 있습니다.</div><div className={shared.dialogActions}><button type="button" className={shared.dialogBtn} style={{ border: '1px solid rgba(0,0,0,.12)', background: '#fff', color: '#52525b' }} onClick={() => setConfirm(null)}>취소</button><button type="button" className={shared.dialogBtn} style={{ border: 0, background: '#dc2626', color: '#fff' }} onClick={() => deleteEvent(confirmEvent.id)}>삭제</button></div></div></div>}
    </div>
  );
}
