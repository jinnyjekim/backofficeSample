...
export function FaqPage() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<"전체" | FaqCategory>(
    "전체",
  );
  const [importantFilter, setImportantFilter] = useState<"전체" | "중요만">(
    "전체",
  );
  ...
  const [orderCategory, setOrderCategory] = useState<FaqCategory>(
    categories[0],
  );
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTER_LABELS.forEach((k) => {
      c[k] = faqs.filter((f) => matchesQuickFilter(f, k)).length;
    });
    return c;
  }, [faqs]);
  ...
  const categoryCounts = useMemo(() => {
    ...
    categories.forEach((cat) => {
      c[cat] = faqs.filter((f) => f.category === cat).length;
    });
    return c;
  }, [faqs, categories]);
  ...
  function toggleSelAll() {
    setSelectedIds((prev) =>
      prev.length === filtered.length ? [] : filtered.map((f) => f.id),
    );
  }
  ...
  function addMemo(id: string, text: string) {
    updateFaq(id, (f) => ({
      ...f,
      memos: [...f.memos, { when: "방금", by: "관리자", text }],
    }));
  }
  ...
  function confirmBulkCategory() {
    selectedIds.forEach((id) =>
      updateFaq(id, (f) => ({ ...f, category: bulkCategory })),
    );
    setSelectedIds([]);
    setConfirm(null);
  }
  ...
  function confirmBulkImportant() {
    selectedIds.forEach((id) =>
      updateFaq(id, (f) => ({ ...f, important: bulkImportant === "지정" })),
    );
    setSelectedIds([]);
    setConfirm(null);
  }
  ...
  function saveOrder() {
    orderDraft.forEach((id, i) =>
      updateFaq(id, (f) => ({ ...f, order: i + 1 })),
    );
    setConfirm(null);
  }
  ...
  function submitEditor(form: FaqFormData) {
    ...
    const endAt =
      form.endMode === "없음" ? null : `${form.endDate} ${form.endTime}`;
    ...
    if (existing) {
      updateFaq(existing.id, (f) => ({
        ...
        history: [
          ...f.history,
          { when: "방금", title: "FAQ 수정", by: "관리자" },
        ],
        memos: form.memo
          ? [...f.memos, { when: "방금", by: "관리자", text: form.memo }]
          : f.memos,
      }));
  ...
  const rows: GridRow[] = filtered.map((f) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "noWarn",
        no: `${f.important ? "⭐ " : ""}${f.question}`,
        hasIssue: review.flag,
        issueTitle: review.reasons.join(" · "),
      },
      {
        kind: "text",
        text: f.important ? "중요" : "-",
        color: f.important ? "#dc2626" : "#a1a1aa",
        size: "11.5px",
        weight: 600,
      },
      ...
      {
        kind: "text",
        text: f.views.toLocaleString("ko-KR"),
        color: "#3f3f46",
        size: "12px",
        weight: 600,
        align: "right",
  ...
  const confirmFaq =
    confirm && "id" in confirm
      ? (faqs.find((f) => f.id === confirm.id) ?? null)
      : null;
    ...
    <div className={styles.page} onClick={() => menuId && setMenuId(null)}>
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              사용자가 자주 묻는 질문과 답변을 카테고리·검색 키워드·노출 순서로
              관리합니다.
            </div>
          </div>
          ...
          <div className={styles.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                onChange={(e) =>
                  setImportantFilter(e.target.value as "전체" | "중요만")
                }
              >
        ...
        <div className={styles.resultRow}>
          ...
          <div className={styles.resultActions}>
            <button
              ...
              onClick={() => setConfirm({ kind: "categoryManage" })}
            >
              카테고리 관리
            </button>
            <button
      ...
      {selectedIds.length > 0 && (
        <div className={styles.bulkBar}>
          ...
          <button
            type="button"
            className={styles.bulkBtn}
            onClick={() => setConfirm({ kind: "bulkCategory" })}
          >
            카테고리 변경
          </button>
          <button
            type="button"
            className={styles.bulkBtn}
            onClick={() => setConfirm({ kind: "bulkStatus" })}
          >
            공개 설정
          </button>
          <button
            type="button"
            className={styles.bulkBtn}
            onClick={() => setConfirm({ kind: "bulkImportant" })}
          >
            중요 FAQ 설정
          </button>
          <button
      ...
      {selected && (
        <FaqDetailDrawer
          ...
          onRequestRepost={() =>
            setConfirm({ kind: "repost", id: selected.id })
          }
          onDuplicate={() => duplicate(selected.id)}
          onRequestDelete={() =>
            setConfirm({ kind: "delete", id: selected.id })
          }
          ...
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogActions}>
              ...
              <button
                ...
                onClick={() => confirmRepost(confirmFaq.id)}
              >
                재게시
              </button>
            </div>
      ...
      {confirm?.kind === "delete" && confirmFaq && (
        ...
        >
          <div className={styles.dialogBox}>
            ...
            >{`'${confirmFaq.question}'이(가) 삭제됩니다. 공개 이력이 없는 FAQ만 삭제할 수 있습니다.`}</div>
            <div className={styles.dialogActions}>
              <button
                type="button"
                className={styles.dialogBtn}
              ...
              <button
                ...
                onClick={() => confirmDelete(confirmFaq.id)}
              >
                삭제
              </button>
            </div>
          ...
          <div className={styles.dialogBox}>
            ...
            >
              <label style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input
                  type="radio"
                  checked={bulkStatus === "공개"}
                  onChange={() => setBulkStatus("공개")}
                />
                공개
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input
                  type="radio"
                  checked={bulkStatus === "비공개"}
                  onChange={() => setBulkStatus("비공개")}
                />
                비공개
              </label>
            </div>
      ...
      {confirm?.kind === "bulkImportant" && (
        ...
        >
          <div className={styles.dialogBox}>
            <div className={styles.dialogTitle}>
              {selectedIds.length}건 중요 FAQ 설정
            </div>
            <div
              className={styles.dialogBody}
              style={{ display: "flex", gap: 14, marginBottom: 16 }}
            >
              <label style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input
                  type="radio"
                  checked={bulkImportant === "지정"}
                  onChange={() => setBulkImportant("지정")}
                />
                중요로 지정
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input
                  type="radio"
                  checked={bulkImportant === "해제"}
                  onChange={() => setBulkImportant("해제")}
                />
                중요 해제
              </label>
            </div>
                ...
                <div className={styles.dialogSummaryRow} key={c}>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        opacity: (categoryCounts[c] ?? 0) > 0 ? 0.3 : 1,
                        cursor:
                          (categoryCounts[c] ?? 0) > 0 ? "default" : "pointer",
                      }}
                      title={
                        (categoryCounts[c] ?? 0) > 0
                          ? "연결된 FAQ가 있어 삭제할 수 없습니다"
                          : "삭제"
                      }
        ...
        >
          <div className={styles.dialogBox}>
            ...
            <select
              ...
              value={orderCategory}
              onChange={(e) => changeOrderCategory(e.target.value)}
            >
              {categories.map((c) => (
                <option key={c}>{c}</option>
              ))}
                  ...
                  <div className={styles.dialogSummaryRow} key={id}>
                    ...
                    <span style={{ display: "flex", gap: 4 }}>
                      ...
                      <button
                        ...
                        style={{
                          opacity: i === orderDraft.length - 1 ? 0.3 : 1,
                        }}
                        onClick={() => moveOrder(i, 1)}
                      >
                        ↓
