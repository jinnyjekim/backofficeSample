  ...
  return <div className={shared.page} onClick={() => menuId && setMenuId(null)}>
    <header className={shared.header}>
      ...
      <div className={shared.filterBox}>
        <form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="약관명 / 약관 코드 / 버전"/><button type="submit" className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>
        {QUICK.map((item) => {
