...
import { TERMS, currentVersion, type ConsentType, type TermsDefinition, type TermsStatus, type TermsVersion } from './termsData';
...
type FormState = { name: string; code: string; type: string; version: string; consent: ConsentType; effectiveFrom: string; effectiveTo: string; description: string; content: string; changeReason: string; reConsent: boolean };
...
const COLUMNS = [
  { label: '약관명 / 코드' }, { label: '유형' }, { label: '버전' }, { label: '동의 구분' }, { label: '상태' },
  { label: '적용 시작일' }, { label: '적용 종료일' }, { label: '최종 수정' }, { label: '관리', align: 'right' as const },
];

const emptyForm = (): FormState => ({
  name: '', code: '', type: '이용약관', version: 'v1.0', consent: '필수', effectiveFrom: '', effectiveTo: '',
...
export function TermsManagementPage() {
  ...
  }), [consentFilter, dateFrom, dateTo, quick, search, statusFilter, terms, typeFilter]);
  ...
  function resetFilters() {
    setQuick('전체'); setKeyword(''); setSearch(''); setTypeFilter(''); setStatusFilter(''); setConsentFilter(''); setDateFrom(''); setDateTo('');
  }
  ...
  function openCreate(source?: TermsDefinition) {
    ...
    setForm({ name: `[복사본] ${source.name}`, code, type: source.type, version: 'v1.0', consent: source.consent, effectiveFrom: '', effectiveTo: '', description: source.description, content: version.content, changeReason: '기존 약관 복제', reConsent: false });
  ...
  function openEdit(term: TermsDefinition) {
    ...
    setForm({ name: term.name, code: term.code, type: term.type, version: version.version, consent: term.consent, effectiveFrom: version.effectiveFrom, effectiveTo: version.effectiveTo ?? '', description: term.description, content: version.content, changeReason: version.changeReason, reConsent: version.reConsent });
  ...
  function openNewVersion(term: TermsDefinition) {
    const version = currentVersion(term);
    setForm({ name: term.name, code: term.code, type: term.type, version: '', consent: term.consent, effectiveFrom: '', effectiveTo: '', description: term.description, content: version.content, changeReason: '', reConsent: false });
  ...
  function save(mode: 'draft' | 'publish') {
    ...
    if (editor.mode === 'edit' && editor.termId) {
      setTerms((items) => items.map((term) => term.id !== editor.termId ? term : {
        ...term, name: form.name.trim(), code: form.code.trim(), type: form.type, consent: form.consent, description: form.description.trim(),
      ...
      setTerms((items) => items.map((term) => term.id !== editor.termId ? term : {
        ...term, name: form.name.trim(), type: form.type, consent: form.consent, description: form.description.trim(),
      ...
      const term: TermsDefinition = { id, name: form.name.trim(), code: form.code.trim(), type: form.type, consent: form.consent, description: form.description.trim(), versions: [version], agreement: { target: 0, agreed: 0 }, history: [{ at: timestamp, actor: 'admin01', action: status === '임시저장' ? '임시저장' : `${version.version} 최초 등록`, detail: form.changeReason.trim() || `${form.effectiveFrom || '미정'} 시행` }] };
  ...
  const rows: GridRow[] = filtered.map((term) => {
    ...
    return { id: term.id, onClick: () => setDetailId(term.id), cells: [
      ...
      { kind: 'rowMenu', align: 'right', detailLabel: '상세', onDetail: () => setDetailId(term.id), open: menuId === term.id, onToggle: () => setMenuId(menuId === term.id ? null : term.id), items: [
        { label: '상세 보기', click: () => setDetailId(term.id) },
  ...
  return <section className={shared.page} onClick={() => menuId && setMenuId(null)}>
    <div className={shared.headTop}>
      ...
      <div className={shared.quickFilters}>{QUICK.map((item) => <button key={item} type="button" className={`${shared.qfBtn} ${quick === item ? styles.quickActive : ''}`} onClick={() => setQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{terms.filter((term) => quickMatch(term, item)).length}</span></button>)}</div>
      <div className={shared.filterBox}>
        <form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm}><option>전체 검색</option><option>약관명</option><option>약관 코드</option><option>버전</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="약관명 / 약관 코드 / 버전"/><button type="submit" className={shared.searchBtn}>조회</button></form>
        <div className={shared.filterRow2}><select className={shared.selectSm} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="">전체 유형</option>{TYPES.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">전체 상태</option>{Object.keys(STATUS_META).map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={consentFilter} onChange={(event) => setConsentFilter(event.target.value)}><option value="">전체 동의 구분</option><option>필수</option><option>선택</option><option>동의 불필요</option></select><label className={styles.checkRow}>시행일 <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)}/><span>–</span><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)}/></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={resetFilters}>초기화</button></div>
      </div>
    </div>
    <div className={shared.gridWrap}><div className={shared.resultRow}><span className={shared.resultLabel}>총 {filtered.length}개 약관 · 코드 단위</span><div className={shared.resultActions}><select className={shared.pageSizeSelect}><option>20개씩</option><option>50개씩</option></select></div></div><DataGrid columns={COLUMNS} rows={rows} gridTemplate="minmax(205px,1.7fr) 90px 65px 95px 88px 100px 100px 105px 88px" minWidth="1030px" empty={filtered.length === 0} emptyText="검색 조건에 해당하는 약관이 없습니다." emptySubtext="검색어나 필터 조건을 변경하거나 새 약관을 등록해 주세요." emptyActionLabel="약관 등록" emptyActionClick={() => openCreate()} showPagination pages={[{ label: '‹' }, { label: '1', active: true }, { label: '›' }]} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'}/></div>
