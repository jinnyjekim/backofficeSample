...
import { currentPolicyVersion, policyScopes, type PolicyDefinition, type PolicyStatus } from './policyData';
...
export function PolicyDetailDrawer({ policy, onClose, onNewVersion, onEdit, onEnd, onViewVersion }: Props) {
  ...
  return <aside ref={asideRef} className={`${drawer.aside} ${styles.detailDrawer}`}>
    ...
    <div className={drawer.scroll}>
      {tab === 'basic' && <><div className={drawer.fieldBox}><Info label="정책명" value={policy.name}/><Info label="정책 코드" value={policy.code}/><Info label="적용 범위" value={policyScopes(policy).join(' · ')}/><Info label="정책 유형" value={policy.type}/><Info label="현재 버전" value={version.version}/><Info label="공개 범위" value={policy.visibility}/><Info label="상태" value={version.status}/><Info label="적용 시작일" value={version.effectiveFrom || '-'}/><Info label="적용 종료일" value={version.effectiveTo ?? '-'}/></div><div className={drawer.bodyText}>{policy.description || '등록된 설명이 없습니다.'}</div></>}
