...
const COLUMNS = [{ label: '정책명 / 코드' }, { label: '적용 범위' }, { label: '정책 유형' }, { label: '버전' }, { label: '공개 여부' }, { label: '상태' }, { label: '적용 시작일' }, { label: '적용 종료일' }, { label: '최근 수정일' }];
...
export function PolicyManagementPage() {
  ...
  function notify(message: string) { showToast({ message, type: 'success' }); }
  function resetFilters() { setQuick('전체'); setScopeFilter('통합'); setKeyword(''); setSearch(''); setTypeFilter(''); setStatusFilter(''); setVisibilityFilter(''); setDateFrom(''); setDateTo(''); }
  ...
  const rows: GridRow[] = filtered.map((policy) => {
    const version = currentPolicyVersion(policy); const meta = STATUS_META[version.status]; const scopes = policyScopes(policy); const scopeMeta = scopes.length === 1 ? CONFIG_SCOPE_BADGE_META[scopes[0]] : CONFIG_SCOPE_BADGE_META.공통;
  ...
  return <div className={shared.page} onClick={() => menuId && setMenuId(null)}>
    <header className={shared.header}>
      <div className={shared.headerTop}><div><div className={shared.title}>정책 관리</div><div className={shared.subtitle}>서비스 운영에 필요한 공개·내부 정책 문서를 등록하고 개정 및 적용 이력을 관리합니다.</div></div><button type="button" className={shared.createBtn} onClick={() => openCreate()}>+ 정책 등록</button></div>
      
      <div className={shared.filterBox}><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="정책명 / 정책 코드 / 정책 내용"/><button type="submit" className={shared.searchBtn}>조회</button><div className={shared.quickFilters}>
        {QUICK.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={shared.qfCount}>{policies.filter((policy) => matchesConfigScope(policyScopes(policy), scopeFilter) && quickMatch(policy, item)).length}</span>
            </CommonButton>
          );
        })}
      </div></form><div className={shared.filterRow2}><label className="globalFilterField"><span>적용 범위</span><select className={shared.selectSm} value={scopeFilter} onChange={(event) => setScopeFilter(event.target.value as ConfigScopeFilter)} aria-label="적용 범위">{CONFIG_SCOPE_FILTERS.map((item) => <option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>정책 유형</span><select aria-label="정책 유형" className={shared.selectSm} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="">전체 정책 유형</option>{types.map((item) => <option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>상태</span><select aria-label="상태" className={shared.selectSm} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">전체 상태</option>{Object.keys(STATUS_META).map((item) => <option key={item}>{item}</option>)}</select></label><label className="globalFilterField"><span>공개 범위</span><select aria-label="공개 범위" className={shared.selectSm} value={visibilityFilter} onChange={(event) => setVisibilityFilter(event.target.value)}><option value="">전체 공개 범위</option><option>공개</option><option>내부용</option></select></label><label className={shared.dateFilterField}><span>적용일</span><span className={shared.dateRange}><DatePicker value={dateFrom} onChange={(event) => setDateFrom(event.target.value)}/><span className={shared.dateSeparator} aria-hidden="true">~</span><DatePicker value={dateTo} onChange={(event) => setDateTo(event.target.value)}/></span></label><span className={shared.rowSpacer}/><button type="button" className="detailFilterBtn">상세 필터</button><button type="button" className={shared.resetBtn} onClick={resetFilters}>초기화</button></div></div>
    </header>
    <div className={shared.gridWrap}><div className={shared.resultRow}><span className={shared.resultLabel}>총 {filtered.length}개 정책 · 문서 코드 단위</span><div className={shared.resultActions}><ExcelDownloadButton type="button" data-grid-download /><PageSizeSelect className={shared.pageSizeSelect} defaultValue="20개씩 보기"><option>20개씩 보기</option><option>50개씩 보기</option></PageSizeSelect></div></div><DataGrid columns={COLUMNS} rows={rows} gridTemplate="minmax(205px,1.7fr) 114px 80px 60px 62px 76px 88px 88px 94px" minWidth="995px" empty={filtered.length === 0} emptyText="검색 조건에 해당하는 정책이 없습니다." emptySubtext="검색어나 필터 조건을 변경하거나 새 정책을 등록해 주세요." emptyActionLabel="정책 등록" emptyActionClick={() => openCreate()} showPagination pages={[{ label: '‹' }, { label: '1', active: true }, { label: '›' }]} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'}/></div>
