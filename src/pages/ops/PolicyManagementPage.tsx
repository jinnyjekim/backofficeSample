...
import { POLICIES, currentPolicyVersion, type PolicyDefinition, type PolicyStatus, type PolicyVersion, type PolicyVisibility } from './policyData';
...
type FormState = { name: string; code: string; type: string; version: string; visibility: PolicyVisibility; effectiveFrom: string; effectiveTo: string; description: string; content: string; changeReason: string };
...
const COLUMNS = [{ label: '정책명 / 코드' }, { label: '정책 유형' }, { label: '버전' }, { label: '공개 여부' }, { label: '상태' }, { label: '적용 시작일' }, { label: '적용 종료일' }, { label: '최근 수정일' }, { label: '관리', align: 'right' as const }];
const emptyForm = (): FormState => ({ name: '', code: '', type: '서비스 운영', version: 'v1.0', visibility: '공개', effectiveFrom: '', effectiveTo: '', description: '', content: '', changeReason: '' });
...
export function PolicyManagementPage() {
  ...
  }), [dateFrom, dateTo, policies, quick, search, searchField, statusFilter, typeFilter, visibilityFilter]);
  ...
  function resetFilters() { setQuick('전체'); setSearchField('전체'); setKeyword(''); setSearch(''); setTypeFilter(''); setStatusFilter(''); setVisibilityFilter(''); setDateFrom(''); setDateTo(''); }
  ...
  function openCreate(source?: PolicyDefinition) {
    ...
    setForm({ name: `[복사본] ${source.name}`, code, type: source.type, version: 'v1.0', visibility: source.visibility, effectiveFrom: '', effectiveTo: '', description: source.description, content: version.content, changeReason: '기존 정책 복제' });
  ...
  function openEdit(policy: PolicyDefinition) {
    ...
    setForm({ name: policy.name, code: policy.code, type: policy.type, version: version.version, visibility: policy.visibility, effectiveFrom: version.effectiveFrom, effectiveTo: version.effectiveTo ?? '', description: policy.description, content: version.content, changeReason: version.changeReason });
    setEditor({ mode: 'edit', policyId: policy.id }); setDetailId(null);
  }

  function openNewVersion(policy: PolicyDefinition) {
    const version = currentPolicyVersion(policy);
    setForm({ name: policy.name, code: policy.code, type: policy.type, version: '', visibility: policy.visibility, effectiveFrom: '', effectiveTo: '', description: policy.description, content: version.content, changeReason: '' });
  ...
  function save(mode: 'draft' | 'publish') {
    ...
    if (editor.mode === 'edit' && editor.policyId) {
      ...
      setPolicies((items) => items.map((policy) => policy.id !== editor.policyId ? policy : { ...policy, name: form.name.trim(), type: form.type.trim(), visibility: form.visibility, description: form.description.trim(), versions: [version, ...policy.versions.map((item) => mode === 'publish' && item.status === '적용중' ? { ...item, status: '종료' as PolicyStatus, effectiveTo: dayBefore(form.effectiveFrom) } : item)], history: [{ at: timestamp, actor: 'admin01', action: status === '임시저장' ? `${form.version.trim()} 임시저장` : `${form.version.trim()} 신규 등록`, detail: mode === 'publish' ? `${form.effectiveFrom} 시행 · 기존 적용 버전 자동 종료` : form.changeReason.trim() }, ...policy.history] }));
      setDetailId(editor.policyId);
    } else {
      ...
      setPolicies((items) => [{ id, name: form.name.trim(), code: form.code.trim(), type: form.type.trim(), visibility: form.visibility, description: form.description.trim(), versions: [version], history: [{ at: timestamp, actor: 'admin01', action: status === '임시저장' ? '임시저장' : `${version.version} 최초 등록`, detail: form.changeReason.trim() || `${form.effectiveFrom || '미정'} 시행` }] }, ...items]);
      setDetailId(id);
    }
  ...
  const rows: GridRow[] = filtered.map((policy) => {
    const version = currentPolicyVersion(policy); const meta = STATUS_META[version.status]; const hasActive = policy.versions.some((item) => item.status === '적용중'); const draftOnly = policy.versions.every((item) => item.status === '임시저장');
    return { id: policy.id, onClick: () => setDetailId(policy.id), cells: [
      { kind: 'stack', title: policy.name, subtitle: policy.code }, { kind: 'text', text: policy.type, size: '11.5px' }, { kind: 'link', text: version.version },
      ...
      { kind: 'rowMenu', align: 'right', detailLabel: '상세', onDetail: () => setDetailId(policy.id), open: menuId === policy.id, onToggle: () => setMenuId(menuId === policy.id ? null : policy.id), items: [{ label: '상세 보기', click: () => setDetailId(policy.id) }, ...(version.status === '임시저장' ? [{ label: '수정', click: () => openEdit(policy) }] : [{ label: '새 버전 등록', click: () => openNewVersion(policy) }]), { label: '복제', click: () => openCreate(policy) }, { label: '정책 미리보기', click: () => viewVersion(policy, version.id) }, ...(hasActive ? [{ sep: true }, { label: '적용 종료', fg: '#dc2626', click: () => setEndId(policy.id) }] : []), ...(draftOnly ? [{ sep: true }, { label: '삭제', fg: '#dc2626', click: () => setDeleteId(policy.id) }] : [])] },
    ] };
  });
  ...
  return <section className={shared.page} onClick={() => menuId && setMenuId(null)}>
    <div className={shared.headTop}>
      ...
      <div className={shared.quickFilters}>{QUICK.map((item) => <button key={item} type="button" className={`${shared.qfBtn} ${quick === item ? styles.quickActive : ''}`} onClick={() => setQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{policies.filter((policy) => quickMatch(policy, item)).length}</span></button>)}</div>
      <div className={shared.filterBox}><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm} value={searchField} onChange={(event) => setSearchField(event.target.value)}><option>전체</option><option>정책명</option><option>정책 코드</option><option>정책 내용</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="정책명 / 정책 코드 / 정책 내용"/><button type="submit" className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="">전체 정책 유형</option>{types.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">전체 상태</option>{Object.keys(STATUS_META).map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={visibilityFilter} onChange={(event) => setVisibilityFilter(event.target.value)}><option value="">전체 공개 범위</option><option>공개</option><option>내부용</option></select><label className={styles.checkRow}>적용일 <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)}/><span>–</span><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)}/></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={resetFilters}>초기화</button></div></div>
    </div>
    <div className={shared.gridWrap}><div className={shared.resultRow}><span className={shared.resultLabel}>총 {filtered.length}개 정책 · 문서 코드 단위</span><div className={shared.resultActions}><select className={shared.pageSizeSelect}><option>20개씩</option><option>50개씩</option></select></div></div><DataGrid columns={COLUMNS} rows={rows} gridTemplate="minmax(205px,1.7fr) 100px 65px 85px 88px 100px 100px 105px 88px" minWidth="1040px" empty={filtered.length === 0} emptyText="검색 조건에 해당하는 정책이 없습니다." emptySubtext="검색어나 필터 조건을 변경하거나 새 정책을 등록해 주세요." emptyActionLabel="정책 등록" emptyActionClick={() => openCreate()} showPagination pages={[{ label: '‹' }, { label: '1', active: true }, { label: '›' }]} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'}/></div>
