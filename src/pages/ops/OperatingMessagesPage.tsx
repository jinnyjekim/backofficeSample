...
const COLUMNS = [{ label: '메시지명 / ID' }, { label: '사용자 노출 제목' }, { label: '유형' }, { label: '채널' }, { label: '대상' }, { label: '상태' }, { label: '발송 예정일' }, { label: '발송 완료일' }, { label: '관리', align: 'right' as const }];
...
export function OperatingMessagesPage() {
  ...
  const rows: GridRow[] = filtered.map((message) => { const meta = STATUS_META[message.status]; const sendDate = message.sentAt ?? message.scheduledAt ?? '-'; return { id: message.id, onClick: () => setDetailId(message.id), cells: [
    ...
    { kind: 'rowMenu', align: 'right', detailLabel: '상세', onDetail: () => setDetailId(message.id), open: menuId === message.id, onToggle: () => setMenuId(menuId === message.id ? null : message.id), items: [{ label: '상세 보기', click: () => setDetailId(message.id) }, ...(['작성중', '발송 예정'].includes(message.status) ? [{ label: '수정', click: () => openEdit(message) }] : []), { label: '복제', click: () => openCreate(message) }, { label: '미리보기', click: () => showPreview(message) }, ...(['실패', '일부 실패'].includes(message.status) ? [{ label: '실패 대상 재발송', click: () => retry(message) }] : []), ...(['발송 예정', '발송중'].includes(message.status) ? [{ sep: true }, { label: message.status === '발송 예정' ? '발송 취소' : '발송 중지', fg: '#dc2626', click: () => setCancelId(message.id) }] : [])] },
  ] }; });
  ...
  return <section className={shared.page} onClick={() => menuId && setMenuId(null)}>
    <div className={shared.headTop}><div className={shared.headRow}><div><h1 className={shared.title}>운영 메시지</h1><p className={shared.subtitle}>서비스 사용자에게 전달할 운영 안내를 작성하고 대상·채널·발송 일정 및 결과를 관리합니다.</p></div><button type="button" className={shared.createBtn} onClick={() => openCreate()}>+ 메시지 등록</button></div>
      <div className={shared.quickFilters}>{QUICK.map((item) => <button key={item} type="button" className={`${shared.qfBtn} ${quick === item ? styles.quickActive : ''}`} onClick={() => setQuick(item)}><span className={shared.qfLabel}>{item}</span><span className={shared.qfCount}>{messages.filter((message) => quickMatch(message, item)).length}</span></button>)}</div>
      <div className={shared.filterBox}><form className={shared.filterRow1} onSubmit={(event) => { event.preventDefault(); setSearch(keyword.trim()); }}><select className={shared.selectSm} value={searchField} onChange={(event) => setSearchField(event.target.value)}><option>전체</option><option>메시지 ID</option><option>메시지명</option><option>제목</option><option>등록자</option></select><input className={shared.searchInput} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="메시지 ID / 메시지명 / 제목 / 등록자"/><button type="submit" className={shared.searchBtn}>조회</button></form><div className={shared.filterRow2}><select className={shared.selectSm} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="">전체 메시지 유형</option>{types.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">전체 발송 상태</option>{Object.keys(STATUS_META).map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={channelFilter} onChange={(event) => setChannelFilter(event.target.value)}><option value="">전체 발송 채널</option>{CHANNELS.map((item) => <option key={item}>{item}</option>)}</select><select className={shared.selectSm} value={targetFilter} onChange={(event) => setTargetFilter(event.target.value)}><option value="">전체 대상 유형</option><option>전체 사용자</option><option>조건 지정</option><option>개별 지정</option></select><label className={styles.radioRow}>발송일 <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)}/><span>–</span><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)}/></label><span className={shared.rowSpacer}/><button type="button" className={shared.resetBtn} onClick={resetFilters}>초기화</button></div></div>
    </div>
    <div className={shared.gridWrap}><div className={shared.resultRow}><span className={shared.resultLabel}>총 {filtered.length}개 메시지</span><div className={shared.resultActions}><select className={shared.pageSizeSelect}><option>20개씩</option><option>50개씩</option></select></div></div><DataGrid columns={COLUMNS} rows={rows} gridTemplate="minmax(190px,1.5fr) minmax(170px,1.3fr) 72px 125px 110px 88px 125px 125px 88px" minWidth="1150px" empty={filtered.length === 0} emptyText="검색 조건에 해당하는 운영 메시지가 없습니다." emptySubtext="검색어나 필터 조건을 변경하거나 새 메시지를 등록해 주세요." emptyActionLabel="메시지 등록" emptyActionClick={() => openCreate()} showPagination pages={[{ label: '‹' }, { label: '1', active: true }, { label: '›' }]} rangeLabel={filtered.length ? `1–${filtered.length} / ${filtered.length}` : '0건'}/></div>
