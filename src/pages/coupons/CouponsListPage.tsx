...
export function CouponsListPage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = coupons.filter((cp) => matchesQuickFilter(cp, f)).length;
    });
    return c;
  }, [coupons]);
  ...
  const filtered = useMemo(
    () =>
      coupons.filter((c) => {
        ...
        if (issueMethodFilter && c.issueMethod !== issueMethodFilter)
          return false;
  ...
  const selected = drawerId
    ? (coupons.find((c) => c.id === drawerId) ?? null)
    : null;
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "stop") {
      ...
      const updated = history(
        ...
        confirm.item.active ? "쿠폰 비활성화" : "쿠폰 활성화",
      );
      setCoupons((prev) =>
        prev.map((c) => (c.id === updated.id ? updated : c)),
      );
      ...
      setCoupons((prev) =>
        prev.map((c) =>
          selectedIds.includes(c.id)
            ? history({ ...c, active: false }, "쿠폰 비활성화 (일괄)")
            : c,
        ),
      );
  ...
  function rowMenuItems(c: Coupon, status: ReturnType<typeof computeStatus>) {
    ...
    if (status !== "발급 종료")
      items.push({ label: "수정", click: () => openEditor(c) });
    ...
    if (status === "발급중")
      items.push({
        label: "발급 중지",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "stop", item: c }),
      });
    if (status === "발급 예정" || status === "비활성")
      items.push({
        label: c.active ? "비활성화" : "활성화",
        click: () => setConfirm({ kind: "toggleActive", item: c }),
      });
    if (c.issuedCount === 0)
      items.push({
        label: "삭제",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "delete", item: c }),
      });
    return items;
  }
  ...
  const rows: GridRow[] = filtered.map((c) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: `발급 ${issuePeriodSummary(c)} · ${validitySummary(c)}`,
        color: "#71717a",
        size: "11px",
        weight: 500,
      },
      {
        kind: "text",
        text: `${c.issuedCount.toLocaleString("ko-KR")} / ${c.usedCount.toLocaleString("ko-KR")}`,
        color: "#3f3f46",
        size: "12px",
        weight: 600,
        align: "right",
        ...
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              쿠폰의 할인 조건, 발급 및 사용 가능 상태를 관리합니다.
            </div>
          </div>
          <button
            type="button"
          ...
          <div className={shared.filterRow2}>
            <label className="globalFilterField">
              ...
              <select
                ...
                onChange={(e) =>
                  setApplyUnitFilter(e.target.value as CouponApplyUnit | "")
                }
              >
            ...
            <label className="globalFilterField">
              ...
              <select
                aria-label="발급방식"
                className={shared.selectSm}
                value={issueMethodFilter}
                onChange={(e) =>
                  setIssueMethodFilter(e.target.value as IssueMethod | "")
                }
              >
        ...
        <DataGrid
          ...
          emptyText={
            coupons.length === 0
              ? "등록된 쿠폰이 없습니다."
              : quickFilter === "검토 필요"
                ? "현재 확인이 필요한 쿠폰 설정이 없습니다."
                : "검색 조건에 해당하는 쿠폰이 없습니다."
          }
          emptySubtext={
            coupons.length === 0
              ? "할인 쿠폰을 등록해 보세요."
          ...
          emptyActionLabel={coupons.length === 0 ? "+ 쿠폰 등록" : "초기화"}
          emptyActionClick={
            coupons.length === 0 ? () => openEditor("new") : resetFilters
          }
        />
      </div>
          ...
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>현재 발급</span>
                <span>
                  {confirm.item.issuedCount.toLocaleString("ko-KR")}장
                </span>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>사용</span>
                <span>{confirm.item.usedCount.toLocaleString("ko-KR")}장</span>
              </div>
            </div>
            <div className={shared.dialogBody} style={{ marginBottom: 0 }}>
              신규 발급은 중단되며, 이미 발급된 쿠폰의 사용 가능 여부는 기존
              유효기간 정책을 따릅니다.
            </div>
      ...
      {confirm?.kind === "delete" && (
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div
              className={shared.dialogBody}
            >{`'${confirm.item.name}'이(가) 삭제됩니다. 발급 이력이 없는 쿠폰만 삭제할 수 있습니다.`}</div>
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogBody}>
              {confirm.item.active
                ? "비활성화하면 발급 기간과 관계없이 신규 발급이 즉시 중단됩니다."
                : "활성화하면 발급 기간 조건에 따라 다시 발급될 수 있습니다."}
            </div>
            <div className={shared.dialogActions}>
              <button
                type="button"
