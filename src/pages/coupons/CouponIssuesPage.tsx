...
import {
  CouponIssueFormDrawer,
  type CouponIssueFormData,
} from "./CouponIssueFormDrawer";
...
export function CouponIssuesPage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = issues.filter((h) => matchesQuickFilter(h, f)).length;
    });
    return c;
  }, [issues]);
  ...
  const filtered = useMemo(
    () =>
      issues.filter((h) => {
        ...
        if (
          search &&
          !`${h.member} ${h.couponNameSnapshot} ${h.couponCode} ${h.id}`
            .toLowerCase()
  ...
  function toggleSelAll() {
    setSelectedIds((prev) =>
      prev.length === filtered.length ? [] : filtered.map((h) => h.id),
    );
  }

  const selected = drawerId
    ? (issues.find((h) => h.id === drawerId) ?? null)
    : null;
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "revoke") {
      ...
      const eligible = selectedIds.filter(
        (id) => computeStatus(issues.find((h) => h.id === id)!) === "사용 가능",
      );
      eligible.forEach((id) =>
        revoke(
          issues.find((h) => h.id === id)!,
          revokeReason,
          revokeDetail.trim(),
        ),
      );
  ...
  const bulkRevokeEligible = selectedIds.filter(
    (id) => computeStatus(issues.find((h) => h.id === id)!) === "사용 가능",
  );
  ...
  const rows: GridRow[] = filtered.map((h) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "titleWarn",
        title: `${h.couponNameSnapshot} · ${h.couponCode}`,
        hasIssue: issueList.length > 0,
        issueTitle: issueList.join(" · "),
      },
      {
        kind: "text",
        text: h.issuedAt.slice(5, 10).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
      ...
      {
        kind: "text",
        text: h.expiresAt.slice(5, 10).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
      ...
      {
        kind: "text",
        text: h.usedAt ? h.usedAt.slice(5, 10).replace("-", ".") : "-",
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
        ...
        <div className={shared.filterCard}>
          ...
          >
            <input
              ...
              placeholder="회원, 쿠폰명, 쿠폰코드 또는 발급번호 검색"
            />
          ...
          <div className={shared.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as HolderStatus | "")
                }
              >
        ...
        {selectedIds.length > 0 && (
          <div className={shared.bulkBar}>
            ...
            <button
              ...
              onClick={() => setConfirm({ kind: "bulkRevoke" })}
            >
              쿠폰 회수
            </button>
            <button
        ...
        <DataGrid
          ...
          emptyText={
            issues.length === 0
              ? "발급된 쿠폰이 없습니다."
              : quickFilter === "확인 필요"
                ? "현재 확인이 필요한 발급 쿠폰이 없습니다."
                : "검색 조건에 해당하는 발급 쿠폰이 없습니다."
          }
          emptySubtext={
            issues.length === 0
              ? "회원에게 쿠폰이 발급되면 이곳에서 확인할 수 있습니다."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={issues.length === 0 ? "+ 쿠폰 발급" : "초기화"}
          emptyActionClick={
            issues.length === 0 ? () => setShowIssueForm(true) : resetFilters
          }
        />
      </div>
          ...
          <div className={shared.dialogBox}>
            ...
            {confirm.kind === "revoke" ? (
              <div className={shared.dialogSummary}>
                ...
                <div className={shared.dialogSummaryRow}>
                  <span>쿠폰</span>
                  <span>{confirm.item.couponNameSnapshot}</span>
                </div>
                <div className={shared.dialogSummaryRow}>
                  <span>만료일</span>
                  <span>{confirm.item.expiresAt.slice(0, 10)}</span>
                </div>
              </div>
            ) : (
              <div className={shared.dialogSummary}>
                <div className={shared.dialogSummaryRow}>
                  <span>회수 가능</span>
                  <span>{bulkRevokeEligible.length}건</span>
                </div>
                <div className={shared.dialogSummaryRow}>
                  <span>제외 (사용완료/만료/회수됨)</span>
                  <span>
                    {selectedIds.length - bulkRevokeEligible.length}건
                  </span>
                </div>
              </div>
            )}
            ...
            <div className={shared.dialogActions}>
              ...
              <button
                ...
                disabled={
                  confirm.kind === "bulkRevoke" &&
                  bulkRevokeEligible.length === 0
                }
