...
export function CouponUsagePage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = usages.filter((u) => matchesQuickFilter(u, f)).length;
    });
    return c;
  }, [usages]);
  ...
  const filtered = useMemo(
    () =>
      usages.filter((u) => {
        ...
        if (
          search &&
          !`${u.orderId} ${u.member} ${u.couponNameSnapshot} ${u.id}`
            .toLowerCase()
  ...
  const selected = drawerId
    ? (usages.find((u) => u.id === drawerId) ?? null)
    : null;
  ...
  const rows: GridRow[] = filtered.map((u) => {
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "titleWarn",
        title: `${u.couponNameSnapshot} · ${u.couponCode}`,
      ...
      },
      {
        kind: "text",
        text:
          u.currentDiscountAmount < 0
            ? fmtWon(u.currentDiscountAmount)
            : `-${fmtWon(u.currentDiscountAmount)}`,
        color: u.currentDiscountAmount < 0 ? "#dc2626" : "#18181b",
        size: "12.5px",
        weight: 700,
        align: "right",
        numeric: true,
      ...
      {
        kind: "text",
        text: u.usedAt.slice(5, 10).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
        ...
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              주문에서 실제 사용된 쿠폰과 할인 처리 결과를 조회합니다.
            </div>
          </div>
        </div>
        ...
        <div className={shared.filterCard}>
          ...
          >
            <input
              ...
              placeholder="주문번호, 회원, 쿠폰명 또는 사용번호 검색"
            />
          ...
          <div className={shared.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as UsageStatus | "")
                }
              >
        ...
        <DataGrid
          ...
          emptyText={
            usages.length === 0
              ? "쿠폰 사용 내역이 없습니다."
              : quickFilter === "확인 필요"
                ? "현재 확인이 필요한 쿠폰 사용 내역이 없습니다."
                : "검색 조건에 해당하는 쿠폰 사용 내역이 없습니다."
          }
          emptySubtext={
            usages.length === 0
              ? "회원이 쿠폰을 주문에 사용하면 이곳에서 확인할 수 있습니다."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={usages.length > 0 ? "초기화" : undefined}
          emptyActionClick={usages.length > 0 ? resetFilters : undefined}
        />
      </div>

      {selected && (
        <CouponUsageDetailDrawer
          key={selected.id}
          usage={selected}
