...
export function RefundPage() {
  ...
  const drawerItem = drawerId
    ? (refunds.find((r) => r.id === drawerId) ?? null)
    : null;
      ...
      history(
        {
          ...
          status: "완료",
          completedAt: NOW,
          externalRefundNo: `R-${id.replace("REF-", "")}`,
        },
        "환불 완료",
        "admin01",
      ),
  ...
  const addMemo = (id: string, text: string) => {
    update(id, (r) => ({
      ...
      memos: [
        ...r.memos,
        { id: `M-${Date.now()}`, at: NOW, by: "admin01", text },
      ],
    }));
  };
  ...
  const runBulkReviewStart = () => {
    const targets = [...selected].filter(
      (id) => refunds.find((r) => r.id === id)?.status === "요청",
    );
    if (targets.length === 0) {
          ...
          ? history(
              {
                ...r,
                status: "검토중",
                owner: r.owner === "미배정" ? "admin01" : r.owner,
              },
              "검토 시작 (일괄)",
              "admin01",
            )
      ...
      history(
        {
          ...
          approvedAt: NOW,
          approvedBy: "admin01",
          approvalMemo: approveMemo.trim() || undefined,
        },
        "환불 승인",
        "admin01",
      ),
  ...
  const runReconsider = () => {
    ...
    update(id, (r) =>
      history(
        ...
        "admin01",
        undefined,
        reconsiderReason.trim() || undefined,
      ),
    );
  ...
  const rows: GridRow[] = filtered.map((r) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: r.refundType === "전체 환불" ? "전체" : "부분",
          bg: r.refundType === "전체 환불" ? "#eef2ff" : "#f4f4f5",
          fg: r.refundType === "전체 환불" ? "#4338ca" : "#71717a",
        },
        {
          kind: "text",
          text: fmtWon(itemsAmount(r.items)),
          size: "12px",
          align: "right",
          numeric: true,
          color: "#3f3f46",
        ...
        {
          kind: "text",
          text: r.requestedAt.slice(5, 10).replace("-", "."),
          size: "11.5px",
          color: "#71717a",
          numeric: true,
        },
        {
          ...
          text: r.owner,
          size: "12px",
          color: r.owner === "미배정" ? "#d97706" : "#3f3f46",
        },
        {
          kind: "rowMenu",
          align: "right",
          ...
          items: [
            ...
            ...(r.status === "요청"
              ? [
                  { label: "검토 시작", click: () => startReview(r.id) },
                  {
                    label: "담당자 지정",
                    click: () => setModal({ kind: "assign", ids: [r.id] }),
                  },
                ]
              : []),
            ...(r.status === "검토중"
              ? [
                  {
                    label: "승인",
                    click: () => setModal({ kind: "approve", item: r }),
                  },
                  {
                    label: "반려",
                    fg: "#dc2626",
                    click: () => setModal({ kind: "reject", item: r }),
                  },
                ]
              : []),
            ...(r.status === "승인"
              ? [{ label: "환불 실행", click: () => execute(r.id) }]
              : []),
            ...(r.status === "처리중"
              ? [{ label: "상태 재조회", click: () => poll(r.id) }]
              : []),
            ...(r.status === "실패"
              ? [
                  { label: "상태 재조회", click: () => poll(r.id) },
                  {
                    label: "재시도",
                    click: () => setModal({ kind: "retry", item: r }),
                  },
                ]
              : []),
            ...(r.status === "반려"
              ? [
                  {
                    label: "재검토",
                    click: () => setModal({ kind: "reconsider", item: r }),
                  },
                ]
              : []),
          ],
    ...
    >
                ...
                return (
                  ...
                  >
                    ...
                    <span className={shared.qfCount}>
                      {
                        refunds.filter((r) => matchesQuickFilter(r, filter))
                          .length
                      }
                    </span>
                  </CommonButton>
          ...
          <div className={shared.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={typeFilter}
                onChange={(e) =>
                  setTypeFilter(e.target.value as RefundType | "")
                }
              >
      ...
      {selected.size > 0 && (
        <div className={shared.bulkBar}>
          ...
          <button
            ...
            onClick={() => setModal({ kind: "assign", ids: [...selected] })}
          >
            담당자 지정
          </button>
          <button
      ...
      <div className={shared.gridWrap} style={{ marginTop: 0 }}>
        <DataGrid
          columns={[
            ...
            { label: "고객" },
            { label: "유형" },
            { label: "요청금액", align: "right" as const },
            { label: "환불금액", align: "right" as const },
            { label: "상태" },
            { label: "요청일" },
            { label: "담당자" },
      ...
      {drawerItem && (
        <RefundDrawer
          ...
          onAddAdjustment={(type, amount, reason) =>
            addAdjustment(drawerItem.id, type, amount, reason)
          }
          ...
          onAssignClick={() =>
            setModal({ kind: "assign", ids: [drawerItem.id] })
          }
          ...
          onReconsiderClick={() =>
            setModal({ kind: "reconsider", item: drawerItem })
          }
        />
      )}
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <p className={shared.dialogBody}>
              선택한 {modal.ids.length}건에 담당자를 지정합니다.
            </p>
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogSummary}>
              ...
              <div className={shared.dialogSummaryRow}>
                <span>환불유형</span>
                <strong>{modal.item.refundType}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>최종 환불금액</span>
                <strong>{fmtWon(computeFinalAmount(modal.item))}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>환불수단</span>
                <strong>{modal.item.paymentMethod}</strong>
              </div>
            </div>
            <p className={shared.dialogBody}>
              승인 후 '환불 실행'을 눌러야 실제 결제수단으로 환불 요청이
              전송됩니다.
            </p>
            <label className={styles.formField}>
              <span>관리자 메모</span>
            ...
            <div className={shared.dialogActions}>
              ...
              <button
                ...
                disabled={!canApprove(modal.item, refunds)}
                onClick={runApprove}
              >
                환불 승인
              </button>
          ...
          <div className={shared.dialogBox}>
            ...
            <label className={styles.formField}>
              ...
              <textarea
                ...
                placeholder="고객 안내에 사용될 상세 사유를 입력하세요"
              />
            </label>
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>기존 실패</span>
                <strong>{modal.item.failCode}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>환불금액</span>
                <strong>{fmtWon(computeFinalAmount(modal.item))}</strong>
      ...
      {modal?.kind === "reconsider" && (
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <p className={shared.dialogBody}>
              반려된 환불 건을 다시 검토중 상태로 전환합니다. 변경 사유를
              남겨주세요.
            </p>
            <label className={styles.formField}>
              <span>변경 사유</span>
