...
export function ReturnCollectedPage() {
  ...
  const collectedItems = useMemo(
    () => items.filter((it) => it.stage === "회수 완료"),
    [items],
  );
  ...
  const handleStartInspection = (id: string) => {
    ...
    showToast({
      message: "상품 검수 대기열로 배정되었습니다. (상품 확인 메뉴로 이동)",
      type: "success",
    });
    setSelectedId(null);
  };
    ...
    <div className={styles.page}>
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              물류센터에 입고 완료된 반품 상품을 조회하고 실물 검수 대기열을
              관리합니다.
            </div>
          </div>
        </div>
      ...
      {selected && (
        ...
        >
          ...
          >
            물류센터 검수 담당자에게 상품이 전달되었습니다. 외관 손상 및 구성품
            유무를 확인한 뒤 [상품 검수 시작]을 진행하세요.
          </div>
        </DetailDrawer>
      )}
    </div>
