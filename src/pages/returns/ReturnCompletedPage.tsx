...
export function ReturnCompletedPage() {
  ...
  const completedItems = useMemo(() => items.filter((it) => it.stage === '반품 완료'), [items]);
      ...
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>검수가 통과되어 결제 취소 및 환불 처리가 정상 완료된 반품 목록을 조회합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 반품 완료`}</span>
      ...
      {selected && (
        <DetailDrawer
          ...
          stats={[
            ...
            { label: '실제 환불액', value: `${selected.refundAmount.toLocaleString()}원` },
            { label: '검수 판정', value: selected.inspectionResult ?? '정상 양품' },
          ]}
          fields={[
            { label: '원 결제 금액', value: `${selected.amount.toLocaleString()}원` },
            { label: '차감 배송비', value: selected.deductFee > 0 ? `${selected.deductFee.toLocaleString()}원` : '0원 (면제)' },
            { label: '반품 사유', value: `${selected.reasonCategory} - ${selected.reasonDetail}` },
            { label: '환불 수단', value: '원결제 수단 부분 취소 (PG 자동 연동)' },
          ]}
        />
      )}
    </div>
