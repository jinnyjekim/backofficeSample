...
const GRID_TEMPLATE = "140px 140px 90px 100px minmax(200px, 1fr) 110px 120px";
...
export function ReturnApprovalPage() {
  ...
  const approvalItems = useMemo(
    () => items.filter((it) => it.stage === "반품 승인"),
    [items],
  );
  ...
  const handleStartCollecting = (id: string) => {
    ...
    showToast({
      message: "택배사 회수 지시가 전송되었습니다. (반품 회수 메뉴로 이동)",
      type: "success",
    });
    setSelectedId(null);
  };
    ...
    <div className={styles.page}>
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              승인 완료된 반품 건에 대해 택배사 수거 연동 및 회수 송장을
              발행합니다.
            </div>
          </div>
        </div>
      ...
      {selected && (
        ...
        >
          ...
          >
            [택배사 회수 지시] 버튼을 클릭하면 지정된 {selected.carrier}{" "}
            시스템으로 방문 수거 지시가 자동 전송되며 회수 전용 송장이
            채번됩니다.
          </div>
        </DetailDrawer>
      )}
