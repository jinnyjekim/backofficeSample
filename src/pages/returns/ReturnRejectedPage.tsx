...
export function ReturnRejectedPage() {
  ...
  const rejectedItems = useMemo(() => items.filter((it) => it.stage === '반품 반려'), [items]);
  ...
  const rows: GridRow[] = filtered.map((item) => {
    return {
      ...
      cells: [
        ...
        { kind: 'text', text: item.rejectReason ?? '반려 사유 미기재', color: '#dc2626', size: '12px' },
      ...
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>접수 심사 반려 및 실물 검수 불합격(구성품 누락/훼손)으로 반려된 건을 조회합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 반품 반려`}</span>
      ...
      {selected && (
        <DetailDrawer
          ...
          stats={[
            ...
            { label: '검수 판정', value: selected.inspectionResult ?? '심사 반려' },
          ]}
          fields={[
