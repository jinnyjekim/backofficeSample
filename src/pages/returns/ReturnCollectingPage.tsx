...
export function ReturnCollectingPage() {
  ...
  const collectingItems = useMemo(() => items.filter((it) => it.stage === '반품 회수'), [items]);
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              history: [
                ...item.history,
                { when: '08.27 15:55', title: '물류센터 반품 상품 입고 완료', by: 'admin01' },
              ],
            }
          : item
      )
    );
    showToast({ message: '물류센터 입고가 확인되었습니다. (회수 완료 메뉴로 이동)', type: 'success' });
    setSelectedId(null);
  };
      ...
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>택배사 기사가 고객 주소지로 방문하여 상품을 수거 중인 진행 상황을 조회합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 회수 진행중`}</span>
      ...
      {selected && (
        <DetailDrawer
          ...
          fields={[
            ...
            { label: '신청 사유', value: `${selected.reasonCategory} - ${selected.reasonDetail}` },
          ]}
        >
          ...
          <div style={{ fontSize: '13px', lineHeight: 1.6, color: '#334155', background: '#f8fafc', padding: '12px 14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            택배사 기사가 상품을 수거하여 물류센터로 이송 중입니다. 물류창고 하역 검수 후 [물류센터 입고 확인]을 진행하세요.
          </div>
        </DetailDrawer>
      )}
    </div>
