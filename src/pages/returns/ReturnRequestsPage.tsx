...
const GRID_TEMPLATE =
  "140px 140px 90px 110px minmax(200px, 1fr) 90px 90px 120px";
...
export function ReturnRequestsPage() {
  ...
  const requestItems = useMemo(
    () => items.filter((it) => it.stage === "반품 요청"),
    [items],
  );
  ...
  const handleApprove = (id: string) => {
    ...
    showToast({
      message: "반품 요청이 승인되었습니다. (반품 승인 메뉴로 이동)",
      type: "success",
    });
    setSelectedId(null);
  };
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              assignee: "admin01",
              rejectReason:
                "고객 반품 요청 사유 불충족 또는 반품 가능 기간(7일) 초과로 반려.",
              history: [
                ...item.history,
                {
                  when: "08.27 15:50",
  ...
  const rows: GridRow[] = filtered.map((item) => {
    const rm = REASON_META[item.reasonCategory] ?? {
      bg: "#f4f4f5",
      fg: "#52525b",
    };
    return {
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              고객이 신청한 신규 반품 건을 조회하고 접수 심사(승인/반려)를
              진행합니다.
            </div>
          </div>
        </div>
            ...
            { label: "수량", value: `${selected.quantity}개` },
            {
              ...
              value:
                selected.deductFee > 0
                  ? `${selected.deductFee.toLocaleString()}원 (고객 부담)`
                  : "무료 (판매자 부담)",
            },
