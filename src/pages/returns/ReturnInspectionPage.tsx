...
export function ReturnInspectionPage() {
  ...
  const inspectionItems = useMemo(
    () => items.filter((it) => it.stage === "상품 확인"),
    [items],
  );
  ...
  const handlePass = (id: string) => {
    ...
    showToast({
      message:
        "검수 합격 처리되어 반품 및 환불이 완료되었습니다. (반품 완료 메뉴로 이동)",
      type: "success",
    });
    setSelectedId(null);
  };
  ...
  const handleFail = (id: string) => {
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              rejectReason:
                "실물 상품 검수 불합격(훼손/누락)으로 반품 반려. 고객 착불 재반송.",
              history: [
                ...item.history,
                {
                  when: "08.27 16:00",
                  title: "검수 불합격에 따른 반품 반려 처리",
                  by: "admin01",
                },
              ],
            }
    ...
    showToast({
      message:
        "검수 불합격으로 반품 반려 처리되었습니다. (반품 반려 메뉴로 이동)",
      type: "info",
    });
    setSelectedId(null);
  };
  ...
  const rows: GridRow[] = filtered.map((item) => {
    const im = item.inspectionResult
      ? INSPECTION_META[item.inspectionResult]
      : { bg: "#f4f4f5", fg: "#52525b" };
    return {
      id: item.id,
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: item.inspectionResult ?? "검수중",
          bg: im.bg,
          fg: im.fg,
        },
    ...
    <div className={styles.page}>
        ...
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>
              입고된 반품 상품의 외관 손상, 포장 상태 및 구성품 유무를 판정하고
              환불을 승인합니다.
            </div>
          </div>
        </div>
          ...
          <div className={styles.filterRow1}>
            ...
            <div className={styles.quickFilters}>
              {QUICK_FILTERS.map((k) => {
                ...
                const count = inspectionItems.filter(
                  (item) => k === "전체" || item.inspectionResult === k,
                ).length;
                return (
                  <CommonButton
                    key={k}
      ...
      {selected && (
        <DetailDrawer
          ...
          statusMeta={
            selected.inspectionResult
              ? INSPECTION_META[selected.inspectionResult]
              : { bg: "#f4f4f5", fg: "#52525b" }
          }
          ...
          stats={[
            {
              label: "검수 결과",
              value: selected.inspectionResult ?? "판정 대기",
            },
        ...
        >
          ...
          >
            양품 또는 재포장 가능 상품인 경우 [검수 합격]을 선택하여 PG 환불 및
            재고 환입을 실행합니다. 사용 흔적이나 구성품 누락으로 판매 불가능한
            경우 [검수 불합격]을 선택하여 고객 착불 반송 처리합니다.
          </div>
        </DetailDrawer>
      )}
    </div>
