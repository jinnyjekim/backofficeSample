...
function SystemManagementPage({ kind }: { kind: SystemKind }) {
  ...
  const [items, setItems] = useState(() =>
    SYSTEM_ITEMS.filter((item) => item.kind === kind),
  );
    ...
    <section className={shared.page}>
        ...
        <FilterBox>
          ...
          >
            <input
              ...
              placeholder="관리 ID / 항목명 / 분류 / 담당 조직"
            />
      ...
      {selected && (
        <DetailDrawer
          ...
          stats={[
            { label: "현재 값", value: selected.value },
        ...
        >
          ...
          <div className={styles.boundaryNote}>
            시스템 설정은 상품 정보가 아니라 전체 서비스 실행 방식에 영향을
            줍니다. 변경 사유와 작업자를 감사 로그에 남기고 Production 변경은
            권한과 승인 절차를 거쳐야 합니다.
          </div>
        </DetailDrawer>
      )}
...
export const ServiceSettingsPage = () => (
  <SystemManagementPage kind="service" />
);
export const CommonCodesPage = () => <SystemManagementPage kind="codes" />;
export const IntegrationManagementPage = () => (
  <SystemManagementPage kind="integrations" />
);
