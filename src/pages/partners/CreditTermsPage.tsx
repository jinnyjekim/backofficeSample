...
export function CreditTermsPage() {
  ...
  const counts = useMemo(
    () => CREDIT_FILTERS.map((f) => data.filter(f.match).length),
    [data],
  );
  ...
  const rows: GridRow[] = filtered.map((c) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "text",
          text: c.credit ? "허용" : "미사용",
          color: c.credit ? "#059669" : "#a1a1aa",
          size: "12px",
          weight: 600,
        },
        {
          kind: "text",
          text: c.credit ? fmtWon(c.limit) : "-",
          color: "#3f3f46",
          size: "12px",
          weight: 500,
          align: "right",
        ...
        {
          kind: "text",
          text: c.credit ? fmtWon(remain) : "-",
          color: remain < 0 ? "#dc2626" : "#18181b",
          size: "12px",
          weight: 600,
          align: "right",
          numeric: true,
        ...
        {
          kind: "text",
          text: `${c.method}${c.dueDays !== "-" ? " · " + c.cutoff + " · " + c.collectDay : ""}`,
          color: "#52525b",
          size: "11.5px",
          weight: 500,
        },
    ...
    setData((prev) =>
      prev.map((c) =>
        c.code === code
          ? {
              ...c,
              memos: [{ when: "방금 전", admin: "관리자", text }, ...c.memos],
            }
          : c,
      ),
    );
  ...
  const selected = openCode
    ? (data.find((c) => c.code === openCode) ?? null)
    : null;
