...
const GRID_TEMPLATE =
  "minmax(150px,1.1fr) 76px 84px 84px 84px 88px 66px 136px 60px";
const COLUMN_LABELS = [
  "회사",
  "현재상태",
  "거래시작일",
...
export function TradeStatusPage() {
  ...
  const [statusFilter, setStatusFilter] =
    useState<(typeof QUICK_FILTER_LABELS)[number]>("전체");
  ...
  const managers = useMemo(
    () => Array.from(new Set(data.map((r) => r.manager))),
    [data],
  );

  const counts = useMemo(
    () => TRADE_FILTERS.map((f) => data.filter(f.match).length),
    [data],
  );
  ...
  const filtered = useMemo(() => {
    return data.filter((r) => {
      ...
      if (managerFilter !== "담당자 전체" && r.manager !== managerFilter)
        return false;
      if (
        q &&
        !(r.name.includes(q) || r.code.includes(q) || r.bizNo.includes(q))
      )
        return false;
      return true;
    });
  ...
  function issueLabel(r: TradeStatusRecord): string {
    ...
    if (r.pendingChange)
      return `예정 · ${r.pendingChange.toStatus} ${r.pendingChange.applyDate}`;
    return "-";
  }
    ...
    return {
      ...
      cells: [
        ...
        },
        {
          kind: "text",
          text: issueLabel(r),
          color: hasIssue ? "#d97706" : r.pendingChange ? "#4338ca" : "#a1a1aa",
          size: "11.5px",
          weight: 500,
        },
    ...
    setData((prev) =>
      prev.map((r) => {
        ...
        if (mode === "예약") {
          return {
            ...r,
            pendingChange: { toStatus: target, applyDate, reason },
          };
        }
        const entry = {
          when: "방금 전",
          ...
          by: "관리자",
          snapshot: {
            receivable: r.impact.receivable,
            overdue: r.impact.overdue,
            activeOrders: r.impact.activeOrders,
          },
        };
        return {
          ...r,
    ...
    setData((prev) =>
      prev.map((r) =>
        r.code === code
          ? {
              ...r,
              memos: [{ when: "방금 전", by: "관리자", text }, ...r.memos],
            }
          : r,
      ),
    );
  ...
  const selected = openCode
    ? (data.find((r) => r.code === openCode) ?? null)
    : null;
        ...
        <div className={styles.head}>
          <div>
            ...
            <div className={styles.subtitle}>
              거래처별 현재 거래 상태를 관리하고 상태 변경 사유와 이력을
              추적합니다.
            </div>
          </div>
        </div>
