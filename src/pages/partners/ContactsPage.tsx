...
const GRID_TEMPLATE =
  "minmax(150px,1.1fr) 82px 82px 66px 106px 134px 60px 46px 80px 60px";
const COLUMN_LABELS = [
  "담당자",
  "회사",
  "부서/직책",
  ...
  const counts = useMemo(
    () =>
      CONTACT_FILTERS.map((f) =>
        f.match ? data.filter(f.match).length : data.length,
      ),
    [data],
  );
  ...
  const rows: GridRow[] = filtered.map((c) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "text",
          text: c.isPrimary ? "주담당" : "-",
          color: c.isPrimary ? "oklch(0.52 0.16 258)" : "#c4c4c9",
          size: "11px",
          weight: 700,
        },
        {
  ...
  function confirmPrimary(id: string) {
    setData((prev) => {
      ...
      return prev.map((c) => {
        ...
        if (c.company === target.company && c.isPrimary)
              ...
              <div>
                ...
                <div className={styles.radioRow}>
                  <label className={styles.radioLabel}>
                    <input type="radio" name="usestate" defaultChecked />
                    사용
                  </label>
                  <label className={styles.radioLabel}>
