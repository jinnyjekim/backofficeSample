...
const GRID_TEMPLATE =
  "68px minmax(160px,1.3fr) 76px 70px 76px 40px 60px 52px 96px 84px 60px";
const COLUMN_LABELS = [
  "회사코드",
  "회사명",
  "사업자번호",
...
export function CompaniesPage() {
    ...
    return {
      ...
      cells: [
        ...
        },
        {
          kind: "titleWarn",
          title: c.name,
          hasIssue: c.issues.length > 0,
          issueTitle: c.issues.join(", "),
        },
        {
          kind: "text",
          text: c.bizNo,
  ...
  function handleStatusChange(code: string, status: CompanyStatus) {
    setData((prev) =>
      prev.map((c) =>
        c.code === code ? { ...c, ...({ status } as Partial<Company>) } : c,
      ),
    );
  }
          ...
          <div className={styles.registerBody}>
            ...
            <div className={styles.formGroup}>
              <div className={styles.formRow}>
                ...
                <button type="button" className={styles.addrSearchBtn}>
                  주소검색
                </button>
              </div>
