...
import {
  CONTENT_BUSINESS_META,
  CONTENT_BUSINESS_MODES,
  type ContentBusinessType,
} from "./contentBusiness";
import { CommonButton } from "../../components/common";

const GRID_TEMPLATE =
  "minmax(230px,2fr) 104px 100px 76px 67px 94px 52px 70px 40px";
...
const CREATE_OPTIONS: Record<
  ContentBusinessType,
  { kinds: string[]; audiences: string[]; categories: string[] }
> = {
  B2C: {
...
const USER_SITE_URL = (
  (import.meta.env.VITE_USER_SITE_URL as string | undefined) ??
...
export function ContentListPage() {
  ...
  const [businessMode, setBusinessMode] = useState<ContentBusinessType>(() => {
    const previewBusiness = CONTENT_ITEMS.find(
      (item) => item.id === initialPreviewId,
    )?.businessType;
    if (previewBusiness) return previewBusiness;
    return CONTENT_BUSINESS_MODES.includes(
      requestedBusiness as ContentBusinessType,
    )
      ? (requestedBusiness as ContentBusinessType)
      : "B2C";
  });
  ...
  const [requestText, setRequestText] = useState(
    "운영 정책에 맞게 콘텐츠 내용을 수정한 후 다시 검수를 요청해 주세요.",
  );
  const [toast, setToast] = useState("");

  const modeData = useMemo(
    () => data.filter((item) => item.businessType === businessMode),
    [businessMode, data],
  );
  const categories = useMemo(
    () => Array.from(new Set(modeData.map((r) => r.cat))),
    [modeData],
  );
  const createCategories = useMemo(
    () =>
      Array.from(
        new Set([...CREATE_OPTIONS[businessMode].categories, ...categories]),
      ),
  ...
  if (catFilter !== "전체")
    chips.push({
      key: "cat",
      label: `카테고리: ${catFilter}`,
      clear: () => setCatFilter("전체"),
    });
  if (reviewFilter !== "전체")
    chips.push({
      key: "review",
      label: `검수: ${reviewFilter}`,
      clear: () => setReviewFilter("전체"),
    });
  if (exposeFilter !== "전체")
    chips.push({
      key: "expose",
      label: `노출: ${exposeFilter}`,
      clear: () => setExposeFilter("전체"),
    });
  if (utypeFilter !== "전체")
    chips.push({
      key: "utype",
      label: `등록자: ${utypeFilter}`,
      clear: () => setUtypeFilter("전체"),
    });
  if (recFilter !== "전체")
    chips.push({
      key: "rec",
      label: `추천: ${recFilter}`,
      clear: () => setRecFilter("전체"),
    });
  if (from || to) {
    chips.push({
      key: "date",
  ...
  function summaryCount(label: (typeof SUMMARY_LABELS)[number]) {
    ...
    if (label === "검수대기")
      return modeData.filter((r) => r.review === "대기").length;
  ...
  function bulkChangeStatus(nextStatus: ContentStatus) {
    ...
    setData((prev) =>
      prev.map((item) =>
        targets.has(item.id) ? { ...item, status: nextStatus } : item,
      ),
    );
    flash(
      `${targets.size}개 콘텐츠의 공개 상태를 '${nextStatus}'(으)로 변경했습니다.`,
    );
  }
  ...
  function bulkChangeCategory(nextCategory: string) {
        ...
        targets.has(item.id)
          ? {
              ...
              cats: [
                ...
                ...item.cats.filter(
                  (category) =>
                    category !== nextCategory && category !== item.cat,
                ),
              ],
            }
          : item,
    ...
    flash(
      `${targets.size}개 콘텐츠의 카테고리를 '${nextCategory}'(으)로 변경했습니다.`,
    );
  }
  ...
  function bulkChangeExposure(nextExposure: ContentItem["expose"]) {
    ...
    setData((prev) =>
      prev.map((item) =>
        targets.has(item.id) ? { ...item, expose: nextExposure } : item,
      ),
    );
    flash(
      `${targets.size}개 콘텐츠의 노출 방식을 '${nextExposure}'(으)로 변경했습니다.`,
    );
  }
  ...
  function downloadExcel() {
    const targets =
      sel_.length > 0
        ? filtered.filter((item) => sel_.includes(item.id))
        : filtered;
    const columns = [
      "콘텐츠 ID",
      "비즈니스 타입",
        ...
        item.id === preview.id
          ? {
              ...
              cats: [
                editDraft.cat,
                ...item.cats.filter(
                  (cat) => cat !== editDraft.cat && cat !== item.cat,
                ),
              ],
  ...
  function saveCreate(asDraft: boolean) {
    ...
    const nextNumber =
      Math.max(
        10284,
        ...data.map((item) => Number(item.id.replace(/\D/g, "")) || 0),
      ) + 1;
    ...
    flash(
      asDraft
        ? "콘텐츠를 임시저장했습니다."
        : "콘텐츠를 등록하고 검수를 요청했습니다.",
    );
  }

  const preview = previewId
    ? (data.find((r) => r.id === previewId) ?? null)
    : null;
  const previewAsideRef = useRef<HTMLElement>(null);
  useOutsideClose(
    previewAsideRef,
    closeDetail,
    !!preview && !requestId && !confirmId,
  );
  ...
  const confirmTarget = confirmId
    ? (data.find((r) => r.id === confirmId) ?? null)
    : null;
  const requestTarget = requestId
    ? (data.find((r) => r.id === requestId) ?? null)
    : null;
  ...
  const rows: GridRow[] = filtered.map((r) => {
    ...
    const moreItems: {
      ...
      {
        label: r.utype === "관리자" ? "수정" : "운영 정보 수정",
        click: () => startEdit(r),
      },
      {
        label: "서비스 미리보기 ↗",
        click: () => {
          openServicePreview(r);
          setMenuId(null);
        },
      },
      ...(r.utype === "회원"
    ...
    const cells: Cell[] = [
      {
        ...
        title: r.title,
        id: r.id,
        titleFg: r.status === "삭제" ? "#a1a1aa" : "#18181b",
        onClick: () => openDetail(r.id),
      },
      {
        kind: "stack",
        title: r.contentKind,
        subtitle: r.cat + (r.cats.length > 1 ? ` +${r.cats.length - 1}` : ""),
      },
      {
        kind: "text",
        text: r.author,
        tip: `${r.author}\n회원번호: ${r.uid}\n가입일: ${r.ujoin}`,
        color: r.utype === "관리자" ? "#4338ca" : "#3f3f46",
        size: "12.5px",
        weight: 500,
      },
  ...
  const pages: PageBtn[] = ["1", "2", "3", "4", "5"].map((label) => ({
    label,
    active: String(page) === label,
    onClick: () => setPage(parseInt(label, 10)),
  }));
  ...
  const showBanner =
    sel_.length === filtered.length && filtered.length > 0 && !allPages;
    ...
    <div className={sh.page} onClick={() => menuId && setMenuId(null)}>
      {confirmTarget && (
        ...
        >
          <div className={sh.dialogBox}>
            ...
            <div
              className={sh.dialogBody}
            >{`'${confirmTarget.title}'이(가) 서비스에서 삭제됩니다. 즉시 완전 삭제되지 않고 삭제 상태로 전환되며, 30일간 복구할 수 있습니다.`}</div>
      ...
      {requestTarget && (
        <div
          className={sh.dialogOverlay}
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setRequestId(null);
          }}
        >
          <div className={sh.dialogBox}>
            <div className={sh.dialogTitle}>작성자 수정 요청</div>
            <div className={sh.dialogBody}>
              회원 작성 콘텐츠의 본문은 운영자가 직접 변경하지 않습니다. 요청
              내용은 작성자에게 발송되고 운영 이력에 기록됩니다.
            </div>
            <div className={styles.requestSummary}>
              <span>{requestTarget.id}</span>
              <strong>{requestTarget.title}</strong>
              <small>
                {requestTarget.author} · {requestTarget.uid}
              </small>
            </div>
            <label className={styles.requestField}>
              ...
              <textarea
                ...
                placeholder="수정이 필요한 항목과 사유를 입력하세요."
              />
            </label>
            <div className={sh.dialogActions}>
              <button
      ...
      {createDraft && (
        ...
        >
          ...
          <div className={styles.previewBody}>
            <div className={styles.editForm}>
              <div className={styles.editContext}>
                ...
                <strong>신규 등록</strong>
                <small>
                  등록한 콘텐츠는 비공개 상태로 저장되고 검수 대기 목록에
                  추가됩니다.
                </small>
              </div>
              <div className={styles.editGrid}>
                <label className={styles.editField}>
                  <span>콘텐츠 유형 *</span>
                  <select
                    value={createDraft.contentKind}
                    onChange={(e) =>
                      setCreateDraft({
                        ...createDraft,
                        contentKind: e.target.value,
                      })
                    }
                  >
                    {CREATE_OPTIONS[businessMode].kinds.map((item) => (
                ...
                <label className={styles.editField}>
                  ...
                  >
                    {CREATE_OPTIONS[businessMode].audiences.map((item) => (
                      <option key={item}>{item}</option>
                    ))}
                  </select>
                </label>
              ...
              <label className={styles.editField}>
                <span>본문 요약 *</span>
                <textarea
                  value={createDraft.desc}
                  onChange={(e) =>
                    setCreateDraft({ ...createDraft, desc: e.target.value })
                  }
                  placeholder="목록과 상세 화면에 표시할 콘텐츠 내용을 입력하세요."
                />
              </label>
              <div className={styles.editGrid}>
                <label className={styles.editField}>
                  <span>대표 카테고리 *</span>
                  <select
                    value={createDraft.cat}
                    onChange={(e) =>
                      setCreateDraft({ ...createDraft, cat: e.target.value })
                    }
                  >
                ...
                <label className={styles.editField}>
                  <span>노출 방식</span>
                  <select
                    value={createDraft.expose}
                    onChange={(e) =>
                      setCreateDraft({
                        ...createDraft,
                        expose: e.target.value as ContentItem["expose"],
                      })
                    }
                  >
                    {(["일반", "추천", "고정"] as ContentItem["expose"][]).map(
                      (item) => (
                        <option key={item}>{item}</option>
                      ),
                    )}
          ...
          <div className={styles.previewFooter}>
            <button
              type="button"
              className={styles.previewFooterBtn}
              disabled={!createDraft.title.trim() || !createDraft.desc.trim()}
            ...
            <button
              type="button"
              className={styles.previewFooterBtnSolid}
              disabled={!createDraft.title.trim() || !createDraft.desc.trim()}
            ...
            {editDraft ? (
              <div className={styles.editForm}>
                <div className={styles.editContext}>
                  ...
                  <small>
                    {preview.utype === "관리자"
                      ? "본문과 운영 정보를 수정할 수 있습니다."
                      : "본문은 작성자 소유이므로 운영 정보만 수정할 수 있습니다."}
                  </small>
                </div>
                {preview.utype === "관리자" && (
                  <>
                ...
                <label className={styles.editField}>
                  ...
                  <select
                    value={editDraft.cat}
                    onChange={(e) =>
                      setEditDraft({ ...editDraft, cat: e.target.value })
                    }
                  >
                    {categories.map((cat) => (
                      <option key={cat}>{cat}</option>
                ...
                <div className={styles.editGrid}>
                  ...
                  <label className={styles.editField}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditDraft({
                          ...editDraft,
                          review: e.target.value as ContentItem["review"],
                        })
                      }
                    >
                      {(
                        ["승인", "대기", "반려", "—"] as ContentItem["review"][]
                      ).map((item) => (
                        <option key={item}>{item}</option>
                      ))}
                    </select>
                  ...
                  <label className={styles.editField}>
                    ...
                    >
                      {(
                        ["일반", "추천", "고정"] as ContentItem["expose"][]
                      ).map((item) => (
                        <option key={item}>{item}</option>
                      ))}
                    </select>
                ...
                {preview.utype === "회원" && (
                  <button
                    type="button"
                    className={styles.requestInlineBtn}
                    onClick={() => startEditRequest(preview)}
                  >
                    본문 변경이 필요하면 작성자에게 수정 요청
                  </button>
                )}
              </div>
            ) : (
              <>
                <div
                  ...
                  style={{
                    background: `repeating-linear-gradient(135deg, ${preview.tint} 0 10px, #fff 10px 20px)`,
                  }}
                >
                  ...
                  {[
                    {
                      ...
                      ...STATUS_PILL[preview.status],
                      pad: "2px 9px",
                    },
                    {
                      label: "검수",
                      value: preview.review,
                      ...REVIEW_PILL[preview.review],
                      pad: "2px 9px",
                    },
                    {
                      label: "노출",
                      value: preview.expose,
                      ...EXPOSE_PILL[preview.expose],
                      pad: "2px 9px",
                    },
                    {
                      label: "비즈니스 타입",
                      value: preview.businessType,
                      bg: "#eef2ff",
                      fg: "#4338ca",
                      pad: "2px 9px",
                    },
                    ...
                    {
                      label: "작성자",
                      value: `${preview.author} · ${preview.uid}`,
                      bg: "transparent",
                      fg: "#18181b",
                      pad: "0",
                    },
                    ...
                    <div key={f.label} className={styles.previewFieldRow}>
                      ...
                      <span
                        className={styles.previewFieldValue}
                        style={{
                          background: f.bg,
                          color: f.fg,
                          padding: f.pad,
            ...
            {editDraft ? (
              <>
                ...
                <button
                  ...
                  disabled={
                    preview.utype === "관리자" &&
                    (!editDraft.title.trim() || !editDraft.desc.trim())
                  }
                  onClick={saveEdit}
                >
                  저장
              ...
              <>
                <button
                  type="button"
                  className={styles.previewFooterBtn}
                  onClick={() => openServicePreview(preview)}
                >
                  서비스 미리보기 ↗
                </button>
                {preview.utype === "회원" && (
      ...
      <header className={sh.header}>
        ...
        <button type="button" className={sh.primaryBtn} onClick={startCreate}>
          ＋ 콘텐츠 등록
        </button>
      </header>
        ...
        <div className={sh.topPad}>
          <div className={styles.stepLabel}>
            ...
            <span className={styles.stepHint}>
              검색어와 자주 쓰는 조건을 먼저 선택하세요.
            </span>
            {chips.length > 0 && (
              <span className={styles.appliedCount}>
                {chips.length}개 조건 적용
              </span>
            )}
          </div>
          ...
          <div className={styles.filterPanel}>
            <div className={styles.searchLine}>
              ...
              <div className={styles.searchActions}>
                <button
                  type="button"
                  className={`${styles.detailButton} ${detailFilterCount > 0 ? styles.hasFilter : ""}`}
                  onClick={() => setAdv((v) => !v)}
                  aria-expanded={adv}
                >
                ...
                <div className={styles.advancedGrid}>
                  ...
                  <label className={styles.filterControl}>
                    ...
                    <select
                      value={exposeFilter}
                      onChange={(e) => {
                        setExposeFilter(e.target.value);
                        setSel([]);
                      }}
                    >
                      {["전체", "일반", "추천", "고정"].map((o) => (
        ...
        <div className={`${sh.listArea} ${styles.listArea}`}>
          <div className={sh.toolbarRow}>
            {sel_.length > 0 ? (
              <div className={sh.selBar}>
                ...
                <select
                  className={sh.selSelect}
                  value=""
                  aria-label="선택 콘텐츠 공개 상태 변경"
                  onChange={(e) =>
                    bulkChangeStatus(e.target.value as ContentStatus)
                  }
                >
                ...
                <select
                  className={sh.selSelect}
                  value=""
                  aria-label="선택 콘텐츠 카테고리 변경"
                  onChange={(e) => bulkChangeCategory(e.target.value)}
                >
                  <option value="" disabled>
                    카테고리 변경
                  </option>
                ...
                <select
                  className={sh.selSelect}
                  value=""
                  aria-label="선택 콘텐츠 노출 방식 변경"
                  onChange={(e) =>
                    bulkChangeExposure(e.target.value as ContentItem["expose"])
                  }
                >
                  <option value="" disabled>
                    노출 설정
              ...
              <div className={sh.noSelBar}>
                ...
                >{`총 ${filtered.length.toLocaleString("ko-KR")}개`}</span>
                <div className={sh.rowSpacer} />
                <button
                  type="button"
                  className={sh.resetBtn}
          ...
          {showBanner && (
            <div className={sh.banner}>
              ...
              <button
                type="button"
                className={sh.bannerAction}
                onClick={() => setAllPages(true)}
              >{`검색 결과 ${filtered.length}개 전체 선택`}</button>
            </div>
          )}
          ...
          <div className={sh.gridArea}>
            <DataGrid
              ...
              selectable
              allSelected={
                filtered.length > 0 && sel_.length === filtered.length
              }
              ...
              emptyActionLabel={isEmptySearch ? "초기화" : undefined}
