...
export function CategoriesPage() {
  ...
  const [useFilter, setUseFilter] =
    useState<(typeof STATUS_LABELS)[number]>("전체");
  ...
  const descend = (id: string): string[] =>
    kids(id).reduce<string[]>((a, c) => a.concat([c.id], descend(c.id)), []);
  const rollCount = (id: string): number =>
    (byId(id)?.count ?? 0) +
    descend(id).reduce((a, i) => a + (byId(i)?.count ?? 0), 0);
  const pathOf = (id: string): string => {
    const out: string[] = [];
    let p = byId(id);
    while (p) {
      out.unshift(p.name);
      p = p.parent ? byId(p.parent) : undefined;
    }
    return out.join(" › ");
  };

  const query = q.trim().toLowerCase();
  const hit = (it: Category) =>
    ...
    ? scopeItems
        .filter(
          (item) =>
            (!query || hit(item)) &&
            (useFilter === "전체" ||
              (useFilter === "사용" ? item.use : !item.use)),
        )
        .map((item) => item.id)
    : [];
  ...
  const walk = (parent: string | null, depth: number) => {
    scopeItems
      ...
      .forEach((it) => {
        ...
        const parentOpen =
          !it.parent || !!query || openIds.indexOf(it.parent) >= 0;
  ...
  const dirty = isNew
    ? !!(
        effectiveDraft &&
        (effectiveDraft.name || effectiveDraft.code || effectiveDraft.desc)
      )
    : !!(
        cur &&
        effectiveDraft &&
        (effectiveDraft.name !== cur.name ||
          effectiveDraft.code !== cur.code ||
          (effectiveDraft.parent || "") !== (cur.parent || "") ||
          effectiveDraft.use !== cur.use ||
          effectiveDraft.desc !== cur.desc)
      );
  const draftDepth = effectiveDraft
    ? effectiveDraft.parent
      ? depthOf(effectiveDraft.parent) + 1
      : 1
    : 1;
  ...
  if (cur?.locked)
    currentWarnings.push("사용 이력이 있어 카테고리 코드가 잠겨 있습니다.");
  if (cur && depthOf(cur.id) >= MAX_DEPTH)
    currentWarnings.push(
      `최대 ${MAX_DEPTH} Depth 카테고리로 하위 항목을 추가할 수 없습니다.`,
    );
  if (cur && !cur.use && rollCount(cur.id) > 0)
    currentWarnings.push(
      `미사용 상태지만 ${rollCount(cur.id).toLocaleString("ko-KR")}건의 콘텐츠가 연결되어 있습니다.`,
    );
  if (cur && cur.count === 0 && kids(cur.id).length === 0)
    currentWarnings.push(
      "연결된 콘텐츠와 하위 카테고리가 없는 빈 카테고리입니다.",
    );
  ...
  const parentOptions = [{ value: "", label: "최상위 카테고리" }].concat(
    scopeItems
      .filter((it) =>
        isNew
          ? depthOf(it.id) < MAX_DEPTH
          : it.id !== selId &&
            descend(selId).indexOf(it.id) < 0 &&
            depthOf(it.id) < MAX_DEPTH,
      )
      .map((it) => ({
        value: it.id,
        label:
          "　".repeat(depthOf(it.id) - 1) +
          (depthOf(it.id) > 1 ? "└ " : "") +
          it.name,
      })),
  );
  ...
  function doSave() {
    ...
    if (!effectiveDraft.name.trim()) {
      setErr("카테고리명을 입력해 주세요.");
      return;
    }
    if (!effectiveDraft.code.trim()) {
      setErr("카테고리 코드를 입력해 주세요.");
      return;
    }
    const dup = items.some(
      (it) =>
        it.scope === scope &&
        it.id !== (isNew ? "" : selId) &&
        (it.parent || "") === (effectiveDraft.parent || "") &&
        it.name.trim() === effectiveDraft.name.trim(),
    );
    if (dup) {
      setErr("같은 상위 카테고리 안에 동일한 이름이 이미 있습니다.");
      return;
    }
    if (isNew) {
      const id = "n" + Date.now();
      const rec: Category = {
        ...
        code: effectiveDraft.code.trim().toUpperCase(),
        parent: effectiveDraft.parent || null,
        use: effectiveDraft.use,
        desc: effectiveDraft.desc,
        count: 0,
        created: "2026.08.13",
        updated: "2026.08.13",
        locked: false,
      ...
      setMode("edit");
      setDraft(null);
      if (effectiveDraft.parent)
        setOpenIds((prev) => prev.concat([effectiveDraft.parent]));
  ...
  function askDelete(it: Category) {
    if (kids(it.id).length) {
      setDialog({
        title: "하위 카테고리가 존재합니다.",
        body: "하위 카테고리를 먼저 이동하거나 삭제해 주세요.",
        buttons: [{ label: "닫기", kind: "solid", act: "close" }],
      });
    } else if (it.count > 0) {
      setDialog({
        title: "카테고리를 삭제할 수 없습니다.",
        body: `현재 이 카테고리에 ${it.count}개의 콘텐츠가 연결되어 있습니다. 연결된 콘텐츠의 카테고리를 먼저 변경해 주세요.`,
        buttons: [
          { label: "닫기", kind: "ghost", act: "close" },
          {
            label: "콘텐츠 목록 보기",
            kind: "solid",
            act: "contents",
      ...
      setDialog({
        ...
        body: `삭제된 카테고리는 복구할 수 없습니다.\n카테고리: ${it.name}`,
        buttons: [
          { label: "취소", kind: "ghost", act: "close" },
          { label: "삭제", kind: "danger", act: "del", to: it.id },
        ],
      });
    }
    setMenuId(null);
  ...
  function askUse(it: Category, next: boolean) {
    if (!next) {
      setDialog({
        title: "카테고리를 미사용 처리하시겠습니까?",
        body: "신규 콘텐츠에서는 해당 카테고리를 선택할 수 없습니다. 기존 콘텐츠의 카테고리 정보는 유지됩니다.",
        buttons: [
          { label: "취소", kind: "ghost", act: "close" },
          { label: "미사용 처리", kind: "solid", act: "use", to: it.id },
        ],
      });
    } else {
      setItems((prev) =>
        prev.map((x) => (x.id === it.id ? { ...x, use: true } : x)),
      );
    }
    setMenuId(null);
  }
  ...
  function dlgRun(b: DialogButton) {
    ...
    if (b.act === "use" && b.to) {
      ...
      if (target)
        setItems((prev) =>
          prev.map((x) => (x.id === b.to ? { ...x, use: false } : x)),
        );
  ...
  function move(fromId: string, toId: string) {
    const a = byId(fromId);
    const b = byId(toId);
    if (!a || !b || a.id === b.id || (a.parent || "") !== (b.parent || ""))
      return;

    const siblings = items.filter(
      (item) =>
        item.scope === scope && (item.parent || "") === (a.parent || ""),
    );
    ...
    >
            ...
            <div className={sh.dialogActions}>
              {dialog.buttons.map((b) => (
                <button
                  ...
                  style={{
                    border:
                      b.kind === "ghost" ? "1px solid rgba(0,0,0,.12)" : "none",
                    background:
                      b.kind === "solid"
                        ? ACCENT
                        : b.kind === "danger"
      ...
      <header className={sh.header}>
        <div>
          ...
          <div className={sh.headerSub}>
            콘텐츠 분류에 사용되는 카테고리를 등록하고 관리합니다.
          </div>
        </div>
        <ContentBusinessSwitch
          ...
          note={
            scope === "공통" ? "여러 비즈니스에서 공유" : `${scope} 전용 분류`
          }
          label="분류 범위"
        />
        ...
        <button
          ...
          onClick={() => {
            setMode("new");
            setDraft({ name: "", code: "", parent: "", use: true, desc: "" });
            setMenuId(null);
            setErr("");
          }}
        >
          ...
          <div className={styles.treeList}>
                ...
                <div key={it.id}>
                  <div
                    ...
                    style={{
                      ...
                      background: selId === it.id ? "#eef2ff" : "transparent",
                    }}
                  >
                    ...
                    <span
                      ...
                      style={{
                        fontWeight: selId === it.id ? 700 : 500,
                        color: it.use
                          ? selId === it.id
                            ? "#18181b"
                            : "#3f3f46"
                    ...
                    <span className={styles.treeUnused}>
                      {it.use ? "" : "미사용"}
                    </span>
                    <span
                      className={styles.treeCount}
                      style={{
                        color: rollCount(it.id) ? "#71717a" : "#c4c4c8",
                      }}
                    >
                      {rollCount(it.id).toLocaleString("ko-KR")}건
                    </span>
                    <button
                      type="button"
                      className={styles.treeMoreBtn}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    {menuId === it.id && (
                      ...
                      >
                        <button
                          ...
                          style={{
                            color: depth >= MAX_DEPTH ? "#c4c4c8" : "#3f3f46",
                          }}
                          onClick={(e) => {
                            e.stopPropagation();
                            ...
                            setMenuId(null);
                            setOpenIds((prev) => prev.concat([it.id]));
                            setErr("");
                          }}
                        >
                          하위 카테고리 추가
            ...
            {emptySearch && (
              <div className={sh.emptyBlock}>
                <div className={sh.emptyTitle}>
                  조건에 맞는 카테고리가 없습니다.
                </div>
                <div className={sh.emptySub}>
                  검색어나 사용 상태를 변경해 주세요.
                </div>
                <button
                  type="button"
                  className={sh.emptyActionBtn}
                  onClick={() => {
                    setQ("");
                    setUseFilter("전체");
                  }}
            ...
            {emptyAll && (
              <div className={sh.emptyBlock}>
                ...
                <div className={sh.emptySub}>
                  콘텐츠 분류를 위한 첫 카테고리를 등록해 주세요.
                </div>
                <button
                  type="button"
                  className={sh.emptyActionBtnSolid}
                  onClick={() => {
                    setMode("new");
                    setDraft({
                      name: "",
        ...
        <div className={styles.panel}>
          <div className={styles.panelHead}>
            <div className={styles.panelHeadTitle}>
              {isNew ? "새 카테고리 등록" : "카테고리 정보"}
            </div>
            <div className={styles.panelHeadSub}>
              {isNew
                ? "카테고리명과 코드를 입력하고 상위 카테고리를 지정합니다."
                : cur
                  ? pathOf(cur.id)
                  : ""}
            </div>
            ...
            <>
              <div className={styles.panelBody}>
                <div className={styles.formColumn}>
                  <div className={sh.formField}>
                    <span className={sh.formFieldLabel}>
                      카테고리명 <span className={sh.required}>*</span>
                    </span>
                    <input
                      className={sh.formInput}
                  ...
                  <div className={sh.formField}>
                    <span className={sh.formFieldLabel}>
                      카테고리 코드 <span className={sh.required}>*</span>
                    </span>
                    <div
                      style={{ display: "flex", alignItems: "center", gap: 8 }}
                    >
                      <input
                        className={`${sh.formInput} ${sh.mono}`}
                        value={effectiveDraft.code}
                        onChange={(e) => setD({ code: e.target.value })}
                        readOnly={!isNew && !!cur?.locked}
                    ...
                    <div className={styles.panelCodeHint}>
                      {!isNew && cur?.locked
                        ? "이미 사용 중인 코드는 변경할 수 없습니다"
                        : "영문 대문자와 _ 조합을 권장합니다"}
                    </div>
                  </div>
                  ...
                  <div className={sh.formField}>
                    ...
                    <select
                      className={sh.formSelect}
                      value={effectiveDraft.parent}
                      onChange={(e) => setD({ parent: e.target.value })}
                    >
                      {parentOptions.map((o) => (
                    ...
                    <div className={styles.panelDepthHint}>
                      {`최대 ${MAX_DEPTH} Depth까지 등록할 수 있습니다`} ·
                      자신과 하위 카테고리는 선택할 수 없습니다
                    </div>
                  </div>

                  <div className={sh.formField}>
                    <span className={sh.formFieldLabel}>
                      사용 여부 <span className={sh.required}>*</span>
                    </span>
                    <div className={sh.useToggleRow}>
                      <button
                        ...
                        style={{
                          border: `1px solid ${effectiveDraft.use ? ACCENT : "rgba(0,0,0,.12)"}`,
                          background: effectiveDraft.use ? ACCENT : "#fff",
                          color: effectiveDraft.use ? "#fff" : "#52525b",
                        }}
                      ...
                      <button
                        ...
                        className={sh.useToggleBtn}
                        style={{
                          border: `1px solid ${!effectiveDraft.use ? "#18181b" : "rgba(0,0,0,.12)"}`,
                          background: !effectiveDraft.use ? "#18181b" : "#fff",
                          color: !effectiveDraft.use ? "#fff" : "#52525b",
                        }}
                        onClick={() => {
                          if (isNew) setD({ use: false });
                          else if (cur) askUse(cur, false);
                        }}
                      >
                        미사용
                      </button>
                  ...
                  <div className={sh.formField}>
                    ...
                    <textarea
                      className={sh.formTextarea}
                      style={{ height: 70 }}
                      value={effectiveDraft.desc}
                      onChange={(e) => setD({ desc: e.target.value })}
                      placeholder="관리자용 메모입니다. 운영 정책이나 사용 기준을 기록해 두세요."
                    />
                  </div>
                    ...
                    <>
                      ...
                      <section className={styles.railCard}>
                        ...
                        <div className={styles.checkList}>
                          {[
                            {
                              ok: !!effectiveDraft.name.trim(),
                              text: "카테고리명 입력",
                            },
                            {
                              ok: /^[A-Z0-9_]+$/.test(
                                effectiveDraft.code.trim(),
                              ),
                              text: "영문 대문자·숫자·_ 코드",
                            },
                            {
                              ok: draftDepth <= MAX_DEPTH,
                              text: `최대 ${MAX_DEPTH} Depth 이내`,
                            },
                            {
                              ok:
                              ...
                              text: "같은 단계의 이름 중복 없음",
                            },
                          ].map((item) => (
                            <div
                              key={item.text}
                              className={`${styles.checkItem} ${item.ok ? styles.checkOk : ""}`}
                            >
                              <span>{item.ok ? "✓" : "·"}</span>
                              {item.text}
                            </div>
                      ...
                      <section
                        className={`${styles.railCard} ${styles.guideCard}`}
                      >
                        <div className={styles.railTitle}>설계 가이드</div>
                        <ul>
                          <li>
                            사용자가 이해할 수 있는 짧은 이름을 권장합니다.
                          </li>
                          <li>코드는 연동 이후 변경하기 어렵습니다.</li>
                          <li>
                            비슷한 카테고리는 새로 만들기 전에 기존 경로를
                            확인하세요.
                          </li>
                        </ul>
                      </section>
                    ...
                    <>
                      <section className={styles.railCard}>
                        <div className={styles.railCardHead}>
                          <span>운영 현황</span>
                          <em
                            className={cur.use ? styles.useOn : styles.useOff}
                          >
                            {cur.use ? "사용중" : "미사용"}
                          </em>
                        </div>
                        <div className={styles.metricGrid}>
                          <div>
                            <span>직접 연결</span>
                            <strong>{cur.count.toLocaleString("ko-KR")}</strong>
                            <small>건</small>
                          </div>
                          <div>
                            <span>하위 포함</span>
                            <strong>
                              {rollCount(cur.id).toLocaleString("ko-KR")}
                            </strong>
                            <small>건</small>
                          </div>
                          <div>
                            <span>하위 항목</span>
                            <strong>{kids(cur.id).length}</strong>
                            <small>개</small>
                          </div>
                          <div>
                            <span>현재 단계</span>
                            <strong>{depthOf(cur.id)}</strong>
                            <small>Depth</small>
                          </div>
                        </div>
                              ...
                              return (
                                ...
                                >
                                  <span>
                                    ...
                                    <em>{count.toLocaleString("ko-KR")}건</em>
                                  </span>
                                  <i>
                                    <b
                                      style={{
                      ...
                      {currentWarnings.length > 0 && (
                        <section
                          className={`${styles.railCard} ${styles.warningCard}`}
                        >
                      ...
                      <section className={styles.railMeta}>
                        <div>
                          <span>등록일</span>
                          <strong>{cur.created}</strong>
                        </div>
                        <div>
                          <span>최근 수정</span>
                          <strong>{cur.updated}</strong>
                        </div>
                        <div>
                          <span>카테고리 코드</span>
              ...
              <div className={styles.panelFooter}>
                {!isNew && (
                  <button
                    type="button"
                    className={sh.dangerBtn}
                    onClick={() => cur && askDelete(cur)}
                  >
                    삭제
                  </button>
                )}
                ...
                <button
                  ...
                  style={{
                    background: isNew || dirty ? ACCENT : "#e4e4e7",
                    color: isNew || dirty ? "#fff" : "#a1a1aa",
                  }}
                  onClick={() => {
                    if (isNew || dirty) doSave();
                  }}
                >
                  {isNew ? "등록" : "변경사항 저장"}
                </button>
