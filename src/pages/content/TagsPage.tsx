...
export function TagsPage() {
  ...
  const list = useMemo(() => {
    ...
    let out = items.filter((it) => {
      ...
      if (
        linkF !== "전체" &&
        (linkF === "연결 있음" ? it.count === 0 : it.count > 0)
      )
        return false;
    ...
    out = out
      .slice()
      .sort((a, b) => (sortDesc ? -cmp[sortKey](a, b) : cmp[sortKey](a, b)));
    return out;
  ...
  function statusCount(label: (typeof STATUS_LABELS)[number]) {
    ...
    return scoped.filter((item) => (label === "사용" ? item.use : !item.use))
      .length;
  }
  ...
  function sortHead(key: SortKey) {
    return {
      arrow: sortKey === key ? (sortDesc ? "↓" : "↑") : "",
      click: () =>
        setSortKey((k) => {
          setSortDesc(k === key ? !sortDesc : true);
          return key;
        }),
    };
  }
  ...
  const dupTag =
    modal && modal.name.trim()
      ? items.find(
          (it) =>
            it.scope === scope &&
            it.id !== modal.id &&
            it.name.trim().toLowerCase() === modal.name.trim().toLowerCase(),
        )
      : null;
  ...
  function askUse(it: Tag, next: boolean) {
    if (!next) {
      setDialog({
        title: "태그를 미사용 처리하시겠습니까?",
        body: "신규 콘텐츠에서는 해당 태그를 선택할 수 없습니다. 기존 콘텐츠에 연결된 태그 정보는 유지됩니다.",
        buttons: [
          { label: "취소", kind: "ghost", act: "close" },
          { label: "미사용 처리", kind: "solid", act: "unuse", to: it.id },
        ],
      });
    } else {
      setItems((prev) =>
  ...
  function askDelete(it: Tag) {
    if (it.count > 0) {
      setDialog({
        title: "태그를 삭제할 수 없습니다.",
        body: `현재 ${it.count}개의 콘텐츠가 이 태그를 사용하고 있습니다.\n연결된 콘텐츠에서 태그를 제거하거나 다른 태그로 병합해 주세요.`,
        buttons: [
          ...
          { label: "태그 병합", kind: "solid", act: "gomerge", to: it.id },
        ],
      });
    } else {
      setDialog({
  ...
  function dlgRun(b: DialogButton) {
    ...
    if (b.act === "gomerge" && b.to) {
      setDialog(null);
      setMerge({ source: [b.to], keep: "" });
    }
    if (b.act === "del" && b.to) {
  ...
  const mgKeepOptions = merge
    ? merge.source.length > 1
      ? items.filter(
          (it) => it.scope === scope && merge.source.indexOf(it.id) >= 0,
        )
      : items.filter(
          (it) => it.scope === scope && merge.source.indexOf(it.id) < 0,
        )
    : [];
  const mgMergeList = merge
    ? merge.source.length > 1
      ? merge.source.filter((id) => id !== merge.keep).map(byId)
      : merge.source.map(byId)
    : [];
  const mgKeepTag = merge && merge.keep ? byId(merge.keep) : null;
  const mgTotal = merge
    ? mgMergeList
        .filter((x): x is Tag => !!x)
        .reduce((a, it) => a + it.count, 0) + (mgKeepTag ? mgKeepTag.count : 0)
    : 0;

  function doMerge() {
    if (!merge || !merge.keep) return;
    const removeIds = mgMergeList
      .filter((x): x is Tag => !!x)
      .map((it) => it.id);
    const addCount = mgMergeList
      .filter((x): x is Tag => !!x)
      .reduce((a, it) => a + it.count, 0);
    setItems((prev) =>
      prev
        .filter((it) => removeIds.indexOf(it.id) < 0)
        .map((it) =>
          it.id === merge.keep
            ? { ...it, count: it.count + addCount, updated: "2026.08.13" }
            : it,
        ),
    );
  ...
  function bulkUse(val: boolean) {
    setItems((prev) =>
      prev.map((it) => (sel_.indexOf(it.id) >= 0 ? { ...it, use: val } : it)),
    );
  }

  function bulkDelete() {
    const linked = list.filter(
      (it) => sel_.indexOf(it.id) >= 0 && it.count > 0,
    );
    ...
    if (linked.length) {
      ...
      setSel([]);
      setDialog({
        title: "일부 태그를 삭제할 수 없습니다.",
        body: `${linked.length}개 태그에 연결된 콘텐츠가 있어 삭제되지 않았습니다. 연결이 없는 ${removable.length}개 태그만 삭제되었습니다.`,
        buttons: [{ label: "닫기", kind: "ghost", act: "close" }],
      });
    } else {
  ...
  const columns: GridColumn[] = [
    {
      label: `태그명 ${sortHead("name").arrow}`,
      onClick: sortHead("name").click,
    },
    { label: "태그 코드" },
    {
      label: `연결 콘텐츠 ${sortHead("count").arrow}`,
      onClick: sortHead("count").click,
    },
    { label: "상태" },
    {
      label: `등록일 ${sortHead("created").arrow}`,
      onClick: sortHead("created").click,
    },
    {
      label: `수정일 ${sortHead("updated").arrow}`,
      onClick: sortHead("updated").click,
    },
  ...
  const rows: GridRow[] = list.map((it) => {
    ...
    const moreItems = [
      ...
      { sep: true },
      it.use
        ? { label: "사용 중지", click: () => askUse(it, false) }
        : { label: "사용하기", click: () => askUse(it, true) },
      {
        label: "태그 병합",
        click: () => {
          setMerge({ source: [it.id], keep: "" });
          setMenuId(null);
        },
      },
    ...
    const cells: Cell[] = [
      ...
      {
        kind: "text",
        text: `${it.count.toLocaleString("ko-KR")}건`,
        color: "#3f3f46",
        size: "12.5px",
        numeric: true,
      },
      ...
      },
      {
        kind: "rowMenu",
        open: menuId === it.id,
        onToggle: () => setMenuId(menuId === it.id ? null : it.id),
        items: moreItems,
      },
    ];
    return {
      ...
      onToggleSelect: () =>
        setSel((prev) =>
          isSel ? prev.filter((x) => x !== it.id) : prev.concat([it.id]),
        ),
      onClick: () => openEdit(it),
      bg: isSel ? "#f7f8ff" : "transparent",
    };
  });
  ...
  return (
    ...
    >
      ...
      {merge && (
        <div
          className={sh.modalOverlay}
          style={{ zIndex: 72 }}
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setMerge(null);
          }}
        >
          <div className={sh.modalBox} style={{ width: "min(440px,100%)" }}>
            <div className={sh.modalTitle}>태그 병합</div>
            <div
              className={sh.modalSub}
              style={{ fontVariantNumeric: "normal" }}
            >
              병합 대상 태그에 연결된 콘텐츠는 유지할 태그로 자동 이전됩니다.
            </div>

            <div style={{ marginTop: 14 }}>
              <div className={sh.formFieldLabel} style={{ marginBottom: 6 }}>
                유지할 태그 <span className={sh.required}>*</span>
              </div>
              <select
                className={sh.formSelect}
                value={merge.keep}
                onChange={(e) => setMerge({ ...merge, keep: e.target.value })}
              >
                <option value="">선택해 주세요</option>
                {mgKeepOptions.map((it) => (
                  <option
                    key={it.id}
                    value={it.id}
                  >{`${it.name} (${it.count.toLocaleString("ko-KR")}건)`}</option>
                ))}
              </select>
            </div>
            ...
            >
              ...
              >
                {mgTotal.toLocaleString("ko-KR")}건
              </span>
            </div>
      ...
      {modal && (
        ...
        >
          <div className={sh.modalBox}>
            <div className={sh.modalTitle}>
              {modal.mode === "new" ? "태그 등록" : "태그 수정"}
            </div>
            ...
            >
              <div className={sh.formField}>
                <span className={sh.formFieldLabel}>
                  태그명 <span className={sh.required}>*</span>
                </span>
                <input
                ...
                {dupTag && (
                  <div className={sh.warnBox}>
                    <div className={sh.warnBoxTitle}>
                      ⚠ 동일한 이름의 태그가 이미 존재합니다.
                    </div>
                    <div
                      className={sh.warnBoxBody}
                    >{`${dupTag.name} · ${dupTag.code} · 연결 콘텐츠 ${dupTag.count.toLocaleString("ko-KR")}건`}</div>
                    <button
                      type="button"
                      className={sh.warnBoxLink}
                      onClick={() =>
                        setModal({
                          mode: "edit",
                          id: dupTag.id,
              ...
              <div className={sh.formField}>
                <span className={sh.formFieldLabel}>
                  태그 코드 <span className={sh.required}>*</span>
                </span>
              ...
              <div className={sh.formField}>
                ...
                <div className={sh.useToggleRow}>
                  <button
                    ...
                    style={{
                      border: `1px solid ${modal.use ? ACCENT : "rgba(0,0,0,.12)"}`,
                      background: modal.use ? ACCENT : "#fff",
                      color: modal.use ? "#fff" : "#52525b",
                    }}
                    onClick={() => setModal({ ...modal, use: true })}
                  >
                    사용
                  </button>
                  <button
                    ...
                    className={sh.useToggleBtn}
                    style={{
                      border: `1px solid ${!modal.use ? "#18181b" : "rgba(0,0,0,.12)"}`,
                      background: !modal.use ? "#18181b" : "#fff",
                      color: !modal.use ? "#fff" : "#52525b",
                    }}
                    onClick={() => setModal({ ...modal, use: false })}
                  >
                    미사용
                  </button>
                </div>
              ...
              {modal.mode === "edit" && modalCur && (
                <>
                  <div className={sh.linkedCard}>
                    ...
                    <span className={sh.linkedCardValue}>
                      {modalCur.count.toLocaleString("ko-KR")}건
                    </span>
                    <button
                      type="button"
                      className={sh.selBtn}
                      onClick={() => goContents(modalCur.name)}
                    >
                      콘텐츠 목록 보기
                    </button>
                  </div>
                  <div className={sh.metaRow}>
                    {[
                      ["등록일", modalCur.created],
                      ["수정일", modalCur.updated],
                    ].map(([label, value]) => (
            ...
            <div className={sh.modalActions}>
              {modal.mode === "edit" && (
                <button
                  ...
                  onClick={() => modalCur && askDelete(modalCur)}
                >
                  삭제
                </button>
              )}
              ...
              <button
                type="button"
                className={sh.solidBtn}
                disabled={!modal.name.trim() || !modal.code.trim() || !!dupTag}
                onClick={saveModal}
              >
                {modal.mode === "new" ? "등록" : "변경사항 저장"}
              </button>
            </div>
          </div>
        </div>
      ...
      <header className={sh.header}>
        <div>
          ...
          <div className={sh.headerSub}>
            콘텐츠 분류 및 검색에 사용되는 태그를 등록하고 관리합니다.
          </div>
        </div>
      ...
      <div className={sh.body}>
        <div className={sh.topPad}>
          <div className={filterStyles.stepLabel}>
            ...
            <span className={filterStyles.stepHint}>
              태그명과 연결 상태로 빠르게 찾아보세요.
            </span>
            {chips.length > 0 && (
              <span className={filterStyles.appliedCount}>
          ...
          <div className={filterStyles.filterPanel}>
            <div className={filterStyles.searchLine}>
              ...
              <div className={filterStyles.searchActions}>
                <button
                  type="button"
                  className={`${filterStyles.detailButton} ${detailFilterCount > 0 ? filterStyles.hasFilter : ""}`}
                ...
                >
                  ...
                  상세 필터
                  {detailFilterCount > 0 && (
                    <span className={filterStyles.detailCount}>
                      {detailFilterCount}
                    </span>
                  )}
                  {adv ? (
            ...
            {adv && (
              <div className={filterStyles.advancedPanel}>
                ...
                <div className={filterStyles.advancedGrid}>
                  <label
                    className={`${filterStyles.filterControl} ${filterStyles.dateControl}`}
                  >
                    <span>등록일</span>
            ...
            {chips.length > 0 && (
              <div className={filterStyles.appliedFilters}>
                ...
                <div className={filterStyles.chipList}>
                  {chips.map((chip) => (
                    <button
                      key={chip.key}
                      type="button"
                      className={filterStyles.filterChip}
                      onClick={chip.clear}
                      title={`${chip.label} 조건 해제`}
                    >
                      {chip.label}
                ...
                <button
                  type="button"
                  className={filterStyles.clearAllButton}
                  onClick={resetAll}
                >
                  전체 해제
                </button>
          ...
          <div
            className={`${filterStyles.stepLabel} ${filterStyles.resultStep}`}
          >
            <span className={filterStyles.stepTitle}>조회 결과</span>
            <span className={filterStyles.stepHint}>
              행을 클릭하면 태그 정보와 연결 현황을 수정할 수 있습니다.
            </span>
          </div>
        </div>
        ...
        <div className={`${sh.listArea} ${filterStyles.listArea}`}>
              ...
              <div className={sh.selBar}>
                ...
                {sel_.length > 1 && (
                  <button
                    type="button"
                    className={sh.selBtn}
                    onClick={() => setMerge({ source: sel_.slice(), keep: "" })}
                  >
                    태그 병합
                  </button>
                )}
          ...
          <div className={sh.gridArea}>
            <DataGrid
              ...
              onToggleAll={() =>
                setSel(
                  sel_.length === list.length ? [] : list.map((it) => it.id),
                )
              }
              empty={list.length === 0}
              emptyText={
                emptyAll ? "등록된 태그가 없습니다." : "검색 결과가 없습니다."
              }
              emptySubtext={
                emptyAll
                  ? "콘텐츠 분류에 사용할 태그를 등록해 주세요."
                  : "다른 검색어나 필터 조건을 사용해 주세요."
              }
              emptyActionLabel={emptyAll ? "＋ 태그 등록" : "초기화"}
              emptyActionClick={
                emptyAll
                  ? () =>
                      setModal({
