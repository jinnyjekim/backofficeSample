...
export function ReviewAnalyticsPage() {
  ...
  const averageRating =
    total === 0
      ? 0
      : scopedReviews.reduce((sum, review) => sum + review.rating, 0) / total;
  const exposed = scopedReviews.filter(
    (review) => review.exposure === "노출",
  ).length;
  const pendingReports = scopedReviews.reduce(
    (sum, review) => sum + pendingReportCount(review),
    0,
  );
  const replied = scopedReviews.filter(
    (review) => review.adminReply !== null,
  ).length;
  ...
  const rows = useMemo<GridRow[]>(() => {
    ...
    scopedReviews.forEach((review) =>
      grouped.set(review.productCode, [
        ...(grouped.get(review.productCode) ?? []),
        review,
      ]),
    );
    ...
    return Array.from(grouped.entries())
      .map(([code, reviews]) => {
        ...
        const reports = reviews.reduce(
          (sum, review) => sum + pendingReportCount(review),
          0,
        );
        const responses = reviews.filter(
          (review) => review.adminReply !== null,
        ).length;
        return {
          id: code,
          ...
          row: {
            ...
            cells: [
              ...
              },
              {
                kind: "text",
                text: `${lowRating}건`,
                color: lowRating > 0 ? "#dc2626" : "#71717a",
                size: "12px",
                weight: 600,
                align: "right",
                numeric: true,
              ...
              {
                kind: "text",
                text: `${Math.round((responses / reviews.length) * 100)}%`,
                color: "#4338ca",
                size: "12px",
                weight: 700,
                align: "right",
    ...
    <div className={shared.page}>
      <header className={shared.header}>
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              리뷰 품질, 고객 반응과 운영 대응 현황을 상품별로 분석합니다.
            </div>
          </div>
        </div>
        ...
        <div className={styles.metricGrid}>
          ...
          <div className={styles.metricCard}>
            <span>노출률</span>
            <strong>
              {total === 0 ? 0 : Math.round((exposed / total) * 100)}%
            </strong>
            <small>{exposed}건 노출중</small>
          </div>
          <div className={styles.metricCard}>
            <span>답변률</span>
            <strong>{replyRate}%</strong>
            <small>{replied}건 답변 완료</small>
          </div>
          <div
            className={`${styles.metricCard} ${pendingReports > 0 ? styles.warningCard : ""}`}
          >
            <span>처리 대기 신고</span>
            <strong>{pendingReports}건</strong>
            <small>리뷰 운영에서 처리</small>
          </div>
        </div>
        ...
        <div className={styles.analysisGrid}>
          ...
          <section className={styles.panel}>
            ...
            <div className={styles.checkList}>
              <div>
                <span>신고가 접수된 리뷰</span>
                <strong>
                  {
                    scopedReviews.filter(
                      (review) => pendingReportCount(review) > 0,
                    ).length
                  }
                  건
                </strong>
              ...
              <div>
                ...
                <strong>
                  {
                    scopedReviews.filter((review) => review.exposure !== "노출")
                      .length
                  }
                  건
                </strong>
              ...
              <div>
                <span>저평점(1~2점) 리뷰</span>
                <strong>
                  {scopedReviews.filter((review) => review.rating <= 2).length}
                  건
                </strong>
              </div>
            </div>
      ...
      <div className={shared.gridWrap}>
        <DataGrid
          columns={COLUMNS}
          rows={rows}
          gridTemplate="1.5fr 90px 90px 90px 110px 90px"
          minWidth="760px"
          empty={rows.length === 0}
          emptyText="분석할 리뷰 데이터가 없습니다."
        />
      </div>
    </div>
  );
