...
export function ReviewsListPage() {
  ...
  const counts = useMemo(() => {
    ...
    QUICK_FILTERS.forEach((f) => {
      c[f] = reviews.filter((r) => matchesQuickFilter(r, f)).length;
    });
    return c;
  }, [reviews]);
  ...
  const filtered = useMemo(
    () =>
      reviews.filter((r) => {
        ...
        if (
          search &&
          !`${r.id} ${r.member} ${productName(r.productCode)} ${r.productCode}`
            .toLowerCase()
  ...
  const selected = drawerId
    ? (reviews.find((r) => r.id === drawerId) ?? null)
    : null;
  ...
  function restore(item: Review) {
    update(item.id, (r) =>
      history(
        { ...r, exposure: "노출", hideReason: null, hideDetail: null },
        "노출 복원",
      ),
    );
  ...
  function saveReply(id: string, text: string) {
    update(id, (r) =>
      history(
        { ...r, adminReply: { at: `${TODAY} 15:00`, by: "admin01", text } },
        r.adminReply ? "관리자 답변 수정" : "관리자 답변 등록",
      ),
    );
  ...
  function addMemo(id: string, text: string) {
    update(id, (r) => ({
      ...
      memos: [
        ...r.memos,
        { id: `M-${Date.now()}`, at: `${TODAY} 15:00`, by: "admin01", text },
      ],
    }));
  }
  ...
  function confirmAction() {
    ...
    if (confirm.kind === "hide") {
      update(confirm.item.id, (r) =>
        history(
          {
            ...r,
            exposure: "비노출",
            hideReason,
            hideDetail: hideDetail.trim() || null,
          },
          "비노출 처리",
          hideReason,
        ),
      ...
      setReviews((prev) =>
        prev.map((r) =>
          selectedIds.includes(r.id)
            ? history(
                { ...r, exposure: confirm.expose ? "노출" : "비노출" },
                confirm.expose ? "노출 처리 (일괄)" : "비노출 처리 (일괄)",
              )
            : r,
        ),
      );
      toastBriefly(
        `${selectedIds.length}건을 ${confirm.expose ? "노출" : "비노출"} 처리했습니다.`,
      );
      setSelectedIds([]);
    }
    setConfirm(null);
  ...
  function rowMenuItems(r: Review) {
    ...
    if (r.exposure === "노출") {
      ...
      items.push({
        label: "비노출",
        click: () => setConfirm({ kind: "hide", item: r }),
      });
      items.push({
        label: "삭제 처리",
        fg: "#dc2626",
        click: () => setConfirm({ kind: "delete", item: r }),
      });
  ...
  const rows: GridRow[] = filtered.map((r) => {
    ...
    const cells: Cell[] = [
      {
        kind: "text",
        text: productName(r.productCode),
        color: "#3f3f46",
        size: "12px",
        weight: 600,
      },
      ...
      {
        kind: "titleWarn",
        title: `${"★".repeat(r.rating)} ${r.content.length > 24 ? `${r.content.slice(0, 24)}…` : r.content}`,
      ...
      {
        kind: "text",
        text: r.createdAt.slice(5, 10).replace("-", "."),
        color: "#71717a",
        size: "11.5px",
        weight: 500,
        numeric: true,
    ...
    <div className={styles.page} onClick={() => menuId && setMenuId(null)}>
        ...
        <div className={styles.filterBox}>
          ...
          >
            <input
              ...
              placeholder="리뷰ID, 회원, 상품명 또는 상품코드 검색"
            />
      ...
      {selectedIds.length > 0 && (
        <div className={styles.bulkBar}>
          ...
          <button
            ...
            onClick={() => setConfirm({ kind: "bulkExposure", expose: true })}
          >
            선택 노출
          </button>
          <button
            type="button"
            className={styles.bulkBtn}
            onClick={() => setConfirm({ kind: "bulkExposure", expose: false })}
          >
            선택 비노출
          </button>
          <button
          ...
          emptyText={
            reviews.length === 0
              ...
              : quickFilter === "검토 필요"
                ? "현재 검토가 필요한 리뷰가 없습니다."
                : quickFilter === "신고 리뷰"
                  ? "현재 처리 대기 중인 신고 리뷰가 없습니다."
                  : "검색 조건에 해당하는 리뷰가 없습니다."
          }
          emptySubtext={
            reviews.length === 0
              ? "회원이 상품 리뷰를 작성하면 이곳에서 확인할 수 있습니다."
              : "검색어나 필터 조건을 변경해 주세요."
          }
          emptyActionLabel={reviews.length > 0 ? "초기화" : undefined}
          ...
          <div className={styles.dialogBox}>
            ...
            <div className={styles.dialogSummary}>
              <div className={styles.dialogSummaryRow}>
                <span>작성자</span>
                <span>{confirm.item.member}</span>
              </div>
              <div className={styles.dialogSummaryRow}>
                <span>상품</span>
                <span>{productName(confirm.item.productCode)}</span>
              </div>
            </div>
            <select
              className={styles.selectSm}
              ...
              value={hideReason}
              onChange={(e) => setHideReason(e.target.value as HideReason)}
            >
              {HIDE_REASONS.map((r) => (
            ...
            <input
              className={styles.searchInput}
              style={{ width: "100%", maxWidth: "none", marginBottom: 16 }}
      ...
      {confirm?.kind === "delete" && (
        ...
        >
          <div className={styles.dialogBox}>
            ...
            >{`'${confirm.item.member}'님의 리뷰가 삭제 처리됩니다. 사용자 화면에서는 삭제된 리뷰로 표시되며, 데이터는 감사 목적으로 보존됩니다.`}</div>
            <select
              className={styles.selectSm}
      ...
      {confirm?.kind === "bulkExposure" && (
        ...
        >
          <div className={styles.dialogBox}>
            <div className={styles.dialogTitle}>
              {selectedIds.length}건 {confirm.expose ? "노출" : "비노출"} 처리
            </div>
            <div className={styles.dialogBody}>
              선택한 리뷰를 모두 {confirm.expose ? "노출" : "비노출"} 상태로
              변경합니다.
            </div>
            <div className={styles.dialogActions}>
              <button
