...
export function ExternalTransactionPage() {
  ...
  const [recheckTarget, setRecheckTarget] =
    useState<ExternalTransaction | null>(null);

  const syncWarnings = useMemo(
    () => computeStaleSyncWarnings(txList),
    [txList],
  );
  ...
  const filtered = useMemo(
    () =>
      txList
        .filter((tx) => {
          if (!matchesQuickFilter(tx, quickFilter, txList, payments))
            return false;
          if (search) {
            const haystack =
              `${tx.id} ${tx.linkedPaymentId ?? ""}`.toLowerCase();
  ...
  const rows: GridRow[] = filtered.map((tx) => {
    ...
    return {
      ...
      cells: [
        ...
        {
          kind: "badge",
          text: STATUS_LABEL[tx.externalStatus],
          bg: statusColor.bg,
          fg: statusColor.fg,
        },
        {
          kind: "text",
          text: tx.linkedPaymentId ?? "-",
          size: "12px",
          color: tx.linkedPaymentId ? "#3f3f46" : "#a1a1aa",
        },
      ...
      <header className={shared.header}>
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              PG·은행 등 외부 결제 거래와 내부 결제 데이터의 일치 여부를
              확인합니다.
            </div>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className={shared.quickFilterCount}>
                      {
                        txList.filter((tx) =>
                          matchesQuickFilter(tx, filter, txList, payments),
                        ).length
                      }
                    </span>
                  </CommonButton>
        ...
        {syncWarnings.length > 0 && (
          <div className={styles.issueBanner}>
            ...
            {syncWarnings.map((w) => (
              <span key={w.pg}>
                ⚠ {w.pg} 거래 정보가 최근 동기화되지 않았습니다. 마지막 정상
                동기화: {w.lastSyncedAt}
              </span>
            ))}
          </div>
        ...
        <DataGrid
          ...
          emptyText={
            quickFilter === "미매칭"
              ? "현재 미매칭 외부 거래가 없습니다."
              : quickFilter === "확인 필요"
                ? "현재 확인이 필요한 외부 거래가 없습니다."
                : "검색 결과가 없습니다."
          }
          emptySubtext={
            quickFilter === "미매칭"
              ? "모든 외부 거래가 내부 결제와 정상적으로 연결되어 있습니다."
              : "검색어나 필터 조건을 변경해 주세요."
          }
        ...
        >
          <div className={shared.dialogBox}>
            ...
            <p className={shared.dialogBody}>
              외부 PG/은행에 거래 상태를 다시 조회합니다.
            </p>
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>외부 거래번호</span>
                <strong>{recheckTarget.id}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>외부 상태</span>
                <strong>{STATUS_LABEL[recheckTarget.externalStatus]}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>마지막 조회</span>
                <strong>{recheckTarget.lastSyncedAt}</strong>
              </div>
            </div>
