...
export function PaymentListPage() {
  ...
  useEffect(() => {
    const openPaymentId = (location.state as { openPaymentId?: string } | null)
      ?.openPaymentId;
    if (openPaymentId) {
      setQuickFilter("전체");
    ...
    () =>
      payments
        .filter((p) => {
          ...
          if (businessScope !== "통합" && p.businessType !== businessScope)
            return false;
          if (search) {
            const haystack =
              `${p.id} ${p.orderId} ${p.customerName} ${p.externalTxId ?? ""}`.toLowerCase();
            if (!haystack.includes(search.toLowerCase())) return false;
          }
          if (methodFilter && p.method !== methodFilter) return false;
          if (statusFilter && p.status !== statusFilter) return false;
          if (matchFilter && computeExternalMatch(p) !== matchFilter)
            return false;
    ...
    return {
      ...
      cells: [
        ...
        },
        {
          kind: "badge",
          text: p.businessType,
          bg: BUSINESS_BADGE_META[p.businessType].bg,
          fg: BUSINESS_BADGE_META[p.businessType].fg,
        },
        ...
        {
          kind: "stack",
          title: fmtWon(remainingAmount(p)),
          subtitle:
            p.refundedAmount > 0
              ? `환불 ${fmtWon(p.refundedAmount)}`
              : "환불 없음",
        },
        {
          kind: "statusDot",
    ...
    <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
        ...
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              B2C·C2C·B2B에서 발생한 결제 승인·취소·환불 상태를 통합 조회합니다.
            </div>
          </div>
        </div>
                    ...
                    <span className={shared.quickFilterCount}>
                      {
                        payments.filter(
                          (p) =>
                            (businessScope === "통합" ||
                              p.businessType === businessScope) &&
                            matchesQuickFilter(p, filter, payments),
                        ).length
                      }
                    </span>
                  </CommonButton>
          ...
          <div className={shared.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={matchFilter}
                onChange={(e) =>
                  setMatchFilter(e.target.value as ExternalMatchStatus | "")
                }
              >
            ...
            <button type="button" className={shared.clearBtn} onClick={reset}>
              초기화
            </button>
          </div>
          {showAdvanced && (
      ...
      {recheckTarget && (
        <div
          ...
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setRecheckTarget(null);
          }}
        >
          <div className={shared.dialogBox}>
            <h2 className={shared.dialogTitle}>결제 상태 재조회</h2>
            <p className={shared.dialogBody}>
              외부 PG에 결제 상태를 다시 조회합니다.
            </p>
            <div className={shared.dialogSummary}>
              <div className={shared.dialogSummaryRow}>
                <span>내부 상태</span>
                <strong>{recheckTarget.status}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>마지막 외부 상태</span>
                <strong>{recheckTarget.externalStatus ?? "없음"}</strong>
              </div>
              <div className={shared.dialogSummaryRow}>
                <span>최종 조회</span>
                <strong>
                  {recheckTarget.lastSyncedAt ?? "조회 이력 없음"}
                </strong>
              </div>
            </div>
