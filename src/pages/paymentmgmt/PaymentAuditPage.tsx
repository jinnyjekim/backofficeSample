  ...
  const filtered = useMemo(
    () =>
      PAYMENT_AUDIT_LOGS.filter((log) => {
        ...
        if (search) {
          const haystack =
            `${log.paymentId} ${log.orderId ?? ""} ${log.externalTxId ?? ""} ${log.actor} ${log.id}`.toLowerCase();
    ...
    return {
      ...
      cells: [
        { kind: "stack", title: date, subtitle: time },
        {
          kind: "stack",
          title: log.paymentId,
          subtitle: log.orderId ?? log.externalTxId ?? "",
        },
        { kind: "badge", text: log.category, bg: cc.bg, fg: cc.fg },
        {
          ...
          title:
            log.before && log.after
              ? `${log.before} → ${log.after}`
              : log.action,
          hasIssue: log.important,
          issueTitle: "중요 변경 항목입니다",
        },
        {
          kind: "pillText",
          text: log.actor,
          sub: actorLabel(log.actorType),
          bg: ac.bg,
          fg: ac.fg,
        },
          ...
          items: [
            ...
            ...(log.paymentId !== "-"
              ? [
                  {
                    label: "결제 보기",
                    click: () => window.location.assign("/payment-mgmt/list"),
                  },
                ]
              : []),
            ...(log.externalTxId
              ? [
                  {
                    label: "외부 거래 보기",
                    click: () =>
                      window.location.assign("/payment-mgmt/external"),
                  },
                ]
              : []),
          ],
    ...
    <div className={shared.page} onClick={() => openMenu && setOpenMenu(null)}>
      <header className={shared.header}>
        <div className={shared.headerTop}>
          <div>
            ...
            <div className={shared.subtitle}>
              결제와 관련하여 발생한 관리자 및 시스템 처리 내역을 조회합니다.
            </div>
          </div>
        </div>
          ...
          <div className={shared.filterRow2}>
            ...
            <label className="globalFilterField">
              ...
              <select
                ...
                value={resultFilter}
                onChange={(e) =>
                  setResultFilter(e.target.value as AuditResult | "")
                }
              >
        ...
        <p className={styles.readOnlyNotice}>
          결제 처리 이력은 금융 Audit 자료이며, 다운로드 자체도 처리 이력으로
          기록됩니다.
        </p>
      </header>
      ...
      <div className={shared.tableWrap}>
        <DataGrid
          ...
          emptyActionLabel={search || dateFrom || dateTo ? "초기화" : undefined}
      ...
      {selectedLogId &&
        (() => {
          const log = PAYMENT_AUDIT_LOGS.find(
            (item) => item.id === selectedLogId,
          );
          return log ? (
            <PaymentAuditDetailDrawer
              key={log.id}
              log={log}
