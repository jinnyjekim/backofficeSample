...
const GRID_TEMPLATE = '110px 140px 90px 90px 130px 100px minmax(160px, 1fr) 110px 120px 60px';
...
const QUICK_FILTERS = ['전체', '집하완료', '간선수송', '배달출발', '배송지연'] as const;
  ...
  const rows: GridRow[] = filtered.map((item) => {
    ...
    return {
      ...
      cells: [
        ...
        { kind: 'text', text: item.invoiceNo, color: '#1e40af', size: '12px', numeric: true, weight: 600 },
        { kind: 'badge', text: item.currentStep, bg: sm.bg, fg: sm.fg },
        { kind: 'text', text: item.currentLocation, color: '#3f3f46', size: '12px' },
        { kind: 'text', text: item.eta, color: '#18181b', size: '11.5px', weight: 500 },
        { kind: 'text', text: item.lastUpdated, color: '#71717a', size: '11.5px', numeric: true },
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerTop}>
          <div>
            ...
            <div className={styles.subtitle}>출고된 화물의 실시간 터미널 입출고, 간선 수송 및 배달원 출발 현황을 추적합니다.</div>
          </div>
        </div>
        ...
        <div className={styles.filterCard}>
          ...
          {QUICK_FILTERS.map((k) => {
            ...
            const count = items.filter((item) => k === '전체' || item.currentStep === k).length;
            return (
              <CommonButton
                key={k}
        ...
        <div className={styles.resultBar}>
          <span className={styles.resultLabel}>{`총 ${filtered.length}건 추적중`}</span>
      ...
      {selected && (
        ...
        >
          <div className={drawer.sectionTitleLoose}>배송 진행 단계 (타임라인)</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
            {selected.timeline.map((t, idx) => (
              <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '12.5px' }}>
                <span style={{ width: '9px', height: '9px', borderRadius: '50%', background: t.dot, marginTop: '4px', flexShrink: 0 }} />
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <strong style={{ color: '#18181b' }}>{t.title}</strong>
                  <span style={{ color: '#71717a', fontSize: '11.5px' }}>{t.when} {t.loc && `· ${t.loc}`} ({t.source})</span>
                </div>
              </div>
            ))}
          </div>
