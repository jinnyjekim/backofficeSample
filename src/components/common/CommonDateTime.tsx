...
const classNameOf = (
  value: CommonClassNames | undefined,
  key: "root" | "control" | "error" = "root",
) =>
  typeof value === "string"
    ? key === "root"
      ? value
...
export type CommonDatePickerProps = DatePickerBaseProps &
  Omit<
...
>(function CommonDatePicker(
  ...
  const singleValue = Array.isArray(value)
    ? null
    : (value ?? (Array.isArray(defaultValue) ? null : (defaultValue ?? null)));
  const parsedSingle = parseDate(singleValue);
  const parsedRange = [parseDate(rangeValue[0]), parseDate(rangeValue[1])] as [
    Date | null,
    Date | null,
  ];
  ...
  // key로 강제 리마운트해 defaultRange만으로 외부 값 변경(예: 초기화)까지 반영한다.
  const controlled =
    mode === "single" && value !== undefined && parsedSingle !== null;
  const packageValue = mode === "range" ? parsedRange : parsedSingle;
  const hiddenValue =
    mode === "range"
      ? rangeValue.filter(Boolean).join(" – ")
      : (singleValue ?? "");
  const rangeRemountKey =
    mode === "range"
  ...
  return (
    <span
      className={cx(
        ...
        styles[`datePicker_${size}`],
        classNameOf(classNames),
        className,
      )}
      aria-label={ariaLabel}
    >
      ...
      <M2MDatePicker
        ...
        classNames={cx(
          styles.datePickerAdapter,
          classNameOf(classNames, "control"),
        )}
      />
      {typeof error === "string" && (
        <span
          className={cx(styles.errorText, classNameOf(classNames, "error"))}
        >
          {error}
        </span>
      )}
...
export interface CommonTimePickerProps extends Omit<
  InputHTMLAttributes<HTMLInputElement>,
  "type" | "size" | "onChange"
> {
  value?: string;
...
export const CommonTimePicker = forwardRef<
  HTMLInputElement,
  ...
  return (
    ...
    >
      ...
      <M2MTimePicker
        ...
        onChange={onChange}
        format={format === "HH:mm:ss" ? "HH:mm:ss" : "HH:mm"}
        size={size}
        step={minuteStep}
        clearable={clearable}
        disabled={disabled}
